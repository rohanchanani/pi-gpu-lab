...
LogicalResult verifyNonNegativeI32Attr(Operation *op, StringRef attrName) {
  auto attr = op->getAttrOfType<IntegerAttr>(attrName);
  if (!attr)
    return op->emitOpError() << "requires integer attribute '" << attrName
                             << "'";
  if (attr.getInt() < 0)
    return op->emitOpError() << "requires non-negative '" << attrName << "'";
  return success();
}
...
LogicalResult GetUniformOp::verify() {
  return verifyNonNegativeI32Attr(getOperation(), "index");
}

LogicalResult DmaLoadOp::verify() {
  return verifyNonNegativeI32Attr(getOperation(), "slot");
}
...
LogicalResult StagedReadOp::verify() {
  return verifyNonNegativeI32Attr(getOperation(), "slot");
}

LogicalResult StagedWriteOp::verify() {
  return verifyNonNegativeI32Attr(getOperation(), "slot");
}
