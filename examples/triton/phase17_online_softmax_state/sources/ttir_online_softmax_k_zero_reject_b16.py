...
@triton.jit
def ttir_online_softmax_k_zero_reject_b16_kernel(
    S, VT, O, LDS, LDV, LDO, scale, K_ZERO: tl.constexpr, BLOCK_SIZE: tl.constexpr
):
    q = tl.program_id(0)
    d = tl.program_id(1)
    m = -80.0
    l = 0.0
    acc = 0.0
    for start in tl.range(0, K_ZERO, BLOCK_SIZE):
