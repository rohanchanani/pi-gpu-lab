...
@triton.jit
def mixed_ttir_online_attention_apply_axes_mask_cf_f16_storage_b16_kernel(
    S, VT, O, AUDIT, K, LDS, LDV, LDO, scale, BLOCK_SIZE: tl.constexpr
):
    q = tl.program_id(0)
    d = tl.program_id(1)
    ...
    if group == 0:
        ...
        for start in tl.range(0, K, BLOCK_SIZE):
            ...
            mask = offs < K
            scores_raw = tl.load(S + q * LDS + offs, mask=mask, other=0.0).to(
                tl.float32
            )
