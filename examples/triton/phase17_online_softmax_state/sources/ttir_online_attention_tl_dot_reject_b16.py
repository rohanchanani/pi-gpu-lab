...
@triton.jit
def ttir_online_attention_tl_dot_reject_b16_kernel(
    S, VT, O, K, LDS, LDV, LDO, scale, BLOCK_SIZE: tl.constexpr
):
    q = tl.program_id(0)
    d = tl.program_id(1)
    ...
    e = tl.exp(active_scores - m)
    e = tl.where(mask, e, 0.0)
    v = tl.load(VT + d * LDV + offs, mask=mask, other=0.0)
    weighted = tl.dot(tl.expand_dims(e, 0), tl.expand_dims(v, 1))
