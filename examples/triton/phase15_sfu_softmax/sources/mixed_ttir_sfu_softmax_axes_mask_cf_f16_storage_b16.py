...
@triton.jit
def mixed_ttir_sfu_softmax_axes_mask_cf_f16_storage_b16(x_ptr, scale_ptr, out_ptr, audit_ptr, ncols, stride, BLOCK_SIZE: tl.constexpr):
    row = tl.program_id(axis=0)
    group = tl.program_id(axis=1)
    offs = tl.arange(0, BLOCK_SIZE)
    mask = offs < ncols
    x = tl.load(x_ptr + row * stride + offs, mask=mask, other=0.0).to(tl.float32)
    scale = tl.load(scale_ptr + row, mask=group == 0, other=1.0)
