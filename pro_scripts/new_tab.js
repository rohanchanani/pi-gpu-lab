...
function forceProcessExitSoon() {
  // We intentionally do not call browser.close() here because this script is
  // attached to an externally launched Chrome instance. Calling close/closing
  // the page can close the ChatGPT tab, which we want to keep open.
  setTimeout(() => {
...
`.trim();
  ...
  try {
    ...
    if (!context) {
      throw new Error(`No existing Chrome context found at ${CDP_URL}`);
    }
    ...
    const manifest = {
      ok: written.length > 0,
      mode: "new-chatgpt-tab-gptweb-file-parser",
    ...
    // Default behavior: leave the generated ChatGPT tab open, but cleanly
    // detach Playwright from the externally launched Chrome instance.
    await browser.disconnect().catch(() => {});
    forceProcessExitSoon();
  }
}
