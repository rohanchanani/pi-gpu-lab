...
This note captures the current prototype ABI for the working SAXPY slice. It is intentionally narrow and should describe the code as it exists today, not a broader future design.

## Source-Side Kernel ABI

The supported source kernel shape for `--lower-gpu-saxpy-to-vc4` is:
...
For the working SAXPY slice, the lowered IR uses:
...
The current prototype runtime-side uniform payload for the working SAXPY slice is:
...
This note defines the current prototype runtime layout for the working SAXPY slice. It does not imply that the final runtime ABI is frozen, only that the current prototype should be treated consistently while the backend slice is being stabilized.
...
The current SAXPY slice therefore assumes these values fit in 32 bits.
...
This is a lowering detail for the current VC4 slice. It reflects the current assumption that uniform reads are lane-broadcast across the width-16 execution group, so the lowered VC4 compute path consumes alpha as a vector value without an explicit `vector.broadcast` op.
...
- treat `compiler/examples/saxpy_runtime_abi.h` as the concrete prototype
