...
Phase 17 closes online/multiblock softmax state for precomputed-score
attention apply. The next ladder target is the separate vector.contract /
`tl.dot` / `tt.dot` feature increment. Phase 17 does not claim QK score
generation, dot/contract, non-transposed V gather, scalar global load, block
pointers, cooperative/VPM tiling, full attention, or FlashAttention.
