...
**Scope:** finish the scheduled-VC4 artifact/runtime backend so later `gpu -> vc4` lowering has a stable CUDA-like target  
**Non-scope:** full structured-VC4 lowering, register allocation, instruction scheduling from structured ops, or `gpu` dialect lowering
...
M2 should not lower structured `vc4` SSA to scheduled QPU instructions. That remains M3. M2 may add metadata contracts that structured-VC4 and `gpu` lowering will later target.
