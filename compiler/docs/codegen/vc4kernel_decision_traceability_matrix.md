...
| Dynamic rotate/shuffle | dynamic rotate hardware-proven pending final acceptance; lane broadcast accepted as composite | P11 admits dynamic rotate and the lane-broadcast composite only where hardware proof exists; no first-class broadcast op. | Verifier/conversion/hardware proof, including `fragment_broadcast_lane_composite_vc4kernel`. |
