...
## 38. Phase 16 final attention-apply TTIR lock
...
The accepted TTIR profile for Phase 16 is restricted to controlled real TTIR
snapshots for precomputed-score attention apply with transposed V, stable
...
## 40. Phase 17 final online softmax TTIR lock
...
Phase 17 final-locks controlled real TTIR online/multiblock softmax state.
The accepted TTIR path remains TTIR to standard value IR, then value to
VC4Kernel, SSAVC4, scheduled VC4, and hardware. The final TTIR mixed fixture
proves the source-controlled mixed online attention snapshot through the C++
importer with online `m/l/acc` state, K greater than 16, precomputed scores,
transposed V, natural-exp recurrent softmax, weighted sum reduction, scalar
stores, strict sentinels, and `active_qpus=12`.
