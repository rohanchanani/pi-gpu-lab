...
module {
  vc4kernel.kernel @bad(%x : f32) attributes {
    ...
    // CHECK: f32 fragment_cmp requires finite_only policy in P3
    %cmp = vc4kernel.fragment_cmp %v, %v {predicate = #vc4kernel.cmp<olt>} : vector<16xf32>, vector<16xf32> -> !vc4kernel.pred<16>
    vc4kernel.return
  }
}
