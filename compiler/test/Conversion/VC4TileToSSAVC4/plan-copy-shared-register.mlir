...
// CHECK-LABEL: vc4tile.kernel @plan_copy_shared_register
// CHECK-NOT: vc4tile.copy_tile
// CHECK: vc4tile.shared_store
// CHECK: vc4tile.shared_load
vc4tile.kernel @plan_copy_shared_register(%out : i32) attributes {
  public_name = "plan_copy_shared_register",
  ...
  %loaded = "vc4tile.copy_tile"(%shared, %zero, %mask) {shape = [1, 16], src_space = #vc4tile.memory_space<shared_vpm>, dst_space = #vc4tile.memory_space<register>, src_layout = #vc4tile.layout<vpm_row>, dst_layout = #vc4tile.layout<row_major>, element_type = i32, storage_type = i32, precision = #vc4tile.precision<exact_32>, packing = #vc4tile.packing<none>, elem_bytes = 4 : i32} : (!vc4tile.shared_tile, i32, vector<16xi1>) -> vector<16xi32>
  vc4tile.return
}
