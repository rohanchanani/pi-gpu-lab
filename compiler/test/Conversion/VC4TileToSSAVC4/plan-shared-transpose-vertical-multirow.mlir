...
vc4tile.kernel @plan_shared_transpose_vertical_multirow_aligned(%out : i32, %in : i32) attributes {
  ...
  "vc4tile.tile_store"(%view, %out, %zero, %mask) {shape = [16, 16], layout = #vc4tile.layout<transposed_view>, memory_space = #vc4tile.memory_space<global>, element_type = i32, storage_type = i32, precision = #vc4tile.precision<exact_32>, packing = #vc4tile.packing<none>, shared_row = 0 : i32} : (!vc4tile.shared_tile, i32, i32, vector<16xi1>) -> ()
  vc4tile.return
}
...
vc4tile.kernel @plan_shared_transpose_vertical_multirow_split(%out : i32, %in : i32) attributes {
  ...
  %view = "vc4tile.transpose_view"(%shared) {permutation = [1, 0]} : (!vc4tile.shared_tile) -> !vc4tile.shared_tile
  "vc4tile.tile_store"(%view, %out, %zero, %mask) {shape = [16, 16], layout = #vc4tile.layout<transposed_view>, memory_space = #vc4tile.memory_space<global>, element_type = i32, storage_type = i32, precision = #vc4tile.precision<exact_32>, packing = #vc4tile.packing<none>, shared_row = 12 : i32} : (!vc4tile.shared_tile, i32, i32, vector<16xi1>) -> ()
  vc4tile.return
}
