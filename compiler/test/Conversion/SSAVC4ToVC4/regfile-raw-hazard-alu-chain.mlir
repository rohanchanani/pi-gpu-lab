...
// CHECK-LABEL: vc4.module @raw_hazard_alu_chain
...
// CHECK-NOT: ssavc4.
ssavc4.module @raw_hazard_alu_chain {
  ssavc4.func @kernel() attributes {kernel, threading = #vc4.threading_mode<single>} {
    %x = ssavc4.load_imm <splat32> {value = 7 : i32} : i32
    %y = ssavc4.alu.add %x, %x {opcode = #vc4.add_opcode<add>} : (i32, i32) -> i32
    %z = ssavc4.alu.add %y, %x {opcode = #vc4.add_opcode<add>} : (i32, i32) -> i32
    ssavc4.thread_end
  }
}
