...
// CHECK-LABEL: vc4.func @vdw_dynamic_cols_vpm_pitch2_fallback_kernel
// CHECK: value = 2 : i32
// CHECK: vc4.qpu.vpmvcd_setup {{.*}}side = #vc4.vpmvcd_side<write>
// CHECK: vc4.qpu.vpmvcd_addr {{.*}}side = #vc4.vpmvcd_side<write>
// CHECK: value = 4 : i32
...
ssavc4.module @vdw_true_rect_dynamic_shape {
  ...
  ssavc4.func @vdw_dynamic_cols_vpm_pitch2_fallback_kernel() attributes {
    kernel,
    threading = #vc4.threading_mode<single>,
    "vc4.launch_abi" = {
      public_name = "vdw_dynamic_cols_vpm_pitch2_fallback",
      code_symbol = "vdw_dynamic_cols_vpm_pitch2_fallback_shader",
      tail_policy = "exact_multiple",
    ...
    %source_row = ssavc4.load_imm <splat32> {value = 3 : i32} : i32
    %rows4 = ssavc4.load_imm <splat32> {value = 4 : i32} : i32
    %stride = ssavc4.load_imm <splat32> {value = 64 : i32} : i32

    ssavc4.vdw.store_rect.dynamic %out, %source_row, %rows4, %active_cols, %stride {
      max_rows = 4 : i32,
      max_cols = 16 : i32,
      elem_bytes = 4 : i32,
      orientation = #ssavc4.vpm_orientation<horizontal>,
