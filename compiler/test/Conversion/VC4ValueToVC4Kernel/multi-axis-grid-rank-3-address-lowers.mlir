...
func.func @grid3_linear_address(
    %out: memref<512xi32, #vc4value.global> {vc4value.arg_name = "out", vc4value.direction = "out"},
  ...
  %plane_base = arith.muli %pid2, %ny : index
  %row = arith.addi %plane_base, %pid1 : index
  %row_base = arith.muli %row, %nx : index
  %block_id = arith.addi %row_base, %pid0 : index
  %idx = arith.muli %block_id, %c16 : index
  %mask = vector.create_mask %n : vector<16xi1>
  %value = arith.constant dense<11> : vector<16xi32>
  vector.transfer_write %value, %out[%idx], %mask {in_bounds = [true]} : vector<16xi32>, memref<512xi32, #vc4value.global>
  return
}

// CHECK-LABEL: vc4kernel.kernel @grid3_linear_address
