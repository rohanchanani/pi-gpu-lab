...
func.func @write_i32_full(%out: memref<64xi32, #vc4value.global> {vc4value.arg_name = "out", vc4value.direction = "out"},
                          %base: index {vc4value.arg_name = "base"})
    attributes {vc4value.kernel, vc4value.grid_rank = 1 : i32} {
  %value = arith.constant dense<17> : vector<16xi32>
  vector.transfer_write %value, %out[%base] {in_bounds = [true]} : vector<16xi32>, memref<64xi32, #vc4value.global>
  return
}

// CHECK-LABEL: vc4kernel.kernel @write_i32_full
// CHECK: vc4kernel.fragment_const
// CHECK-SAME: dense<17> : vector<16xi32>
