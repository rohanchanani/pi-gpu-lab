...
// CHECK: ssavc4.vdw.store_vpm %{{.*}}, %{{.*}}, %{{.*}} {
// CHECK-SAME: orientation = #ssavc4.vpm_orientation<vertical>
// CHECK: ssavc4.vdw.store_vpm %{{.*}}, %{{.*}}, %{{.*}} dynamic_subword_selector %{{.*}} {
// CHECK-SAME: width = #ssavc4.vpm_elem_width<w8>
// CHECK: ssavc4.vdw.store_rect.dynamic %{{.*}}, %{{.*}} dynamic_src_x %{{.*}}, %{{.*}}, %{{.*}}, %{{.*}} {
// CHECK-SAME: orientation = #ssavc4.vpm_orientation<horizontal>
// CHECK: ssavc4.vdw.store_rect.dynamic %{{.*}}, %{{.*}} dynamic_src_x %{{.*}}, %{{.*}}, %{{.*}}, %{{.*}} {
// CHECK-SAME: orientation = #ssavc4.vpm_orientation<vertical>
// CHECK: ssavc4.vdw.store_rect.dynamic %{{.*}}, %{{.*}} dynamic_src_x %{{.*}} dynamic_subword_selector %{{.*}}, %{{.*}}, %{{.*}}, %{{.*}} {
// CHECK-SAME: width = #ssavc4.vpm_elem_width<w16>
...
module {
  vc4kernel.kernel @dynamic_vpm_coords(%in : i32, %out : i32, %x_arg : i32) attributes {
    ...
    vc4kernel.vdr_load_to_vpm %in, %c0, %tile, %c0 dynamic_dst_x %x_arg dynamic_subword_selector %sel {rows = 1 : i32, cols = 16 : i32, global_stride_bytes = 16 : i32, elem_bytes = 1 : i32, orientation = #vc4kernel.vpm_orientation<horizontal>, width = #vc4kernel.vpm_width<w8>, subword = #vc4kernel.vpm_subword<packed>, vpm_pitch = 1 : i32, memory_path = #vc4kernel.memory_path<vdr_global_to_vpm>, coherency = #vc4kernel.coherency<dma_ordered>} : i32, i32, !vc4kernel.vpm_tile, i32 dynamic_dst_x i32 dynamic_subword_selector i32
