...
module {
  vc4kernel.kernel @tmu_tail(%ptr : i32, %base_index : i32, %limit : i32) attributes {
    ...
    %v = vc4kernel.tmu_load_fragment %ptr, %offs, %tail {memory_path = #vc4kernel.memory_path<tmu_global_read>, coherency = #vc4kernel.coherency<readonly_tmu>} : i32, vector<16xi32>, !vc4kernel.pred<16> -> vector<16xi32>
    vc4kernel.return
  }
}
