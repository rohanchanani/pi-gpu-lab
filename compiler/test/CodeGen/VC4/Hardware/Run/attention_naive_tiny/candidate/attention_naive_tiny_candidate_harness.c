...
struct attention_naive_tiny_case { uint32_t q_len, k_len, d; float scale; uint32_t equal_scores; };

static const struct attention_naive_tiny_case attention_cases[ATTENTION_NAIVE_TINY_CASES] = {
...
static void fill_case(uint32_t case_id, const struct attention_naive_tiny_case *tc) {
    for (uint32_t i = 0; i < ATTENTION_NAIVE_TINY_MAX_Q_WORDS; i++) q_values[i] = 0.0f;
    for (uint32_t i = 0; i < ATTENTION_NAIVE_TINY_MAX_K_WORDS; i++) k_values[i] = 0.0f;
    for (uint32_t i = 0; i < ATTENTION_NAIVE_TINY_MAX_V_WORDS; i++) v_values[i] = 0.0f;
    for (uint32_t i = 0; i < ATTENTION_NAIVE_TINY_MAX_OUT_WORDS; i++) expected_values[i] = 0.0f;
    for (uint32_t i = 0; i < ATTENTION_NAIVE_TINY_MAX_OUT_WORDS + ATTENTION_NAIVE_TINY_GUARD_WORDS; i++) out_values[i] = ATTENTION_NAIVE_TINY_SENTINEL;
...
void notmain(void) {
    ...
    uint32_t q_bytes = ATTENTION_NAIVE_TINY_MAX_Q_WORDS * sizeof(float);
    uint32_t k_bytes = ATTENTION_NAIVE_TINY_MAX_K_WORDS * sizeof(float);
    uint32_t v_bytes = ATTENTION_NAIVE_TINY_MAX_V_WORDS * sizeof(float);
    uint32_t out_bytes = (ATTENTION_NAIVE_TINY_MAX_OUT_WORDS + ATTENTION_NAIVE_TINY_GUARD_WORDS) * sizeof(float);
    ...
    if (vc4_m2_malloc(program, &q_dev, q_bytes) < 0 || vc4_m2_malloc(program, &k_dev, k_bytes) < 0 ||
        ...
        panic("attention_naive_tiny device allocation failed");

    vc4_dim3 block = vc4_m2_dim3(ATTENTION_NAIVE_TINY_LANES, 1u, 1u);
    printk("ATTENTION_NAIVE_TINY_RUNTIME_SETUP max_q_len=%d max_k_len=%d max_d=%d allocations=1\n",
           ATTENTION_NAIVE_TINY_MAX_Q_LEN, ATTENTION_NAIVE_TINY_MAX_K_LEN, ATTENTION_NAIVE_TINY_MAX_D);

    for (uint32_t case_id = 0; case_id < ATTENTION_NAIVE_TINY_CASES; case_id++) {
        const struct attention_naive_tiny_case *tc = &attention_cases[case_id];
        ...
        if (vc4_m2_copy_htod(program, q_dev, q_values, q_bytes) < 0 ||
            ...
            attention_naive_tiny_launch(program, grid, block, q_dev, k_dev, v_dev, out_dev, tc->q_len, tc->k_len, tc->d, tc->scale) < 0 ||
            vc4_m2_copy_dtoh(program, out_values, out_dev, out_bytes) < 0) {
            launch_failures++;
            printk("ATTENTION_NAIVE_TINY_CASE case=%d q_len=%d k_len=%d d=%d launch=FAIL launches=%d allocations=1\n",
        ...
        for (uint32_t i = 0; i < logical_outputs; i++) {
            ...
            if (invalid_f32(out_values[i]) || adiff > ATTENTION_NAIVE_TINY_TOLERANCE) {
                if (mismatches < 8u)
        ...
        printk("ATTENTION_NAIVE_TINY_CASE case=%d q_len=%d k_len=%d d=%d checked=%d mismatches=%d sentinel_mismatches=%d checksum=%d max_abs_diff=%f launches=%d allocations=1\n",
    ...
    const char *status = (total_mismatches == 0u && sentinel_mismatches == 0u && launch_failures == 0u &&
                          checked_elements > 0u && global_max_abs_diff <= ATTENTION_NAIVE_TINY_TOLERANCE) ? "PASS" : "FAIL";
    printk("VC4_TEST_RESULT name=attention_naive_tiny status=%s cases=%d checked_elements=%d total_mismatches=%d sentinel_mismatches=%d launch_failures=%d active_qpus=%d lanes=%d max_q_len=%d max_k_len=%d max_d=%d checksum_accum=%d max_abs_diff=%f runtime_allocations=%d runtime_launches=%d elapsed_usec=%d\n",
           status, ATTENTION_NAIVE_TINY_CASES, (int)checked_elements, (int)total_mismatches, (int)sentinel_mismatches,
           (int)launch_failures, ATTENTION_NAIVE_TINY_ACTIVE_QPUS, ATTENTION_NAIVE_TINY_LANES,
           ATTENTION_NAIVE_TINY_MAX_Q_LEN, ATTENTION_NAIVE_TINY_MAX_K_LEN, ATTENTION_NAIVE_TINY_MAX_D,
           checksum_accum, global_max_abs_diff, 1, ATTENTION_NAIVE_TINY_CASES, elapsed);
