...
void notmain(void) {
    ...
    uint32_t input_bytes = SHARED_TRANSPOSE_16X16_WORDS * sizeof(uint32_t);
    uint32_t output_bytes = (SHARED_TRANSPOSE_16X16_WORDS + HOST_GUARD_WORDS) * sizeof(uint32_t);

    if (vc4_program_create(&program, 0) < 0 || !program)
        panic("vc4_program_create failed");
    if (vc4Malloc(program, &input_dev, input_bytes) < 0 ||
        vc4Malloc(program, &output_dev, output_bytes) < 0)
        panic("shared_transpose_16x16 device allocation failed");

    uint32_t active_qpus = 12u;
    uint32_t lane_width = 16u;
    vc4_dim3 grid = vc4_m2_dim3(1u, 1u, 1u);
    vc4_dim3 block = vc4_m2_dim3(SHARED_TRANSPOSE_16X16_WARPS_PER_BLOCK * lane_width, 1u, 1u);

    printk("Running VC4 shared_transpose_16x16 M2 candidate bundle...\n");
    printk("SHARED_TRANSPOSE_16X16_RUNTIME_SETUP allocations=%d active_qpus=%d lanes=%d warps_per_block=%d\n",
           1, (int)active_qpus, (int)lane_width, SHARED_TRANSPOSE_16X16_WARPS_PER_BLOCK);
    ...
    for (uint32_t case_id = 0; case_id < SHARED_TRANSPOSE_16X16_CASES; case_id++) {
        fill_case(case_id);
        ...
        uint32_t checksum = checksum_words(output_values, SHARED_TRANSPOSE_16X16_WORDS);
    ...
    printk("VC4_TEST_RESULT name=shared_transpose_16x16 status=%s cases=%d total_mismatches=%d sentinel_mismatches=%d launch_failures=%d active_qpus=%d lanes=%d warps_per_block=%d runtime_allocations=%d runtime_launches=%d timeouts=%d errstat_relevant_changed=%d checksum_accum=%x elapsed_usec=%d\n",
           status, SHARED_TRANSPOSE_16X16_CASES, total_mismatches,
           sentinel_mismatches, launch_failures, (int)active_qpus,
           (int)lane_width, SHARED_TRANSPOSE_16X16_WARPS_PER_BLOCK,
           1, SHARED_TRANSPOSE_16X16_CASES, 0, 0, checksum_accum, elapsed);
