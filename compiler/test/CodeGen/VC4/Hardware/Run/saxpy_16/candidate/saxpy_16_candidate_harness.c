...
void notmain(void) {
    ...
    printk("VC4_TEST_RESULT name=saxpy_16 status=%s mismatches=%d active_qpus=%d n=%d chunks_per_qpu=%d checksum=%d max_abs_diff=%f elapsed_usec=%d\n",
           status, mismatches, (int)activeQpus, (int)n, (int)SAXPY_16_CHUNKS_PER_QPU, checksum, max_abs_diff, elapsed);
