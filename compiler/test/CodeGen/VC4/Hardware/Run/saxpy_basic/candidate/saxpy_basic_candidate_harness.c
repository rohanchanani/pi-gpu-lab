...
#define SAXPY_BASIC_GUARD_WORDS 16u
#define SAXPY_BASIC_SENTINEL_BASE 12345.0f

static float x_values[SAXPY_BASIC_N];
static float y_values[SAXPY_BASIC_N + SAXPY_BASIC_GUARD_WORDS];
...
static void fill_inputs(uint32_t n) {
    ...
    for (uint32_t i = n; i < n + SAXPY_BASIC_GUARD_WORDS; i++)
        y_values[i] = SAXPY_BASIC_SENTINEL_BASE + (float)i;
}
...
static int verify_guard(uint32_t n) {
    int mismatches = 0;
    for (uint32_t i = n; i < n + SAXPY_BASIC_GUARD_WORDS; i++) {
        float expected = SAXPY_BASIC_SENTINEL_BASE + (float)i;
...
void notmain(void) {
    ...
    uint32_t y_bytes = (n + SAXPY_BASIC_GUARD_WORDS) * sizeof(float);
    ...
    uint32_t runtime_allocations = saxpy_basic_runtime_allocations();
    uint32_t runtime_launches = saxpy_basic_runtime_launches();

    const char *status =
        (mismatches == 0 && sentinel_mismatches == 0 && launchFailures == 0) ? "PASS" : "FAIL";
    printk("VC4_TEST_RESULT name=saxpy_basic status=%s checked_elements=%d mismatches=%d sentinel_mismatches=%d launch_failures=%d active_qpus=%d lanes=%d n=%d checksum=%d max_abs_diff=%f runtime_allocations=%d runtime_launches=%d elapsed_usec=%d\n",
           status, (int)n, mismatches, sentinel_mismatches, launchFailures,
           (int)activeQpus, (int)laneWidth, (int)n, checksum, max_abs_diff,
           (int)runtime_allocations, (int)runtime_launches, elapsed);
    if (mismatches || sentinel_mismatches || launchFailures)
        panic("saxpy_basic verification failed: mismatches=%d sentinel_mismatches=%d launch_failures=%d",
