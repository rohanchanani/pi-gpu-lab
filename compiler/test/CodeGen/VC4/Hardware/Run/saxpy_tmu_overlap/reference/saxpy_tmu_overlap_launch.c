...
#include <stddef.h>
#include <string.h>
#include "saxpy_tmu_overlap_launch.h"
#include "saxpy_tmu_overlapshader.h"
...
struct saxpy_tmu_overlap_launch_state
{
    uint32_t code[sizeof(saxpy_tmu_overlapshader) / sizeof(uint32_t)];
...
static size_t saxpy_tmu_overlap_state_size(uint32_t n)
{
    return offsetof(struct saxpy_tmu_overlap_launch_state, payload) +
           (size_t)2 * n * sizeof(float);
}

static float *saxpy_tmu_overlap_x_ptr(volatile struct saxpy_tmu_overlap_launch_state *state)
{
    return (float *)state->payload;
}

static float *saxpy_tmu_overlap_y_ptr(
    volatile struct saxpy_tmu_overlap_launch_state *state,
    uint32_t n)
{
    ...
    uint32_t n)
{
    ...
    size_t allocSize = saxpy_tmu_overlap_state_size(n);
    ...
    volatile struct saxpy_tmu_overlap_launch_state *state =
        (volatile struct saxpy_tmu_overlap_launch_state *)(vc - GPU_BASE);
    if (!state)
    {
        mem_unlock(handle);
        mem_free(handle);
    ...
    state->handle = handle;
    memcpy((void *)state->code, saxpy_tmu_overlapshader, sizeof state->code);

    float *gpuX = saxpy_tmu_overlap_x_ptr(state);
    float *gpuY = saxpy_tmu_overlap_y_ptr(state, n);
