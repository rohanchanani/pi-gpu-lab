...
void notmain(void)
{
    ...
    printk("Running VC4 saxpy_tmu_overlap reference bundle...\n");
    int start = timer_get_usec();
    if (saxpy_tmu_overlap_launch(&rt, x_values, y_values, alpha, n) < 0)
        panic("saxpy_tmu_overlap launch failed");
    ...
    printk("saxpy_tmu_overlap alpha=%f qpus=%d lanes=%d n=%d checksum=%d\n",
           alpha,
           (int)activeQpus,
           (int)laneWidth,
           (int)n,
    ...
    if (mismatches)
        panic("saxpy_tmu_overlap verification failed: mismatches=%d max_abs_diff=%f",
              mismatches,
              max_abs_diff);

    printk("VC4_TEST_RESULT name=saxpy_tmu_overlap status=PASS mismatches=%d active_qpus=%d n=%d checksum=%d max_abs_diff=%f elapsed_usec=%d\n",
           mismatches,
           (int)activeQpus,
           (int)n,
           checksum,
