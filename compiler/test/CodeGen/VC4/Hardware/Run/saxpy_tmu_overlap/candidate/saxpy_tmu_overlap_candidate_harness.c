...
#define SAXPY_TMU_OVERLAP_GUARD_WORDS 16u
#define SAXPY_TMU_OVERLAP_TOTAL_WORDS (SAXPY_TMU_OVERLAP_N + SAXPY_TMU_OVERLAP_GUARD_WORDS)
#define SAXPY_TMU_OVERLAP_GUARD_BASE 0x5a8b0000u

static float x_values[SAXPY_TMU_OVERLAP_TOTAL_WORDS];
static float y_values[SAXPY_TMU_OVERLAP_TOTAL_WORDS];
...
static uint32_t guard_word(uint32_t case_index, uint32_t word) {
    return SAXPY_TMU_OVERLAP_GUARD_BASE ^ (case_index << 8) ^ word;
}
...
static void fill_inputs(uint32_t case_index, uint32_t n) {
    ...
    for (uint32_t i = 0; i < SAXPY_TMU_OVERLAP_GUARD_WORDS; i++) {
...
void notmain(void) {
    ...
    uint32_t bytes = SAXPY_TMU_OVERLAP_TOTAL_WORDS * sizeof(float);
    ...
    for (uint32_t case_index = 0; case_index < case_count; case_index++) {
        ...
        fill_inputs(case_index, SAXPY_TMU_OVERLAP_N);
        ...
        if (vc4_m2_copy_htod(program, x_dev, x_values, bytes) < 0 ||
            ...
            saxpy_tmu_overlap_launch(program, grid, block, x_dev, y_dev, alpha, SAXPY_TMU_OVERLAP_N) < 0 ||
        ...
        verify_results(case_index, SAXPY_TMU_OVERLAP_N, &mismatches, &guard_mismatches, &max_abs_diff);
        ...
        printk("SAXPY_TMU_OVERLAP_CASE case=%d alpha=%f mismatches=%d guard_mismatches=%d checksum=%d expected_checksum=%d max_abs_diff=%f\n",
               (int)case_index, alpha, mismatches, guard_mismatches, checksum,
               expected_checksum, max_abs_diff);
    }
    int elapsed = timer_get_usec() - start;

    uint32_t runtime_allocations = saxpy_tmu_overlap_runtime_allocations();
    uint32_t runtime_launches = saxpy_tmu_overlap_runtime_launches();
    uint32_t runtime_capacity = saxpy_tmu_overlap_runtime_capacity();
    uint32_t code_uploads = saxpy_tmu_overlap_runtime_code_uploads();
    uint32_t recorded_launch_failures = saxpy_tmu_overlap_runtime_launch_failures();
