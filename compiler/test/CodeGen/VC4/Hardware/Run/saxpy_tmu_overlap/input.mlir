...
// Final-stage scheduled-sink input for the saxpy_tmu_overlap_overlap hardware golden.
//
// Semantics:
...
//   [5] num_qpus
//
// This is the latency-hiding counterpart to saxpy_tmu: x and y come from two
// independent TMU0 direct-memory requests that are both issued before the first
// TMU receive. The arithmetic stays as fmul/fadd, and the output path remains
// VPM -> VDW DMA store.

vc4.module @saxpy_tmu_overlap {
  vc4.func @saxpy_tmu_overlap_kernel() attributes {
    ...
    // t0s = y + offset + lane*4, issuing a second independent request before
    // synchronizing on the first. Direct s-only requests consume one TFREQ slot
    // each, so two outstanding requests are intentionally within the reliable
    // subset of the FIFO contract.
    ...
    // Spacer while both TMU requests are in flight.
    ...
    // Receive x. It arrives in r4 for the following instruction.
    ...
    // r2 = alpha * x, and in the same scheduled instruction signal the receive
    // for y. The TMU receive order is FIFO, so this second ldtmu0 consumes the
    // y request issued above.
    vc4.qpu.bundle {sig = #vc4.qpu_signal<ldtmu0>, pm = false, cond_add = #vc4.cond<never>, cond_mul = #vc4.cond<always>, waddr_add = 39 : i32, waddr_mul = 34 : i32, op_add = #vc4.add_opcode<nop>, op_mul = #vc4.mul_opcode<fmul>, raddr_a = 2 : i32, raddr_b = 0 : i32, add_a = #vc4.qpu_mux<r0>, add_b = #vc4.qpu_mux<r1>, mul_a = #vc4.qpu_mux<r4>, mul_b = #vc4.qpu_mux<a>}

    // r2 = alpha * x + y, with y now available in r4.
    vc4.qpu.bundle {sig = #vc4.qpu_signal<none>, pm = false, cond_add = #vc4.cond<always>, cond_mul = #vc4.cond<never>, waddr_add = 34 : i32, waddr_mul = 39 : i32, op_add = #vc4.add_opcode<fadd>, op_mul = #vc4.mul_opcode<nop>, raddr_a = 0 : i32, raddr_b = 1 : i32, add_a = #vc4.qpu_mux<r2>, add_b = #vc4.qpu_mux<r4>, mul_a = #vc4.qpu_mux<r0>, mul_b = #vc4.qpu_mux<r1>}
    ...
    // vw_setup = r3 + row
    vc4.qpu.bundle {sig = #vc4.qpu_signal<none>, pm = false, cond_add = #vc4.cond<always>, cond_mul = #vc4.cond<never>, waddr_add = 49 : i32, waddr_mul = 39 : i32, op_add = #vc4.add_opcode<add>, op_mul = #vc4.mul_opcode<nop>, raddr_a = 8 : i32, raddr_b = 39 : i32, add_a = #vc4.qpu_mux<r3>, add_b = #vc4.qpu_mux<a>, mul_a = #vc4.qpu_mux<r0>, mul_b = #vc4.qpu_mux<r1>}
    ...
    // vw_addr = y + offset
    vc4.qpu.bundle {sig = #vc4.qpu_signal<none>, pm = false, cond_add = #vc4.cond<always>, cond_mul = #vc4.cond<never>, waddr_add = 50 : i32, waddr_mul = 39 : i32, op_add = #vc4.add_opcode<or>, op_mul = #vc4.mul_opcode<nop>, raddr_a = 7 : i32, raddr_b = 39 : i32, add_a = #vc4.qpu_mux<a>, add_b = #vc4.qpu_mux<a>, mul_a = #vc4.qpu_mux<r0>, mul_b = #vc4.qpu_mux<r1>}
