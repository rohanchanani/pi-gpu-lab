ssavc4.module @vdw_user_store_with_spill_transport_ssavc4 {
  ssavc4.func @vdw_user_store_with_spill_transport_ssavc4_kernel() attributes {
    kernel,
    threading = #vc4.threading_mode<single>,
    "vc4.launch_abi" = {
      public_name = "vdw_user_store_with_spill_transport_ssavc4",
      code_symbol = "vdw_user_store_with_spill_transport_ssavc4_shader",
      tail_policy = "exact_multiple",
      uniform_words_per_qpu = 6 : i32,
      args = [
        {name = "out", kind = "buffer", direction = "out", elem_type = "f32", uniform_index = 0 : i32},
        {name = "k", kind = "scalar", direction = "by_value", type = "u32", uniform_index = 1 : i32},
        {name = "beta", kind = "scalar", direction = "by_value", type = "f32", uniform_index = 2 : i32}
      ],
      builtins = [
    ...
    %k = ssavc4.uniform.read 1 : i32
    %beta = ssavc4.uniform.read 2 : f32
  ...
  ^step(%iter_step: i32, %acc_step: vector<16xf32>):
    %acc_next = ssavc4.alu.add %acc_step, %p01 {opcode = #vc4.add_opcode<fadd>} : (vector<16xf32>, vector<16xf32>) -> vector<16xf32>
