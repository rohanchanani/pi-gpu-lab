ssavc4.module @natural_loop_scalar_runtime_bound_ssavc4 {
  ssavc4.func @natural_loop_scalar_runtime_bound_ssavc4_kernel() attributes {
    kernel,
    threading = #vc4.threading_mode<single>,
    "vc4.launch_abi" = {
      public_name = "natural_loop_scalar_runtime_bound_ssavc4",
      code_symbol = "natural_loop_scalar_runtime_bound_ssavc4_shader",
  ...
  ^preheader:
    ...
    %case_seed = ssavc4.alu.add %case_id, %shift_eight {opcode = #vc4.add_opcode<shl>} : (i32, i32) -> i32
    %case_seed_vec = ssavc4.splat %case_seed : i32 -> vector<16xi32>
    %acc0 = ssavc4.alu.add %index, %case_seed_vec {opcode = #vc4.add_opcode<add>} : (vector<16xi32>, vector<16xi32>) -> vector<16xi32>
    %case_step = ssavc4.alu.add %case_id, %shift_two {opcode = #vc4.add_opcode<shl>} : (i32, i32) -> i32
    %case_step_vec = ssavc4.splat %case_step : i32 -> vector<16xi32>
