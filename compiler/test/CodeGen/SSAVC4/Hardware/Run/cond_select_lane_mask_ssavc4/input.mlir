ssavc4.module @cond_select_lane_mask_ssavc4 {
  ssavc4.func @cond_select_lane_mask_ssavc4_kernel() attributes {
    kernel,
    threading = #vc4.threading_mode<single>,
    "vc4.launch_abi" = {
      public_name = "cond_select_lane_mask_ssavc4",
      code_symbol = "cond_select_lane_mask_ssavc4_shader",
      tail_policy = "exact_multiple",
      uniform_words_per_qpu = 4 : i32,
      args = [
        {name = "out", kind = "buffer", direction = "out", elem_type = "u32", uniform_index = 0 : i32},
        {name = "threshold", kind = "scalar", direction = "by_value", type = "u32", uniform_index = 1 : i32}
      ],
      builtins = [
    ...
    %threshold = ssavc4.uniform.read 1 : i32
    ...
    %true_base = ssavc4.load_imm <splat32> {value = 285212672 : i32} : vector<16xi32>
    %false_base = ssavc4.load_imm <splat32> {value = 570425344 : i32} : vector<16xi32>
    ...
    %threshold_vec = ssavc4.splat %threshold : i32 -> vector<16xi32>
    %true_value = ssavc4.alu.add %true_base, %index {opcode = #vc4.add_opcode<add>} : (vector<16xi32>, vector<16xi32>) -> vector<16xi32>
    ...
    %false_value = ssavc4.alu.add %false_base, %triple_index {opcode = #vc4.add_opcode<add>} : (vector<16xi32>, vector<16xi32>) -> vector<16xi32>
    %flags = ssavc4.make_flags %lane, %threshold_vec {kind = #ssavc4.flag_kind<sub>} : (vector<16xi32>, vector<16xi32>) -> !ssavc4.flags
    %selected = ssavc4.cond_select %flags, %true_value, %false_value {cond = #vc4.cond<cs>} : !ssavc4.flags, vector<16xi32>, vector<16xi32> -> vector<16xi32>
