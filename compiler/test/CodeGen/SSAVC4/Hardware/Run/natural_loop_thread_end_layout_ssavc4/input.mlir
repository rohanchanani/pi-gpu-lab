ssavc4.module @natural_loop_thread_end_layout_ssavc4 {
  ssavc4.func @natural_loop_thread_end_layout_ssavc4_kernel() attributes {
    kernel,
    threading = #vc4.threading_mode<single>,
    "vc4.launch_abi" = {
      public_name = "natural_loop_thread_end_layout_ssavc4",
      code_symbol = "natural_loop_thread_end_layout_ssavc4_shader",
      tail_policy = "tail_safe",
      uniform_words_per_qpu = 5 : i32,
      args = [
        {name = "out", kind = "buffer", direction = "out", elem_type = "u32", uniform_index = 0 : i32},
        {name = "iterations", kind = "scalar", direction = "by_value", type = "u32", uniform_index = 1 : i32},
    ...
    %iterations = ssavc4.uniform.read 1 : i32
  ...
  ^loop(%iter: i32, %acc: vector<16xi32>):
    ...
    ssavc4.cond_br %done_flags, ^done(%acc : vector<16xi32>), ^body(%iter, %acc : i32, vector<16xi32>) {cond = #vc4.branch_cond<any_c_clear>} : !ssavc4.flags

  ^done(%final: vector<16xi32>):
  ...
  ^body(%iter_step: i32, %acc_step: vector<16xi32>):
