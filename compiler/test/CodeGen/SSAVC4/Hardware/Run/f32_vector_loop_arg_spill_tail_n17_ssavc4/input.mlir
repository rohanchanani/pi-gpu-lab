ssavc4.module @f32_vector_loop_arg_spill_tail_n17_ssavc4 {
  ssavc4.func @f32_vector_loop_arg_spill_tail_n17_ssavc4_kernel() attributes {
    kernel,
    threading = #vc4.threading_mode<single>,
    "vc4.launch_abi" = {
      public_name = "f32_vector_loop_arg_spill_tail_n17_ssavc4",
      code_symbol = "f32_vector_loop_arg_spill_tail_n17_ssavc4_shader",
    ...
    %alpha = ssavc4.uniform.read 2 : f32
    %beta = ssavc4.uniform.read 3 : f32
    %n = ssavc4.uniform.read 4 : i32
  ...
  ^body:
    ...
    ssavc4.br ^loop(%zero, %x, %beta_v : i32, vector<16xf32>, vector<16xf32>)
  ...
  ^after_loop(%final_acc: vector<16xf32>, %final_carry: vector<16xf32>):
    ...
    ssavc4.cond_br %full_flags, ^full(%s20 : vector<16xf32>), ^tail(%s20 : vector<16xf32>) {cond = #vc4.branch_cond<any_c_clear>} : !ssavc4.flags

  ^tail(%tail_value: vector<16xf32>):
    %active = ssavc4.alu.add %remaining, %shift_two {opcode = #vc4.add_opcode<shr>} : (i32, i32) -> i32
    ssavc4.vdw.store %out_chunk, %tail_value, %active, %qpu_id {width = #ssavc4.vpm_elem_width<w32>, subword = #ssavc4.vpm_subword<none>, vpm_row = 0 : i32, serialize = "mutex"} : i32, vector<16xf32>, i32, i32
    ssavc4.br ^done

  ^full(%full_value: vector<16xf32>):
    ssavc4.vdw.store %out_chunk, %full_value, %active_full, %qpu_id {width = #ssavc4.vpm_elem_width<w32>, subword = #ssavc4.vpm_subword<none>, vpm_row = 0 : i32, serialize = "mutex"} : i32, vector<16xf32>, i32, i32
    ssavc4.br ^done
