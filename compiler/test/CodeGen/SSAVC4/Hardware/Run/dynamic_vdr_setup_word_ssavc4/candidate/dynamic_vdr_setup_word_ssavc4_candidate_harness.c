...
#define SENTINEL 0x87cafe31u

static uint32_t input_values[(ROWS - 1u) * MAX_PITCH_WORDS + COLS + GUARD_WORDS];
static uint32_t output_values[OUT_WORDS + GUARD_WORDS];
static const uint32_t active_rows_cases[] = {0u, 1u, 7u, 15u, 16u, 17u};
static const uint32_t active_cols_cases[] = {0u, 1u, 7u, 15u, 16u, 17u};
static const uint32_t pitch_bytes_cases[] = {64u, 68u, 76u, 124u, 260u};

static uint32_t pattern(uint32_t row, uint32_t col) {
  return 0x87000000u | (row << 8) | col;
}
...
void notmain(void) {
  ...
  if (vc4_program_create(&program, 0) < 0 || !program)
    panic("dynamic_vdr_setup_word_ssavc4 program create failed");
  if (vc4Malloc(program, &input_dev, sizeof(input_values)) < 0 ||
      vc4Malloc(program, &output_dev, sizeof(output_values)) < 0)
    panic("dynamic_vdr_setup_word_ssavc4 allocation failed");
  ...
  for (uint32_t p = 0;
    ...
    for (uint32_t r = 0;
      ...
      for (uint32_t c = 0;
        ...
        for (uint32_t i = 0; i < sizeof(input_values) / sizeof(input_values[0]);
             ++i)
        ...
        if (vc4MemcpyHtoD(program, input_dev, input_values,
            ...
            dynamic_vdr_setup_word_ssavc4_launch(
            ...
            vc4MemcpyDtoH(program, output_values, output_dev,
                          sizeof(output_values)) < 0) {
          printk("ERROR: dynamic_vdr_setup_word launch/copy failed pitch_bytes=%d active_rows=%d active_cols=%d\n",
            ...
            if (actual != expected) {
              if (expected != 0u) {
                if (total_mismatches < 8)
                  printk("ERROR: dynamic_vdr_setup_word pitch_bytes=%d active_rows=%d active_cols=%d row=%d col=%d actual=%x expected=%x\n",
                ...
                if (zero_mismatches < 8)
                  printk("ERROR: dynamic_vdr_setup_word pitch_bytes=%d active_rows=%d active_cols=%d zero_row=%d zero_col=%d actual=%x expected=0\n",
                         (int)pitch_bytes, (int)active_rows,
                         (int)active_cols, (int)row, (int)col, actual);
                ++zero_mismatches;
              }
        ...
        for (uint32_t i = 0; i < GUARD_WORDS; ++i) {
          ...
          if (actual != SENTINEL) {
            if (sentinel_mismatches < 8)
              printk("ERROR: dynamic_vdr_setup_word pitch_bytes=%d active_rows=%d active_cols=%d guard=%d actual=%x expected=%x\n",
                     (int)pitch_bytes, (int)active_rows, (int)active_cols,
  ...
  uint32_t runtime_launches = dynamic_vdr_setup_word_ssavc4_runtime_launches();
  uint32_t runtime_launch_failures =
      dynamic_vdr_setup_word_ssavc4_runtime_launch_failures();
  ...
  const char *status =
      (total_mismatches == 0 && zero_mismatches == 0 &&
       sentinel_mismatches == 0 && launch_failures == 0)
          ? "PASS"
          : "FAIL";
  printk("VC4_TEST_RESULT name=dynamic_vdr_setup_word_ssavc4 status=%s total_mismatches=%d zero_mismatches=%d sentinel_mismatches=%d launch_failures=%d runtime_launches=%d checksum_accum=%u\n",
