ssavc4.module @user_tmu_after_spill_reload_loop_ssavc4 {
  ssavc4.func @user_tmu_after_spill_reload_loop_ssavc4_kernel() attributes {
    kernel,
    threading = #vc4.threading_mode<single>,
    "vc4.launch_abi" = {
      public_name = "user_tmu_after_spill_reload_loop_ssavc4",
      code_symbol = "user_tmu_after_spill_reload_loop_ssavc4_shader",
      ...
      args = [
        ...
        {name = "k", kind = "scalar", direction = "by_value", type = "u32", uniform_index = 2 : i32},
  ...
  ^step(%iter_step: i32, %acc_step: vector<16xf32>):
    %with_reload = ssavc4.alu.add %acc_step, %p01 {opcode = #vc4.add_opcode<fadd>} : (vector<16xf32>, vector<16xf32>) -> vector<16xf32>
    %row_offset = ssavc4.alu.add %iter_step, %shift_six {opcode = #vc4.add_opcode<shl>} : (i32, i32) -> i32
    %row_offset_v = ssavc4.splat %row_offset : i32 -> vector<16xi32>
    %x_base_v = ssavc4.splat %x_base : i32 -> vector<16xi32>
    %x_offset = ssavc4.alu.add %row_offset_v, %lane_bytes {opcode = #vc4.add_opcode<add>} : (vector<16xi32>, vector<16xi32>) -> vector<16xi32>
    %x_addr = ssavc4.alu.add %x_base_v, %x_offset {opcode = #vc4.add_opcode<add>} : (vector<16xi32>, vector<16xi32>) -> vector<16xi32>
    %x_tok = ssavc4.tmu.request %x_addr {unit = "tmu0", mode = "direct"} : vector<16xi32> -> !ssavc4.async.token
    %x_val = ssavc4.tmu.read %x_tok {unit = "tmu0", part = "raw32"} : !ssavc4.async.token -> vector<16xf32>
    %acc_next = ssavc4.alu.add %with_reload, %x_val {opcode = #vc4.add_opcode<fadd>} : (vector<16xf32>, vector<16xf32>) -> vector<16xf32>
  ...
  ^after(%acc_final: vector<16xf32>):
    %s02 = ssavc4.alu.add %acc_final, %p02 {opcode = #vc4.add_opcode<fadd>} : (vector<16xf32>, vector<16xf32>) -> vector<16xf32>
    ...
    ssavc4.vdw.store %out_base, %s24, %active_full, %qpu_id {width = #ssavc4.vpm_elem_width<w32>, subword = #ssavc4.vpm_subword<none>, vpm_row = 0 : i32, serialize = "mutex"} : i32, vector<16xf32>, i32, i32
    ssavc4.thread_end
  }
}
