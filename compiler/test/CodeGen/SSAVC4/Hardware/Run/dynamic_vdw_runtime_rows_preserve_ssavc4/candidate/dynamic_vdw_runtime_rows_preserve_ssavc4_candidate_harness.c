...
void notmain(void) {
  ...
  if (vc4_program_create(&program, 0) < 0 || !program)
    panic("dynamic_vdw_runtime_rows_preserve_ssavc4 program create failed");
  if (vc4Malloc(program, &input_dev, sizeof(input_values)) < 0 ||
      vc4Malloc(program, &output_dev, sizeof(output_values)) < 0)
    panic("dynamic_vdw_runtime_rows_preserve_ssavc4 allocation failed");
  ...
  for (uint32_t c = 0;
    ...
    uint32_t expected_active = active_rows == 0 ? 0 : COLS;
    ...
    if (vc4MemcpyHtoD(program, input_dev, input_values, sizeof(input_values)) < 0 ||
        ...
        dynamic_vdw_runtime_rows_preserve_ssavc4_launch(program, grid, block,
                                                        input_dev, output_dev,
                                                        active_rows) < 0 ||
        vc4MemcpyDtoH(program, output_values, output_dev, sizeof(output_values)) < 0) {
      printk("ERROR: dynamic_vdw_runtime_rows_preserve launch/copy failed active_rows=%d\n",
             (int)active_rows);
      ++launch_failures;
    }
    ...
    for (uint32_t i = 0; i < expected_active; ++i) {
      ...
      if (actual != expected) {
        if (total_mismatches < 8)
          printk("ERROR: dynamic_vdw_runtime active_rows=%d active_col=%d actual=%x expected=%x\n",
    ...
    for (uint32_t i = expected_active; i < COLS + GUARD_WORDS; ++i) {
      ...
      if (actual != SENTINEL) {
        if (sentinel_mismatches < 8)
          printk("ERROR: dynamic_vdw_runtime active_rows=%d preserved_col=%d actual=%x expected=%x\n",
  ...
  uint32_t runtime_launches =
      dynamic_vdw_runtime_rows_preserve_ssavc4_runtime_launches();
  uint32_t runtime_launch_failures =
      dynamic_vdw_runtime_rows_preserve_ssavc4_runtime_launch_failures();
  ...
  const char *status =
      (total_mismatches == 0 && sentinel_mismatches == 0 &&
       launch_failures == 0)
          ? "PASS"
          : "FAIL";
  printk("VC4_TEST_RESULT name=dynamic_vdw_runtime_rows_preserve_ssavc4 status=%s total_mismatches=%d sentinel_mismatches=%d launch_failures=%d runtime_launches=%d checksum_accum=%u\n",
