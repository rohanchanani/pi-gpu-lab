...
void notmain(void) {
  ...
  if (vc4_program_create(&program, 0) < 0 || !program)
    panic("dynamic_vdw_runtime_stride_rows_cols_preserve_ssavc4 program create failed");
  if (vc4Malloc(program, &input_dev, sizeof(input_values)) < 0 ||
      vc4Malloc(program, &output_dev, sizeof(output_values)) < 0)
    panic("dynamic_vdw_runtime_stride_rows_cols_preserve_ssavc4 allocation failed");
    ...
    for (uint32_t r = 0;
      ...
      for (uint32_t c = 0;
        ...
        if (vc4MemcpyHtoD(program, input_dev, input_values,
            ...
            dynamic_vdw_runtime_stride_rows_cols_preserve_ssavc4_launch(
            ...
            vc4MemcpyDtoH(program, output_values, output_dev,
                          sizeof(output_values)) < 0) {
          printk("ERROR: dynamic_vdw_runtime_stride_rows_cols launch/copy failed stride_bytes=%d active_rows=%d active_cols=%d\n",
          ...
          if (actual != expected) {
            if (expected != SENTINEL) {
              if (total_mismatches < 8)
                printk("ERROR: dynamic_vdw_runtime_stride_rows_cols stride_bytes=%d active_rows=%d active_cols=%d index=%d actual=%x expected=%x\n",
                       (int)stride_bytes, (int)active_rows,
                       (int)active_cols, (int)i, actual, expected);
              ++total_mismatches;
            } else {
              if (sentinel_mismatches < 8)
                printk("ERROR: dynamic_vdw_runtime_stride_rows_cols stride_bytes=%d active_rows=%d active_cols=%d sentinel_index=%d actual=%x expected=%x\n",
                       (int)stride_bytes, (int)active_rows,
  ...
  uint32_t runtime_launches =
      dynamic_vdw_runtime_stride_rows_cols_preserve_ssavc4_runtime_launches();
  uint32_t runtime_launch_failures =
      dynamic_vdw_runtime_stride_rows_cols_preserve_ssavc4_runtime_launch_failures();
  ...
  const char *status =
      (total_mismatches == 0 && sentinel_mismatches == 0 &&
       launch_failures == 0)
          ? "PASS"
          : "FAIL";
  printk("VC4_TEST_RESULT name=dynamic_vdw_runtime_stride_rows_cols_preserve_ssavc4 status=%s total_mismatches=%d sentinel_mismatches=%d launch_failures=%d runtime_launches=%d checksum_accum=%u\n",
