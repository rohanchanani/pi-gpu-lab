...
static const uint32_t active_rows_cases[] = {0u, 1u, 2u};
...
void notmain(void) {
  ...
  if (vc4_program_create(&program, 0) < 0 || !program)
    panic("dynamic_vdr_runtime_rows_zero_fill_ssavc4 program create failed");
  if (vc4Malloc(program, &input_dev, sizeof(input_values)) < 0 ||
      vc4Malloc(program, &output_dev, sizeof(output_values)) < 0)
    panic("dynamic_vdr_runtime_rows_zero_fill_ssavc4 allocation failed");
  ...
  for (uint32_t c = 0;
    ...
    uint32_t expected_active = active_rows == 0u ? 0u : COLS;
    ...
    if (vc4MemcpyHtoD(program, input_dev, input_values, sizeof(input_values)) < 0 ||
        ...
        dynamic_vdr_runtime_rows_zero_fill_ssavc4_launch(program, grid, block,
                                                         input_dev, output_dev,
                                                         active_rows) < 0 ||
        vc4MemcpyDtoH(program, output_values, output_dev, sizeof(output_values)) < 0) {
      printk("ERROR: dynamic_vdr_runtime_rows_zero_fill launch/copy failed active_rows=%d\n",
             (int)active_rows);
      ++launch_failures;
    }
    ...
    for (uint32_t i = 0; i < expected_active; ++i) {
      ...
      if (actual != expected) {
        if (total_mismatches < 8)
          printk("ERROR: dynamic_vdr_runtime active_rows=%d active_col=%d actual=%x expected=%x\n",
                 (int)active_rows, (int)i, actual, expected);
        ++total_mismatches;
      }
    }
    for (uint32_t i = expected_active; i < COLS; ++i) {
      uint32_t actual = output_values[i];
      checksum += actual;
      if (actual != 0u) {
        if (zero_mismatches < 8)
          printk("ERROR: dynamic_vdr_runtime active_rows=%d inactive_col=%d actual=%x expected=0\n",
                 (int)active_rows, (int)i, actual);
        ++zero_mismatches;
      }
    }
    for (uint32_t i = 0; i < GUARD_WORDS; ++i) {
      uint32_t actual = output_values[COLS + i];
      if (actual != SENTINEL) {
        if (sentinel_mismatches < 8)
          printk("ERROR: dynamic_vdr_runtime active_rows=%d guard=%d actual=%x expected=%x\n",
                 (int)active_rows, (int)i, actual, SENTINEL);
        ++sentinel_mismatches;
      }
    }
  }
  ...
  uint32_t runtime_launches =
      dynamic_vdr_runtime_rows_zero_fill_ssavc4_runtime_launches();
  uint32_t runtime_launch_failures =
      dynamic_vdr_runtime_rows_zero_fill_ssavc4_runtime_launch_failures();
  ...
  const char *status =
      (total_mismatches == 0 && zero_mismatches == 0 &&
       sentinel_mismatches == 0 && launch_failures == 0)
          ? "PASS"
          : "FAIL";
  printk("VC4_TEST_RESULT name=dynamic_vdr_runtime_rows_zero_fill_ssavc4 status=%s total_mismatches=%d zero_mismatches=%d sentinel_mismatches=%d launch_failures=%d runtime_launches=%d checksum_accum=%u\n",
