...
void notmain(void) {
  ...
  if (vc4_program_create(&program, 0) < 0 || !program)
    panic("vdr_vdw_horizontal_32_roundtrip_ssavc4 program create failed");
  if (vc4Malloc(program, &input_dev, input_bytes) < 0 ||
      vc4Malloc(program, &output_dev, output_bytes) < 0)
    panic("vdr_vdw_horizontal_32_roundtrip_ssavc4 device allocation failed");
  ...
  printk("Running VC4 vdr_vdw_horizontal_32_roundtrip_ssavc4 candidate bundle.\n");
  if (vc4MemcpyHtoD(program, input_dev, input_values, input_bytes) < 0 ||
      ...
      vdr_vdw_horizontal_32_roundtrip_ssavc4_launch(program, grid, block, input_dev,
                                                output_dev) < 0 ||
      vc4MemcpyDtoH(program, output_values, output_dev, output_bytes) < 0) {
    printk("ERROR: vdr_vdw_horizontal_32_roundtrip_ssavc4 launch/copy failed\n");
    ++launch_failures;
  } else {
  ...
  uint32_t runtime_launches = vdr_vdw_horizontal_32_roundtrip_ssavc4_runtime_launches();
  uint32_t runtime_launch_failures =
      vdr_vdw_horizontal_32_roundtrip_ssavc4_runtime_launch_failures();
  ...
  const char *status =
      (launch_failures == 0 && guard_mismatches == 0 &&
       model_mismatches == 0 && sentinel_mismatches == 0)
          ? "PASS"
          : "FAIL";
  printk("VC4_TEST_RESULT name=vdr_vdw_horizontal_32_roundtrip_ssavc4 status=%s cases=%d launch_failures=%d guard_mismatches=%d model_mismatches=%d sentinel_mismatches=%d runtime_launches=%d\n",
