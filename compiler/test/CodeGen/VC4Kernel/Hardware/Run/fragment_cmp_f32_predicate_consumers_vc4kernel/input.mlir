module {
  vc4kernel.kernel @fragment_cmp_f32_predicate_consumers_vc4kernel(%lhs : i32, %rhs : i32, %out : i32) attributes {
    ...
    %lhs_v = vc4kernel.tmu_load_fragment %lhs, %lane_bytes, %full, %p7_safe0 {inactive_load = #vc4kernel.inactive_load<zero>, memory_path = #vc4kernel.memory_path<tmu_global_read>, coherency = #vc4kernel.coherency<readonly_tmu>} : i32, vector<16xi32>, !vc4kernel.pred<16>, i32 -> vector<16xf32>
    %p7_safe1 = arith.constant 0 : i32
    %rhs_v = vc4kernel.tmu_load_fragment %rhs, %lane_bytes, %full, %p7_safe1 {inactive_load = #vc4kernel.inactive_load<zero>, memory_path = #vc4kernel.memory_path<tmu_global_read>, coherency = #vc4kernel.coherency<readonly_tmu>} : i32, vector<16xi32>, !vc4kernel.pred<16>, i32 -> vector<16xf32>
  ...
  ^any_merge(%any_value : vector<16xi32>):
    ...
    vc4kernel.vdw_store_fragment %out, %offs4, %any_value, %full {memory_path = #vc4kernel.memory_path<vdw_global_store>, coherency = #vc4kernel.coherency<dma_ordered>, inactive_store = #vc4kernel.inactive_store<preserve>} : i32, vector<16xi32>, vector<16xi32>, !vc4kernel.pred<16>
  ...
  ^all_merge(%all_value : vector<16xi32>):
    ...
    vc4kernel.vdw_store_fragment %out, %offs5, %all_value, %full {memory_path = #vc4kernel.memory_path<vdw_global_store>, coherency = #vc4kernel.coherency<dma_ordered>, inactive_store = #vc4kernel.inactive_store<preserve>} : i32, vector<16xi32>, vector<16xi32>, !vc4kernel.pred<16>
    vc4kernel.return
  }
}
