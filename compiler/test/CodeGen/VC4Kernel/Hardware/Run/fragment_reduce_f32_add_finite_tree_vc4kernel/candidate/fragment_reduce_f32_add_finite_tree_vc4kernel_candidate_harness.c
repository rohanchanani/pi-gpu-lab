...
#define ACTIVE_N (LANES * PREDICATES)
...
static const float input_values[LANES] = {
    ...
    -1.5f, 2.25f, -3.0f, 4.75f, 0.0f, 5.5f, -6.25f, 7.0f
};
...
static int active(uint32_t predicate, uint32_t lane) {
    ...
    if (predicate == 3u)
        return lane < 9u;
    return lane == 0u || lane == 3u || lane == 5u || lane == 11u ||
           lane == 15u;
}

static float expected_tree_add(uint32_t predicate) {
    float work[LANES];
    float next[LANES];
    const uint32_t amounts[4] = {8u, 4u, 2u, 1u};
    for (uint32_t lane = 0; lane < LANES; lane++)
        work[lane] = active(predicate, lane) ? input_values[lane] : 0.0f;
    for (uint32_t ai = 0; ai < 4u; ai++) {
        uint32_t amount = amounts[ai];
        for (uint32_t lane = 0; lane < LANES; lane++)
            next[lane] = work[lane] + work[(lane + amount) & 15u];
        for (uint32_t lane = 0; lane < LANES; lane++)
            work[lane] = next[lane];
    }
    return work[0];
...
static int verify_active(float *max_abs_diff) {
    ...
    *max_abs_diff = 0.0f;
    for (uint32_t predicate = 0; predicate < PREDICATES; predicate++) {
        float expected = expected_tree_add(predicate);
        for (uint32_t lane = 0; lane < LANES; lane++) {
            uint32_t index = predicate * LANES + lane;
            ...
            if (ad != 0.0f) {
                if (mismatches < 8)
                    printk("ERROR: reduce_f32_add pred=%d lane=%d gpu=%f expected=%f diff=%f\n",
                           (int)predicate, (int)lane, out_values[index],
                           expected, diff);
                mismatches++;
            }
        }
...
static int verify_sentinels(void) {
    ...
    for (uint32_t i = ACTIVE_N; i < BUFFER_N; i++) {
        if (out_values[i] != SENTINEL) {
            if (mismatches < 8)
                printk("ERROR: reduce_f32_add sentinel i=%d gpu=%f expected=%f\n",
...
void notmain(void) {
    ...
    if (vc4_m2_malloc(program, &input_dev, LANES * sizeof(float)) < 0 ||
        ...
        panic("fragment_reduce_f32_add allocation failed");
    ...
    if (vc4_m2_copy_htod(program, input_dev, input_values,
        ...
        fragment_reduce_f32_add_finite_tree_vc4kernel_launch(
    ...
    printk("VC4_TEST_RESULT name=fragment_reduce_f32_add_finite_tree_vc4kernel status=%s cases=1 total_mismatches=%d sentinel_mismatches=%d launch_failures=%d active_qpus=1 lanes=%d kinds=1 predicates=%d finite_tree=1 checksum_accum=%d max_abs_diff=%f runtime_allocations=2 runtime_launches=1 elapsed_usec=%d\n",
           status, total_mismatches, sentinel_mismatches, launch_failures,
           (int)LANES, (int)PREDICATES, checksum_accum, max_abs_diff, elapsed);
