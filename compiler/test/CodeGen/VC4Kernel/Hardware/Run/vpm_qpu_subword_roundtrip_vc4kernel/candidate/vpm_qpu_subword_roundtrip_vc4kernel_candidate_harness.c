  ...
  for (uint32_t segment = 0; segment < SEGMENTS; ++segment) {
    for (uint32_t lane = 0; lane < LANES; ++lane) {
      ...
      *checksum += got;
      if (got != expected) {
        if (mismatches < 8)
          printk("ERROR: vpm_qpu_subword_roundtrip seg=%d lane=%d got=%x expected=%x\n",
...
static int verify_sentinels(void) {
  ...
  for (uint32_t i = LANES * SEGMENTS; i < OUT_WORDS; ++i) {
    if (out_words[i] != SENTINEL) {
      if (mismatches < 8)
        printk("ERROR: vpm_qpu_subword_roundtrip sentinel i=%d got=%x\n",
               (int)i, out_words[i]);
      ++mismatches;
    }
  }
...
void notmain(void) {
  ...
  if (vc4_program_create(&program, 0) < 0 || !program)
    panic("vpm_qpu_subword_roundtrip program create failed");

  vc4_deviceptr_t out_dev = 0;
  uint32_t bytes = OUT_WORDS * sizeof(uint32_t);
  if (vc4_m2_malloc(program, &out_dev, bytes) < 0)
    panic("vpm_qpu_subword_roundtrip allocation failed");
  ...
  if (vc4_m2_copy_htod(program, out_dev, out_words, bytes) < 0 ||
      vpm_qpu_subword_roundtrip_vc4kernel_launch(program, grid, block,
                                                 out_dev) < 0 ||
  ...
  printk("VC4_TEST_RESULT name=vpm_qpu_subword_roundtrip_vc4kernel status=%s cases=1 total_mismatches=%d sentinel_mismatches=%d launch_failures=%d active_qpus=1 lanes=%d saw_vpm_qpu_w8=1 saw_vpm_qpu_w16=1 saw_packed=1 saw_laned=1 saw_horizontal=1 saw_vertical=1 saw_pack_unpack=1 saw_vdw_preserve=1 checksum_accum=%u elapsed_usec=%d\n",
