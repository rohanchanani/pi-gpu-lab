module {
  vc4kernel.kernel @mixed_surface_lock_elementwise_full_vc4kernel(%input : i32, %flags : i32, %out : i32, %audit : i32, %n : i32, %amount_seed : i32, %scale : f32) attributes {
    public_name = "mixed_surface_lock_elementwise_full_vc4kernel",
    ...
    %sel8 = arith.andi %amount, %c0 : i32
    vc4kernel.vdr_load_to_vpm %flags, %base_index, %tile, %c0 dynamic_subword_selector %sel8 {rows = 1 : i32, cols = 16 : i32, global_stride_bytes = 16 : i32, elem_bytes = 1 : i32, orientation = #vc4kernel.vpm_orientation<horizontal>, width = #vc4kernel.vpm_width<w8>, subword = #vc4kernel.vpm_subword<packed>, dst_x = 0 : i32, vpm_pitch = 1 : i32, memory_path = #vc4kernel.memory_path<vdr_global_to_vpm>, coherency = #vc4kernel.coherency<dma_ordered>} : i32, i32, !vc4kernel.vpm_tile, i32 dynamic_subword_selector i32
    %raw_flags = vc4kernel.vpm_read_fragment %tile, %c0 dynamic_subword_selector %sel8, %full {orientation = #vc4kernel.vpm_orientation<horizontal>, width = #vc4kernel.vpm_width<w8>, subword = #vc4kernel.vpm_subword<packed>, stride = 1 : i32, memory_path = #vc4kernel.memory_path<vpm_qpu>, coherency = #vc4kernel.coherency<vpm_local>} : !vc4kernel.vpm_tile, i32 dynamic_subword_selector i32, !vc4kernel.pred<16> -> vector<16xi32>
