module {
  vc4kernel.kernel @mixed_sfu_activation_tmu_vdw_vc4kernel(%input : i32, %flags : i32, %out : i32, %audit : i32, %n : i32, %scale : f32, %bias : f32) attributes {
    public_name = "mixed_sfu_activation_tmu_vdw_vc4kernel",
    ...
    %raw = vc4kernel.tmu_load_fragment %input, %request_offsets, %tail, %safe0 {inactive_load = #vc4kernel.inactive_load<zero>, memory_path = #vc4kernel.memory_path<tmu_global_read>, coherency = #vc4kernel.coherency<readonly_tmu>} : i32, vector<16xi32>, !vc4kernel.pred<16>, i32 -> vector<16xf32>
    ...
    %positive = vc4kernel.fragment_cmp %raw, %zero {predicate = #vc4kernel.cmp<ogt>, fp_policy = #vc4kernel.fp_cmp_policy<finite_only>} : vector<16xf32>, vector<16xf32> -> !vc4kernel.pred<16>
    %guarded = vc4kernel.fragment_select %positive, %raw, %one : !vc4kernel.pred<16>, vector<16xf32>, vector<16xf32> -> vector<16xf32>
    %recip = vc4kernel.fragment_sfu %guarded {kind = #vc4kernel.sfu_kind<recip>, fp_policy = #vc4kernel.fp_math_policy<approx_sfu>, domain = #vc4kernel.fp_domain<finite_nonzero>} : vector<16xf32> -> vector<16xf32>
    %rsqrt = vc4kernel.fragment_sfu %guarded {kind = #vc4kernel.sfu_kind<rsqrt>, fp_policy = #vc4kernel.fp_math_policy<approx_sfu>, domain = #vc4kernel.fp_domain<finite_positive>} : vector<16xf32> -> vector<16xf32>
    %scale_v = vc4kernel.splat %scale : f32 -> vector<16xf32>
    %bias_v = vc4kernel.splat %bias : f32 -> vector<16xf32>
    %scaled_rsqrt = vc4kernel.fragment_alu.mul %rsqrt, %scale_v {opcode = #vc4kernel.mul_alu_opcode<fmul>} : (vector<16xf32>, vector<16xf32>) -> vector<16xf32>
    %sum = vc4kernel.fragment_alu.add %recip, %scaled_rsqrt {opcode = #vc4kernel.add_alu_opcode<fadd>} : (vector<16xf32>, vector<16xf32>) -> vector<16xf32>
    %biased = vc4kernel.fragment_alu.add %sum, %bias_v {opcode = #vc4kernel.add_alu_opcode<fadd>} : (vector<16xf32>, vector<16xf32>) -> vector<16xf32>
    %active_value = vc4kernel.fragment_select %tail, %biased, %zero : !vc4kernel.pred<16>, vector<16xf32>, vector<16xf32> -> vector<16xf32>
    ...
    %idx = vc4kernel.fragment_alu.add %base_index_v, %lanes {opcode = #vc4kernel.add_alu_opcode<add>} : (vector<16xi32>, vector<16xi32>) -> vector<16xi32>
    %twice_flags = vc4kernel.fragment_alu.add %flags_u8, %flags_u8 {opcode = #vc4kernel.add_alu_opcode<add>} : (vector<16xi32>, vector<16xi32>) -> vector<16xi32>
    %audit_v = vc4kernel.fragment_alu.add %twice_flags, %idx {opcode = #vc4kernel.add_alu_opcode<xor>} : (vector<16xi32>, vector<16xi32>) -> vector<16xi32>
