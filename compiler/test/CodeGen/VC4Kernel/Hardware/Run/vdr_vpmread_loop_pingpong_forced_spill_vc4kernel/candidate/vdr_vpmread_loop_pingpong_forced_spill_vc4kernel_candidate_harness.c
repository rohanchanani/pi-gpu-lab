...
static int verify_case(uint32_t active_cols, uint32_t *checksum) {
  ...
  for (uint32_t c = 0; c < COLS; ++c) {
    ...
    *checksum += got;
    if (got != want) {
      if (mismatches < 8)
        printk("ERROR: pingpong_forced_spill active_cols=%d col=%d got=%x want=%x\n",
...
static int verify_guard(uint32_t active_cols) {
  ...
  for (uint32_t i = OUTPUT_WORDS; i < OUTPUT_WORDS + GUARD; ++i) {
    if (output_values[i] != SENTINEL) {
      if (mismatches < 8)
        printk("ERROR: pingpong_forced_spill active_cols=%d guard=%d got=%x want=%x\n",
...
void notmain(void) {
  ...
  if (vc4_program_create(&program, 0) < 0 || !program)
    panic("vdr_vpmread_loop_pingpong_forced_spill_vc4kernel program create failed");
  if (vc4_m2_malloc(program, &in_dev, INPUT_WORDS * sizeof(uint32_t)) < 0 ||
    ...
    panic("vdr_vpmread_loop_pingpong_forced_spill_vc4kernel allocation failed");
  ...
  for (uint32_t i = 0; i < cases; ++i) {
    ...
    if (vc4_m2_copy_htod(program, in_dev, input_values, INPUT_WORDS * sizeof(uint32_t)) < 0 ||
        ...
        vdr_vpmread_loop_pingpong_forced_spill_vc4kernel_launch(
  ...
  launch_failures +=
      (int)vdr_vpmread_loop_pingpong_forced_spill_vc4kernel_runtime_launch_failures();
  uint32_t launches =
      vdr_vpmread_loop_pingpong_forced_spill_vc4kernel_runtime_launches();
  const char *status = (launch_failures == 0 && total_mismatches == 0 &&
                        sentinel_mismatches == 0 && launches == cases)
                           ? "PASS"
                           : "FAIL";
  printk("VC4_TEST_RESULT name=vdr_vpmread_loop_pingpong_forced_spill_vc4kernel status=%s cases=%d total_mismatches=%d sentinel_mismatches=%d launch_failures=%d active_cols_cases=16,1 pingpong_rows=0-1,2-3 forced_spill_live_adjust=%d checksum_accum=%u runtime_launches=%u elapsed_usec=%d\n",
         status, (int)cases, total_mismatches, sentinel_mismatches,
         launch_failures, (int)SPILL_ADJUST, checksum, launches,
