...
static const uint32_t input_values[LANES] = {
    0u, 1u, 0x7fffffffu, 0x80000000u, 0xffffffffu, 2u,
    0xfffffffeu, 0x12345678u, 0x87654321u, 15u, 0x40000000u,
    0xc0000000u, 0x00010000u, 0xffff0000u, 3u, 0x80000001u
};
...
static uint32_t expected_value(uint32_t kind, uint32_t predicate) {
    ...
    for (uint32_t lane = 0; lane < LANES; lane++) {
        if (!active(predicate, lane))
            continue;
        uint32_t value = input_values[lane];
        if (kind == 0u)
        ...
        for (uint32_t predicate = 0; predicate < PREDICATES; predicate++) {
            ...
            for (uint32_t lane = 0; lane < LANES; lane++) {
                ...
                if (out_values[index] != expected) {
                    if (mismatches < 8)
                        printk("ERROR: reduce_unsigned_minmax kind=%d pred=%d lane=%d gpu=%x expected=%x\n",
...
static int verify_sentinels(void) {
    ...
    for (uint32_t i = ACTIVE_N; i < BUFFER_N; i++) {
        if (out_values[i] != SENTINEL) {
            if (mismatches < 8)
                printk("ERROR: reduce_unsigned_minmax sentinel i=%d gpu=%x expected=%x\n",
...
void notmain(void) {
    ...
    if (vc4_m2_malloc(program, &input_dev, input_bytes) < 0 ||
        ...
        panic("fragment_reduce_i32_unsigned_minmax allocation failed");
    ...
    if (vc4_m2_copy_htod(program, input_dev, input_values, input_bytes) < 0 ||
        ...
        fragment_reduce_i32_unsigned_minmax_vc4kernel_launch(program, grid, block,
                                                             input_dev, out_dev) < 0 ||
    ...
    printk("VC4_TEST_RESULT name=fragment_reduce_i32_unsigned_minmax_vc4kernel status=%s cases=1 total_mismatches=%d sentinel_mismatches=%d launch_failures=%d active_qpus=1 lanes=%d kinds=%d predicates=%d checksum_accum=%d runtime_allocations=2 runtime_launches=1 elapsed_usec=%d\n",
