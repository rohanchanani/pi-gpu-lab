    ...
    *checksum_out = 0;
    for (uint32_t i = 0; i < VDW_RECT_VERTICAL_OUTPUT_WORDS + VDW_RECT_VERTICAL_GUARD; i++) {
        ...
        if (got != expected) {
            if (active) {
                if (mismatches < 8)
                    printk("ERROR: mixed_vertical_rect active rows=%d cols=%d stride=%d index=%d gpu=%x expected=%x\n",
                           (int)tc->active_rows, (int)tc->active_cols,
                           (int)tc->pitch_stride_words, (int)i, got, expected);
                mismatches++;
            } else {
                if (sentinel_mismatches < 8)
                    printk("ERROR: mixed_vertical_rect sentinel rows=%d cols=%d stride=%d index=%d gpu=%x expected=%x\n",
                           (int)tc->active_rows, (int)tc->active_cols,
                           (int)tc->pitch_stride_words, (int)i, got, expected);
                sentinel_mismatches++;
            }
...
void notmain(void) {
    ...
    if (vc4_program_create(&program, 0) < 0 || !program)
        panic("mixed_vertical_rect_vpm_vdw_preserve_vc4kernel program create failed");
    if (vc4_m2_malloc(program, &in_dev, sizeof(input_values)) < 0 ||
        vc4_m2_malloc(program, &out_dev, sizeof(output_values)) < 0)
        panic("mixed_vertical_rect_vpm_vdw_preserve_vc4kernel allocation failed");
    ...
    printk("Running VC4 mixed_vertical_rect_vpm_vdw_preserve_vc4kernel candidate bundle...\n");
    for (uint32_t case_id = 0; case_id < sizeof(cases) / sizeof(cases[0]); case_id++) {
        ...
        if (vc4_m2_copy_htod(program, in_dev, input_values, sizeof(input_values)) < 0 ||
            ...
            mixed_vertical_rect_vpm_vdw_preserve_vc4kernel_launch(
            ...
            printk("ERROR: mixed_vertical_rect launch/copy failed case=%d rows=%d cols=%d\n",
        ...
        if (tc->pitch_stride_words == 19u)
            saw_stride19 = 1;
        printk("MIXED_VERTICAL_RECT_VPM_VDW_PRESERVE_CASE case=%d rows=%d cols=%d stride=%d mismatches=%d sentinel_mismatches=%d checksum=%u\n",
               (int)case_id, (int)tc->active_rows, (int)tc->active_cols,
               (int)tc->pitch_stride_words, mismatches, sentinels, checksum);
    }

    launch_failures +=
        (int)mixed_vertical_rect_vpm_vdw_preserve_vc4kernel_runtime_launch_failures();
    uint32_t launches =
        mixed_vertical_rect_vpm_vdw_preserve_vc4kernel_runtime_launches();
    ...
    const char *status = (total_mismatches == 0 && sentinel_mismatches == 0 &&
                          ...
                          saw_cols0 && saw_cols8 && saw_stride19)
                             ? "PASS"
                             : "FAIL";
    printk("VC4_TEST_RESULT name=mixed_vertical_rect_vpm_vdw_preserve_vc4kernel status=%s cases=%d total_mismatches=%d sentinel_mismatches=%d launch_failures=%d max_rows=%d max_cols=%d max_stride_words=%d saw_rows0=%d saw_rows4=%d saw_cols0=%d saw_cols8=%d saw_stride19=%d saw_vertical_vdr_vdw=%d saw_dynamic_active_rows_cols=%d saw_inactive_preserve=%d saw_nonzero_vpm_coord=%d saw_vdr_vpm_path=%d saw_vdw_preserve=%d checksum_accum=%u runtime_allocations=%d runtime_launches=%u elapsed_usec=%d\n",
           ...
           saw_cols0, saw_cols8, saw_stride19, 1, 1, 1, 1, 1, 1,
           checksum_accum, 2, launches, elapsed);
