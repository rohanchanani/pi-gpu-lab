module {
  vc4kernel.kernel @vpm_read_lane_broadcast_reduce_vc4kernel(%in : i32, %out : i32, %kk : i32) attributes {
    ...
    %empty_offsets = vc4kernel.fragment_add %row1_v, %lane_bytes : vector<16xi32>, vector<16xi32> -> vector<16xi32>
    ...
    %broadcast = vc4kernel.fragment_reduce %read, %one_hot {kind = #vc4kernel.reduce<add>} : vector<16xf32>, !vc4kernel.pred<16> -> vector<16xf32>
    %empty_broadcast = vc4kernel.fragment_reduce %read, %empty {kind = #vc4kernel.reduce<add>} : vector<16xf32>, !vc4kernel.pred<16> -> vector<16xf32>
    vc4kernel.vdw_store_fragment %out, %lane_bytes, %broadcast, %full : i32, vector<16xi32>, vector<16xf32>, !vc4kernel.pred<16>
    vc4kernel.vdw_store_fragment %out, %empty_offsets, %empty_broadcast, %full : i32, vector<16xi32>, vector<16xf32>, !vc4kernel.pred<16>
    vc4kernel.return
  }
}
