...
#define GEMM_NAIVE_VC4KERNEL_EPSILON 0.0001f
#define GEMM_NAIVE_VC4KERNEL_CHECKSUM_SCALE 1024.0f
#define GEMM_NAIVE_VC4KERNEL_MAX_M 33u
#define GEMM_NAIVE_VC4KERNEL_MAX_N 33u
#define GEMM_NAIVE_VC4KERNEL_MAX_K 33u
#define GEMM_NAIVE_VC4KERNEL_MAX_LDA 96u
#define GEMM_NAIVE_VC4KERNEL_MAX_LDB 96u
#define GEMM_NAIVE_VC4KERNEL_MAX_LDC 96u
#define GEMM_NAIVE_VC4KERNEL_LANES 16u
#define GEMM_NAIVE_VC4KERNEL_GUARD_ROWS 2u
#define GEMM_NAIVE_VC4KERNEL_A_WORDS \
  (GEMM_NAIVE_VC4KERNEL_MAX_M * GEMM_NAIVE_VC4KERNEL_MAX_LDA)
#define GEMM_NAIVE_VC4KERNEL_B_WORDS \
  (GEMM_NAIVE_VC4KERNEL_MAX_K * GEMM_NAIVE_VC4KERNEL_MAX_LDB)
#define GEMM_NAIVE_VC4KERNEL_C_ROWS \
  (GEMM_NAIVE_VC4KERNEL_MAX_M + GEMM_NAIVE_VC4KERNEL_GUARD_ROWS)
#define GEMM_NAIVE_VC4KERNEL_C_WORDS \
  (GEMM_NAIVE_VC4KERNEL_C_ROWS * GEMM_NAIVE_VC4KERNEL_MAX_LDC)
#define GEMM_NAIVE_VC4KERNEL_SENTINEL (-7654.25f)

struct gemm_naive_vc4kernel_case {
  uint32_t m;
  uint32_t n;
  uint32_t k;
  uint32_t lda;
...
static const struct gemm_naive_vc4kernel_case test_cases[] = {
    {0u, 0u, 0u, 1u, 1u, 1u},
    {0u, 17u, 5u, 12u, 24u, 24u},
    {1u, 1u, 1u, 1u, 1u, 1u},
    ...
    {1u, 16u, 17u, 24u, 24u, 24u},
    {31u, 17u, 9u, 16u, 24u, 24u},
};

static float a_values[GEMM_NAIVE_VC4KERNEL_A_WORDS];
static float b_values[GEMM_NAIVE_VC4KERNEL_B_WORDS];
static float c_values[GEMM_NAIVE_VC4KERNEL_C_WORDS];
static float expected_values[GEMM_NAIVE_VC4KERNEL_C_WORDS];

static float absf_local(float value) { return value < 0.0f ? -value : value; }

static uint32_t rounded_col_blocks(uint32_t n) {
  uint32_t blocks = (n + GEMM_NAIVE_VC4KERNEL_LANES - 1u) /
                    GEMM_NAIVE_VC4KERNEL_LANES;
...
static void fill_inputs(uint32_t m, uint32_t n, uint32_t k, uint32_t lda,
  ...
  for (uint32_t i = 0; i < GEMM_NAIVE_VC4KERNEL_A_WORDS; i++)
    a_values[i] = 77.0f;
  for (uint32_t i = 0; i < GEMM_NAIVE_VC4KERNEL_B_WORDS; i++)
    b_values[i] = -55.0f;
  for (uint32_t i = 0; i < GEMM_NAIVE_VC4KERNEL_C_WORDS; i++) {
    c_values[i] = GEMM_NAIVE_VC4KERNEL_SENTINEL;
    expected_values[i] = GEMM_NAIVE_VC4KERNEL_SENTINEL;
  }
...
static int scaled_checksum(uint32_t m, uint32_t n, uint32_t ldc,
  ...
  for (uint32_t row = 0; row < m; row++)
    for (uint32_t col = 0; col < n; col++)
      checksum +=
          (int)(values[row * ldc + col] * GEMM_NAIVE_VC4KERNEL_CHECKSUM_SCALE);
  return checksum;
}
  ...
  *max_abs_diff = 0.0f;
  for (uint32_t row = 0; row < m; row++) {
    for (uint32_t col = 0; col < n; col++) {
      ...
      if (ad > *max_abs_diff)
        *max_abs_diff = ad;
      if (ad > GEMM_NAIVE_VC4KERNEL_EPSILON) {
        if (*mismatch_count < 8)
          printk("ERROR: gemm_naive_vc4kernel row=%d col=%d gpu=%f cpu=%f diff=%f\n",
...
static int verify_sentinels(uint32_t m, uint32_t n, uint32_t ldc) {
  int mismatches = 0;
  for (uint32_t row = 0; row < GEMM_NAIVE_VC4KERNEL_C_ROWS; row++) {
    for (uint32_t col = 0; col < GEMM_NAIVE_VC4KERNEL_MAX_LDC; col++) {
      uint32_t active = row < m && col < n && col < ldc;
      uint32_t allocated = col < ldc;
      uint32_t idx = row * GEMM_NAIVE_VC4KERNEL_MAX_LDC + col;
      if (!allocated)
        continue;
      idx = row * ldc + col;
      if (!active && c_values[idx] != GEMM_NAIVE_VC4KERNEL_SENTINEL) {
        if (mismatches < 8)
          printk("ERROR: gemm_naive_vc4kernel sentinel row=%d col=%d value=%f expected=%f\n",
                 (int)row, (int)col, c_values[idx],
                 GEMM_NAIVE_VC4KERNEL_SENTINEL);
        mismatches++;
      }
    }
  }
...
void notmain(void) {
  ...
  uint32_t a_bytes = GEMM_NAIVE_VC4KERNEL_A_WORDS * sizeof(float);
  uint32_t b_bytes = GEMM_NAIVE_VC4KERNEL_B_WORDS * sizeof(float);
  uint32_t c_bytes = GEMM_NAIVE_VC4KERNEL_C_WORDS * sizeof(float);
  ...
  if (vc4_m2_malloc(program, &a_dev, a_bytes) < 0 ||
    ...
    panic("gemm_naive_vc4kernel device allocation failed");
  ...
  vc4_dim3 block = vc4_m2_dim3(GEMM_NAIVE_VC4KERNEL_LANES, 1, 1);

  printk("Running VC4 gemm_naive_vc4kernel candidate bundle...\n");
  for (uint32_t case_id = 0; case_id < sizeof(test_cases) / sizeof(test_cases[0]);
    ...
    if (vc4_m2_copy_htod(program, a_dev, a_values, a_bytes) < 0 ||
      ...
      printk("ERROR: gemm_naive_vc4kernel copy-in failed case=%d m=%d n=%d k=%d lda=%d ldb=%d ldc=%d\n",
    ...
    if (gemm_naive_vc4kernel_launch(program, grid, block, a_dev, b_dev, c_dev,
                                    m, n, k, lda, ldb, ldc) < 0 ||
        vc4_m2_copy_dtoh(program, c_values, c_dev, c_bytes) < 0) {
      printk("ERROR: gemm_naive_vc4kernel launch/copy failed case=%d m=%d n=%d k=%d lda=%d ldb=%d ldc=%d\n",
             (int)case_id, (int)m, (int)n, (int)k, (int)lda, (int)ldb,
             (int)ldc);
      launch_failures++;
      continue;
    ...
    if (checksum != expected_checksum) {
      printk("ERROR: gemm_naive_vc4kernel checksum mismatch case=%d gpu=%d cpu=%d\n",
    ...
    saw_k_diff |= (k != m && k != n);
    saw_tail_n |= (n % GEMM_NAIVE_VC4KERNEL_LANES) != 0u;
    ...
    printk("GEMM_NAIVE_VC4KERNEL_CASE case=%d m=%d n=%d k=%d lda=%d ldb=%d ldc=%d grid_x=%d grid_y=%d mismatches=%d sentinel_mismatches=%d checksum=%d max_abs_diff=%f\n",
           (int)case_id, (int)m, (int)n, (int)k, (int)lda, (int)ldb,
  ...
  int runtime_launches = (int)gemm_naive_vc4kernel_runtime_launches();
  launch_failures += (int)gemm_naive_vc4kernel_runtime_launch_failures();
  const char *status =
      (total_mismatches == 0 && sentinel_mismatches == 0 &&
       launch_failures == 0 && saw_zero && saw_non_square && saw_k_diff &&
       saw_tail_n && saw_runtime_ld && saw_contiguous_ld && saw_plus1_ld &&
       saw_plus7_ld && saw_large_ld && saw_old_timeout_shape)
          ? "PASS"
          : "FAIL";
  printk("VC4_TEST_RESULT name=gemm_naive_vc4kernel status=%s cases=%d total_mismatches=%d sentinel_mismatches=%d launch_failures=%d block_x=%d lanes=%d max_m=%d max_n=%d max_k=%d max_ld=%d saw_zero=%d saw_non_square=%d saw_k_diff=%d saw_tail_n=%d saw_runtime_ld=%d saw_contiguous_ld=%d saw_plus1_ld=%d saw_plus7_ld=%d saw_large_ld=%d saw_old_timeout_shape=%d contiguous_layout_only=0 launch_geometry_2d_block16=1 checksum_accum=%d max_abs_diff=%f runtime_allocations=%d runtime_launches=%d elapsed_usec=%d\n",
         status, (int)(sizeof(test_cases) / sizeof(test_cases[0])),
         total_mismatches, sentinel_mismatches, launch_failures,
         GEMM_NAIVE_VC4KERNEL_LANES, GEMM_NAIVE_VC4KERNEL_LANES,
         GEMM_NAIVE_VC4KERNEL_MAX_M, GEMM_NAIVE_VC4KERNEL_MAX_N,
         GEMM_NAIVE_VC4KERNEL_MAX_K, GEMM_NAIVE_VC4KERNEL_MAX_LDC, saw_zero,
