module {
  vc4kernel.kernel @fragment_reduce_sum_vc4kernel(%input : i32, %out : i32, %threshold : i32) attributes {
    ...
    %values = vc4kernel.tmu_load_fragment %input, %byte_offsets, %full {memory_path = #vc4kernel.memory_path<tmu_global_read>, coherency = #vc4kernel.coherency<readonly_tmu>} : i32, vector<16xi32>, !vc4kernel.pred<16> -> vector<16xf32>
