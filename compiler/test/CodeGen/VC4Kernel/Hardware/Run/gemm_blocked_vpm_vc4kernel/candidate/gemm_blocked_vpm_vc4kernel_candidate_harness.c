  ...
  for (uint32_t row = 0; row < m; row++) {
    for (uint32_t col = 0; col < n; col++) {
      ...
      if (ad > GEMM_BLOCKED_VPM_VC4KERNEL_EPSILON) {
        if (*mismatch_count < 8)
          printk("ERROR: gemm_blocked_vpm_vc4kernel row=%d col=%d gpu=%f cpu=%f diff=%f\n",
...
static int verify_sentinels(void) {
  ...
  for (uint32_t i = 0; i < GEMM_BLOCKED_VPM_VC4KERNEL_C_WORDS; i++) {
    if (expected_values[i] == GEMM_BLOCKED_VPM_VC4KERNEL_SENTINEL &&
        c_values[i] != GEMM_BLOCKED_VPM_VC4KERNEL_SENTINEL) {
      if (mismatches < 8)
        printk("ERROR: gemm_blocked_vpm_vc4kernel sentinel index=%d value=%f expected=%f\n",
...
void notmain(void) {
  ...
  if (vc4_m2_malloc(program, &a_dev, a_bytes) < 0 ||
    ...
    panic("gemm_blocked_vpm_vc4kernel device allocation failed");
  ...
  printk("Running VC4 gemm_blocked_vpm_vc4kernel candidate bundle...\n");
  for (uint32_t case_id = 0; case_id < sizeof(test_cases) / sizeof(test_cases[0]);
    ...
    if (vc4_m2_copy_htod(program, a_dev, a_values, a_bytes) < 0 ||
      ...
      printk("ERROR: gemm_blocked_vpm_vc4kernel copy-in failed case=%d m=%d n=%d k=%d lda=%d ldb=%d ldc=%d\n",
    ...
    if (gemm_blocked_vpm_vc4kernel_launch(program, grid, block, a_dev, b_dev,
                                          c_dev, m, n, k, lda, ldb, ldc) < 0 ||
        vc4_m2_copy_dtoh(program, c_values, c_dev, c_bytes) < 0) {
      printk("ERROR: gemm_blocked_vpm_vc4kernel launch/copy failed case=%d m=%d n=%d k=%d lda=%d ldb=%d ldc=%d\n",
             (int)case_id, (int)m, (int)n, (int)k, (int)lda, (int)ldb,
             (int)ldc);
      launch_failures++;
      continue;
    ...
    if (checksum != expected_checksum) {
      printk("ERROR: gemm_blocked_vpm_vc4kernel checksum mismatch case=%d gpu=%d cpu=%d\n",
    ...
    printk("GEMM_BLOCKED_VPM_VC4KERNEL_CASE case=%d m=%d n=%d k=%d lda=%d ldb=%d ldc=%d grid_x=%d grid_y=%d mismatches=%d sentinel_mismatches=%d checksum=%d max_abs_diff=%f\n",
           (int)case_id, (int)m, (int)n, (int)k, (int)lda, (int)ldb,
  ...
  int runtime_launches = (int)gemm_blocked_vpm_vc4kernel_runtime_launches();
  launch_failures += (int)gemm_blocked_vpm_vc4kernel_runtime_launch_failures();
  const char *status =
      (total_mismatches == 0 && sentinel_mismatches == 0 &&
       ...
       saw_plus7_ld && saw_large_ld)
          ? "PASS"
          : "FAIL";
  printk("VC4_TEST_RESULT name=gemm_blocked_vpm_vc4kernel status=%s cases=%d total_mismatches=%d sentinel_mismatches=%d launch_failures=%d lanes=%d block_m=%d block_n=%d block_k=%d max_m=%d max_n=%d max_k=%d max_ld=%d saw_zero=%d saw_non_square=%d saw_k_tail=%d saw_n_tail=%d saw_runtime_ld=%d saw_contiguous_ld=%d saw_plus1_ld=%d saw_plus7_ld=%d saw_large_ld=%d dynamic_shape_args=1 vdr_rect_path=1 blocked_vpm_path=1 no_tmu_ab_path=1 no_fixed_padded_stride=1 checksum_accum=%d max_abs_diff=%f runtime_allocations=%d runtime_launches=%d elapsed_usec=%d\n",
         ...
         saw_k_tail, saw_n_tail, saw_runtime_ld, saw_contiguous_ld,
