...
static uint32_t expected_value(uint32_t segment, uint32_t lane) {
    if (segment == 0u)
        return lane;
    if (segment >= 1u && segment <= 5u) {
        uint32_t shift = segment - 1u;
        return 7u + (lane << shift);
    }
    uint32_t shift = segment - 6u;
    return 1000u - (lane << shift);
}
    ...
    for (uint32_t segment = 0; segment < SEGMENTS; ++segment) {
        for (uint32_t lane = 0; lane < LANES; ++lane) {
            ...
            if (out_values[index] != expected) {
                if (mismatches < 8)
                    printk("ERROR: fragment_const_lane_range_affine segment=%d lane=%d gpu=%x expected=%x\n",
...
static int verify_sentinels(void) {
    ...
    for (uint32_t i = ACTIVE_N; i < BUFFER_N; ++i) {
        if (out_values[i] != SENTINEL) {
            if (mismatches < 8)
                printk("ERROR: fragment_const_lane_range_affine sentinel i=%d gpu=%x expected=%x\n",
...
void notmain(void) {
    ...
    if (vc4_m2_malloc(program, &out_dev, BUFFER_N * sizeof(uint32_t)) < 0)
        panic("fragment_const_lane_range_affine_vc4kernel allocation failed");
    ...
    if (vc4_m2_copy_htod(program, out_dev, out_values, BUFFER_N * sizeof(uint32_t)) < 0 ||
        fragment_const_lane_range_affine_vc4kernel_launch(program, grid, block, out_dev) < 0 ||
    ...
    printk("VC4_TEST_RESULT name=fragment_const_lane_range_affine_vc4kernel status=%s cases=1 total_mismatches=%d sentinel_mismatches=%d launch_failures=%d active_qpus=1 lanes=%d segments=%d lane_range_exact=1 lane_affine_add_shifts=5 lane_affine_sub_shifts=5 checksum_accum=%d runtime_allocations=1 runtime_launches=1 elapsed_usec=%d\n",
