...
static int verify_active(uint32_t n, uint32_t amount_seed,
    ...
    for (uint32_t i = 0; i < n; ++i) {
        ...
        if (diff > max_f32(ABS_TOL, REL_TOL * abs_f32(expected)) ||
            ...
            if (mismatches < 8)
                printk("ERROR: mixed_dynamic_rotate_scan i=%d amount=%d out=%f expected=%f diff=%f rel=%f audit=%x expected_audit=%x\n",
...
static int verify_sentinels(uint32_t n) {
    ...
    for (uint32_t i = n; i < BUFFER_N; ++i) {
        ...
        if (out_values[i] != OUT_SENTINEL || audit_values[i] != expected_audit) {
            if (mismatches < 8)
                printk("ERROR: mixed_dynamic_rotate_scan sentinel i=%d out=%f audit=%x expected_audit=%x\n",
...
void notmain(void) {
    ...
    if (vc4_program_create(&program, 128u * 1024u) < 0 || !program)
        panic("mixed_dynamic_rotate_reduction_scan_vc4kernel program create failed");
    ...
    if (vc4_m2_malloc(program, &input_dev, sizeof(input_values)) < 0 ||
        ...
        panic("mixed_dynamic_rotate_reduction_scan_vc4kernel allocation failed");
    ...
    printk("Running VC4 mixed_dynamic_rotate_reduction_scan_vc4kernel candidate bundle...\n");
    for (uint32_t n_id = 0; n_id < sizeof(n_cases) / sizeof(n_cases[0]); ++n_id) {
        for (uint32_t a_id = 0; a_id < sizeof(amount_cases) / sizeof(amount_cases[0]); ++a_id) {
            ...
            if (vc4_m2_copy_htod(program, input_dev, input_values, sizeof(input_values)) < 0 ||
                ...
                mixed_dynamic_rotate_reduction_scan_vc4kernel_launch(
                ...
                printk("ERROR: mixed_dynamic_rotate_scan launch/copy failed n=%d amount=%d\n",
                       (int)n, (int)amount);
                launch_failures++;
                continue;
            }
            ...
            printk("MIXED_DYNAMIC_ROTATE_REDUCTION_SCAN_CASE n=%d amount=%d waves=%d mismatches=%d sentinel_mismatches=%d checksum=%d max_abs_diff=%f max_rel_diff=%f\n",
                   (int)n, (int)amount, (int)waves, mismatches, sentinels,
                   checksum, max_abs_diff, max_rel_diff);
        }
    }

    launch_failures += (int)mixed_dynamic_rotate_reduction_scan_vc4kernel_runtime_launch_failures();
    uint32_t runtime_launches = mixed_dynamic_rotate_reduction_scan_vc4kernel_runtime_launches();
    const char *status = (total_mismatches == 0 && sentinel_mismatches == 0 &&
                          launch_failures == 0 && saw_amount0 && saw_amount15 &&
                          saw_amount16_or_modulo)
                             ? "PASS"
                             : "FAIL";
    printk("VC4_TEST_RESULT name=mixed_dynamic_rotate_reduction_scan_vc4kernel status=%s cases=%d total_mismatches=%d sentinel_mismatches=%d launch_failures=%d saw_dynamic_rotate=1 saw_amount0=%d saw_amount15=%d saw_amount16_or_modulo=%d saw_tmu_safe_offset=1 saw_fragment_reduce=1 saw_vdw_preserve=1 saw_cmp_select=1 saw_p9_subword_sidepath=1 saw_sfu_sidepath=1 rotate_direction=left_source_plus_amount checksum_accum=%d max_abs_diff=%f max_rel_diff=%f runtime_allocations=4 runtime_launches=%d elapsed_usec=%d\n",
           status,
