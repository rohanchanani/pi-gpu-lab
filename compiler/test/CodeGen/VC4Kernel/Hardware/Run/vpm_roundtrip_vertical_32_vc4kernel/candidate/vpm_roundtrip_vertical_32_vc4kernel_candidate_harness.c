...
static uint32_t expected_value(uint32_t row, uint32_t lane) {
  if (row == 0u)
    return BASE_VALUE + lane;
  return lane == 3u ? BASE_VALUE : 0u;
}
  ...
  *checksum = 0;
  for (uint32_t row = 0; row < ROWS; ++row) {
    for (uint32_t lane = 0; lane < LANES; ++lane) {
      ...
      uint32_t expected = expected_value(row, lane);
      *checksum += out_values[index];
      if (out_values[index] != expected) {
        if (mismatches < 8)
          printk("ERROR: vpm_roundtrip_vertical row=%d lane=%d actual=%x expected=%x\n",
...
static int verify_sentinels(void) {
  ...
  for (uint32_t i = WORDS; i < BUFFER_WORDS; ++i) {
    if (out_values[i] != SENTINEL) {
      if (mismatches < 8)
        printk("ERROR: vpm_roundtrip_vertical sentinel=%d actual=%x expected=%x\n",
...
void notmain(void) {
  ...
  if (vc4_program_create(&program, 0) < 0 || !program)
    panic("vpm_roundtrip_vertical_32_vc4kernel program create failed");

  vc4_deviceptr_t out_dev = 0;
  uint32_t bytes = BUFFER_WORDS * sizeof(uint32_t);
  if (vc4_m2_malloc(program, &out_dev, bytes) < 0)
    panic("vpm_roundtrip_vertical_32_vc4kernel allocation failed");
  ...
  if (vc4_m2_copy_htod(program, out_dev, out_values, bytes) < 0 ||
      vpm_roundtrip_vertical_32_vc4kernel_launch(program, grid, block,
                                                 out_dev) < 0 ||
      vc4_m2_copy_dtoh(program, out_values, out_dev, bytes) < 0) {
    printk("ERROR: vpm_roundtrip_vertical_32_vc4kernel launch/copy failed\n");
    ++launch_failures;
  } else {
  ...
  launch_failures +=
      (int)vpm_roundtrip_vertical_32_vc4kernel_runtime_launch_failures();
  uint32_t runtime_launches =
      vpm_roundtrip_vertical_32_vc4kernel_runtime_launches();
  int elapsed = timer_get_usec() - start;
  const char *status = (total_mismatches == 0 && sentinel_mismatches == 0 &&
                        launch_failures == 0)
                           ? "PASS"
                           : "FAIL";
  printk("VC4_TEST_RESULT name=vpm_roundtrip_vertical_32_vc4kernel status=%s cases=1 total_mismatches=%d sentinel_mismatches=%d launch_failures=%d active_qpus=1 lanes=%d rows=%d checksum_accum=%u runtime_allocations=1 runtime_launches=%u elapsed_usec=%d\n",
