...
static const uint32_t f16_carriers[LANES] = {
...
static const float expected_f32[LANES] = {
...
static int verify_outputs(float *max_abs_diff, uint32_t *checksum) {
    ...
    *checksum = 0;
    for (uint32_t lane = 0; lane < LANES; ++lane) {
        float diff = absf_local(out_f32[lane] - expected_f32[lane]);
        if (diff > *max_abs_diff)
            *max_abs_diff = diff;
        *checksum += f32_bits(out_f32[lane]);
        if (f32_bits(out_f32[lane]) != f32_bits(expected_f32[lane])) {
            if (mismatches < 8)
                printk("ERROR: f16_unpack lane=%d got=%x exp=%x got_f=%f exp_f=%f\n",
                       (int)lane, f32_bits(out_f32[lane]),
                       f32_bits(expected_f32[lane]), out_f32[lane],
                       expected_f32[lane]);
            mismatches++;
        }
    }
...
static int verify_sentinels(void) {
    ...
    for (uint32_t i = LANES; i < BUFFER_N; ++i) {
        if (f32_bits(out_f32[i]) != f32_bits(F_SENTINEL)) {
            if (mismatches < 8)
                printk("ERROR: f16_unpack sentinel i=%d got=%x\n",
...
void notmain(void) {
    ...
    if (vc4_m2_malloc(program, &input_dev, LANES * sizeof(uint32_t)) < 0 ||
        ...
        panic("fragment_f16_unpack_to_f32 allocation failed");
    ...
    if (vc4_m2_copy_htod(program, input_dev, f16_carriers, LANES * sizeof(uint32_t)) < 0 ||
        vc4_m2_copy_htod(program, out_dev, out_f32, BUFFER_N * sizeof(float)) < 0 ||
        fragment_f16_unpack_to_f32_vc4kernel_launch(program, grid, block, input_dev, out_dev) < 0 ||
