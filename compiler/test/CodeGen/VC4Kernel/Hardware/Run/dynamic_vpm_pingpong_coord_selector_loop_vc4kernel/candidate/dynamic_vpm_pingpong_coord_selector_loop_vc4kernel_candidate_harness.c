...
void notmain(void) {
  ...
  if (vc4_program_create(&program, 0) < 0 || !program)
    panic("dynamic_vpm_pingpong_coord_selector_loop_vc4kernel program create failed");
  if (vc4_m2_malloc(program, &in_dev, sizeof(input_bytes)) < 0 ||
      vc4_m2_malloc(program, &out_dev, sizeof(output_bytes)) < 0)
    panic("dynamic_vpm_pingpong_coord_selector_loop_vc4kernel allocation failed");
  ...
  for (uint32_t i = 0; i < sizeof(cases) / sizeof(cases[0]); ++i) {
    ...
    if (vc4_m2_copy_htod(program, in_dev, input_bytes, sizeof(input_bytes)) < 0 ||
        ...
        dynamic_vpm_pingpong_coord_selector_loop_vc4kernel_launch(
  ...
  launch_failures +=
      (int)dynamic_vpm_pingpong_coord_selector_loop_vc4kernel_runtime_launch_failures();
  uint32_t launches =
      dynamic_vpm_pingpong_coord_selector_loop_vc4kernel_runtime_launches();
  ...
  const char *status = (launch_failures == 0 && total_mismatches == 0 &&
                        sentinel_mismatches == 0 && launches == case_count)
                           ? "PASS"
                           : "FAIL";
  printk("VC4_TEST_RESULT name=dynamic_vpm_pingpong_coord_selector_loop_vc4kernel status=%s cases=%d total_mismatches=%d sentinel_mismatches=%d launch_failures=%d saw_dynamic_vpm_pingpong=1 saw_dynamic_vpm_row=1 saw_dynamic_vpm_x=1 saw_dynamic_subword_selector=1 saw_vdr_dynamic_subword_selector=1 saw_vdw_dynamic_subword_selector=1 saw_runtime_loop=1 saw_iters0=1 saw_iters4=1 saw_tail_cols=1 saw_vdw_preserve=1 no_tmu_to_vpm=1 selector_values_w16=0,1 x_values=0,1 checksum_accum=%u runtime_launches=%u elapsed_usec=%d\n",
