...
static const uint32_t input_values[LANES] = {
    0x80000000u, 0xffffffffu, 0u, 1u, 0x7fffffffu, 0xfffffffeu,
    42u, 0xffffffd6u, 0x40000000u, 0xc0000000u, 17u, 0xfffffff0u,
    0x12345678u, 0xedcba988u, 2u, 0xfffffffeu
};
...
static int active(uint32_t predicate, uint32_t lane) {
    ...
    if (predicate == 2u)
        return lane == 7u;
    return lane == 1u || lane == 4u || lane == 10u || lane == 15u;
}

static uint32_t identity(uint32_t kind) {
    return kind == 0u ? 0x7fffffffu : 0x80000000u;
}
...
static uint32_t expected_value(uint32_t kind, uint32_t predicate) {
    ...
    for (uint32_t lane = 0; lane < LANES; lane++) {
        if (!active(predicate, lane))
            continue;
        int32_t a = as_i32(acc);
        int32_t b = as_i32(input_values[lane]);
        if (kind == 0u)
            acc = (b < a) ? input_values[lane] : acc;
        else
            acc = (b > a) ? input_values[lane] : acc;
    }
    return acc;
        ...
        for (uint32_t predicate = 0; predicate < PREDICATES; predicate++) {
            ...
            for (uint32_t lane = 0; lane < LANES; lane++) {
                ...
                if (out_values[index] != expected) {
                    if (mismatches < 8)
                        printk("ERROR: reduce_signed_minmax kind=%d pred=%d lane=%d gpu=%x expected=%x\n",
...
static int verify_sentinels(void) {
    ...
    for (uint32_t i = ACTIVE_N; i < BUFFER_N; i++) {
        if (out_values[i] != SENTINEL) {
            if (mismatches < 8)
                printk("ERROR: reduce_signed_minmax sentinel i=%d gpu=%x expected=%x\n",
...
void notmain(void) {
    ...
    if (vc4_m2_malloc(program, &input_dev, input_bytes) < 0 ||
        ...
        panic("fragment_reduce_i32_signed_minmax allocation failed");
    ...
    if (vc4_m2_copy_htod(program, input_dev, input_values, input_bytes) < 0 ||
        ...
        fragment_reduce_i32_signed_minmax_vc4kernel_launch(program, grid, block,
                                                           input_dev, out_dev) < 0 ||
    ...
    printk("VC4_TEST_RESULT name=fragment_reduce_i32_signed_minmax_vc4kernel status=%s cases=1 total_mismatches=%d sentinel_mismatches=%d launch_failures=%d active_qpus=1 lanes=%d kinds=%d predicates=%d checksum_accum=%d runtime_allocations=2 runtime_launches=1 elapsed_usec=%d\n",
