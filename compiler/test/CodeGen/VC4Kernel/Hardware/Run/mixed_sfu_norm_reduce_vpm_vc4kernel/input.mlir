module {
  vc4kernel.kernel @mixed_sfu_norm_reduce_vpm_vc4kernel(%input : i32, %flags : i32, %out : i32, %audit : i32, %n : i32, %epsilon : f32) attributes {
    public_name = "mixed_sfu_norm_reduce_vpm_vc4kernel",
    ...
    %sq = vc4kernel.fragment_alu.mul %values, %values {opcode = #vc4kernel.mul_alu_opcode<fmul>} : (vector<16xf32>, vector<16xf32>) -> vector<16xf32>
    ...
    %norm = vc4kernel.fragment_alu.mul %values, %inv_norm {opcode = #vc4kernel.mul_alu_opcode<fmul>} : (vector<16xf32>, vector<16xf32>) -> vector<16xf32>
    ...
    %audit_v = vc4kernel.fragment_alu.add %flags_u8, %lanes {opcode = #vc4kernel.add_alu_opcode<xor>} : (vector<16xi32>, vector<16xi32>) -> vector<16xi32>
