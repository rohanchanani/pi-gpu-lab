  ...
  *sentinel_mismatches = 0;
  ...
  for (uint32_t i = 0; i < OUT_BYTES; ++i) {
    ...
    if (got != want) {
      if (active) {
        if (mismatches < 8)
          printk("ERROR: mixed surface vpm active case=%d offset=%d got=%x want=%x\n",
                 (int)case_index, (int)i, got, want);
        ++mismatches;
      } else {
        if (*sentinel_mismatches < 8)
          printk("ERROR: mixed surface vpm sentinel case=%d offset=%d got=%x want=%x\n",
                 (int)case_index, (int)i, got, want);
        ++*sentinel_mismatches;
      }
    }
  ...
  for (uint32_t lane = 0; lane < 16u; ++lane) {
    ...
    if (got != want) {
      if (mismatches < 8)
        printk("ERROR: mixed surface vpm compute lane case=%d lane=%d got=%x want=%x\n",
  ...
  for (uint32_t lane = 0; lane < 16u; ++lane) {
    ...
    if (got != sum) {
      if (mismatches < 8)
        printk("ERROR: mixed surface vpm reduce lane case=%d lane=%d got=%x want=%x\n",
  ...
  if (got_w32 != want_w32) {
    if (mismatches < 8)
      printk("ERROR: mixed surface vpm w32 case=%d got=%x want=%x\n",
...
void notmain(void) {
  ...
  if (vc4_program_create(&program, 0) < 0 || !program)
    panic("mixed_surface_lock_vpm_pipeline_full_vc4kernel program create failed");
  if (vc4_m2_malloc(program, &in_dev, sizeof(input_bytes)) < 0 ||
      vc4_m2_malloc(program, &out_dev, sizeof(output_bytes)) < 0)
    panic("mixed_surface_lock_vpm_pipeline_full_vc4kernel allocation failed");
  ...
  for (uint32_t i = 0; i < sizeof(cases) / sizeof(cases[0]); ++i) {
    ...
    if (vc4_m2_copy_htod(program, in_dev, input_bytes, sizeof(input_bytes)) < 0 ||
        ...
        mixed_surface_lock_vpm_pipeline_full_vc4kernel_launch(
  ...
  launch_failures +=
      (int)mixed_surface_lock_vpm_pipeline_full_vc4kernel_runtime_launch_failures();
  uint32_t launches =
      mixed_surface_lock_vpm_pipeline_full_vc4kernel_runtime_launches();
  ...
  const char *status = (launch_failures == 0 && total_mismatches == 0 &&
                        sentinel_mismatches == 0 && launches == case_count)
                           ? "PASS"
                           : "FAIL";
  printk("VC4_TEST_RESULT name=mixed_surface_lock_vpm_pipeline_full_vc4kernel status=%s cases=%d total_mismatches=%d sentinel_mismatches=%d launch_failures=%d saw_vdr_dynamic_coord_selector=1 saw_vpm_qpu_dynamic_coord_selector=1 saw_vdw_dynamic_coord_selector=1 saw_subword_path=1 saw_byte_halfword_guards=1 saw_vdw_preserve=1 no_tmu_to_vpm=1 no_sfu_reason=1 saw_final_surface_mix=1 saw_w32_path=1 saw_fragment_alu=1 saw_cmp_select=1 saw_fragment_reduce=1 saw_dynamic_rotate=1 saw_iters0=1 saw_iters4=1 selector_values_w8=0,1,2,3 selector_values_w16=0,1 x_values=0,1 checksum_accum=%u runtime_launches=%u elapsed_usec=%d\n",
