  ...
  *max_abs_diff = 0.0f;
  for (uint32_t row = 0; row < m; row++) {
    ...
    if (ad > GEMV_BLOCKED_VPM_VC4KERNEL_EPSILON) {
      if (*mismatch_count < 8)
        printk("ERROR: gemv_blocked_vpm_vc4kernel row=%d gpu=%f cpu=%f diff=%f\n",
...
static int verify_sentinel_region(uint32_t m) {
  ...
  for (uint32_t i = m; i < GEMV_BLOCKED_VPM_VC4KERNEL_Y_WORDS; i++) {
    if (y_values[i] != GEMV_BLOCKED_VPM_VC4KERNEL_SENTINEL) {
      if (mismatches < 8)
        printk("ERROR: gemv_blocked_vpm_vc4kernel sentinel changed row=%d value=%f expected=%f\n",
...
void notmain(void) {
  ...
  if (vc4_m2_malloc(program, &a_dev, a_bytes) < 0 ||
    ...
    panic("gemv_blocked_vpm_vc4kernel device allocation failed");

  vc4_dim3 block = vc4_m2_dim3(GEMV_BLOCKED_VPM_VC4KERNEL_LANES, 1u, 1u);

  printk("Running VC4 gemv_blocked_vpm_vc4kernel candidate bundle...\n");
  ...
  for (uint32_t case_id = 0;
    ...
    if (vc4MemcpyHtoD(program, a_dev, a_values, a_bytes) < 0 ||
      ...
      printk("ERROR: gemv_blocked_vpm_vc4kernel launch/copy failed case=%d m=%d n=%d lda=%d\n",
