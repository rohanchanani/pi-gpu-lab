  ...
  for (uint32_t r = 0; r < ROWS; ++r) {
    for (uint32_t c = 0; c < COLS; ++c) {
      ...
      *checksum += got;
      if (got != want) {
        if (mismatches < 8)
          printk("ERROR: dynamic_vdr_rect_to_vpm row=%d col=%d got=%x want=%x\n",
...
static int verify_guard(void) {
  ...
  for (uint32_t i = OUTPUT_WORDS; i < OUTPUT_WORDS + GUARD; ++i) {
    if (output_values[i] != SENTINEL) {
      if (mismatches < 8)
        printk("ERROR: dynamic_vdr_rect_to_vpm guard=%d got=%x want=%x\n",
...
void notmain(void) {
  ...
  if (vc4_program_create(&program, 0) < 0 || !program)
    panic("dynamic_vdr_rect_to_vpm_vc4kernel program create failed");
  if (vc4_m2_malloc(program, &in_dev, INPUT_WORDS * sizeof(uint32_t)) < 0 ||
    ...
    panic("dynamic_vdr_rect_to_vpm_vc4kernel allocation failed");
  ...
  if (vc4_m2_copy_htod(program, in_dev, input_values, INPUT_WORDS * sizeof(uint32_t)) < 0 ||
      ...
      dynamic_vdr_rect_to_vpm_vc4kernel_launch(
          program, grid, block, in_dev, out_dev, ACTIVE_ROWS, ACTIVE_COLS,
          INPUT_STRIDE_WORDS * sizeof(uint32_t)) < 0 ||
  ...
  launch_failures +=
      (int)dynamic_vdr_rect_to_vpm_vc4kernel_runtime_launch_failures();
  uint32_t launches = dynamic_vdr_rect_to_vpm_vc4kernel_runtime_launches();
  const char *status = (launch_failures == 0 && total_mismatches == 0 &&
                        sentinel_mismatches == 0 && launches == 1)
                           ? "PASS"
                           : "FAIL";
  printk("VC4_TEST_RESULT name=dynamic_vdr_rect_to_vpm_vc4kernel status=%s cases=1 total_mismatches=%d sentinel_mismatches=%d launch_failures=%d rows=%d cols=%d active_rows=%d active_cols=%d checksum_accum=%u runtime_launches=%u elapsed_usec=%d\n",
