...
static uint32_t expect_u8(uint32_t value) {
    return value & 0xffu;
}

static uint32_t expect_s16(uint32_t value) {
...
static int verify_outputs(uint32_t *checksum) {
    ...
    *checksum = 0;
    for (uint32_t lane = 0; lane < LANES; ++lane) {
        uint32_t expected_u8 = expect_u8(input_words[lane]);
        uint32_t expected_s16 = expect_s16(input_words[lane]);
        uint32_t got_u8 = out_words[lane];
        uint32_t got_s16 = out_words[LANES + lane];
        *checksum += got_u8 + got_s16;
        if (got_u8 != expected_u8 || got_s16 != expected_s16) {
            if (mismatches < 8) {
                printk("ERROR: fragment_unpack lane=%d got_u8=%x exp_u8=%x got_s16=%x exp_s16=%x input=%x\n",
                       (int)lane, got_u8, expected_u8, got_s16, expected_s16,
                       input_words[lane]);
            }
            mismatches++;
        }
...
static int verify_sentinels(void) {
    ...
    for (uint32_t i = LANES * SEGMENTS; i < OUT_WORDS; ++i) {
        if (out_words[i] != SENTINEL) {
            if (mismatches < 8)
                printk("ERROR: fragment_unpack sentinel i=%d got=%x\n", (int)i, out_words[i]);
            mismatches++;
        }
    }
    return mismatches;
...
void notmain(void) {
    ...
    if (vc4_m2_malloc(program, &input_dev, LANES * sizeof(uint32_t)) < 0 ||
        ...
        panic("fragment_unpack allocation failed");
    ...
    if (vc4_m2_copy_htod(program, input_dev, input_words, LANES * sizeof(uint32_t)) < 0 ||
        ...
        fragment_unpack_subword_vc4kernel_launch(program, grid, block, input_dev, out_dev) < 0 ||
    ...
    printk("VC4_TEST_RESULT name=fragment_unpack_subword_vc4kernel status=%s cases=1 total_mismatches=%d sentinel_mismatches=%d launch_failures=%d active_qpus=1 lanes=%d saw_pack=0 saw_unpack=1 saw_u8_or_s8=1 saw_u16_or_s16=1 saw_policy_edge_values=1 unpack_modes=2 runtime_allocations=2 runtime_launches=1 checksum_accum=%u elapsed_usec=%d\n",
