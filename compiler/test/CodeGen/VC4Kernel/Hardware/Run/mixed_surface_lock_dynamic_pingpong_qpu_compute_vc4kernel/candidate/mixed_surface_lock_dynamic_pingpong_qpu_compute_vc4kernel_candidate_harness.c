...
static const struct test_case cases[] = {
    {0u, 7u}, {1u, 9u}, {2u, 0u}, {3u, 12u}, {4u, 7u},
};
...
static void fill_buffers(uint32_t case_index) {
  ...
  for (uint32_t iter = 0; iter < 4u; ++iter) {
    input_bytes[192u + iter] = (uint8_t)(0x60u + case_index * 3u + iter);
    for (uint32_t lane = 0; lane < 16u; ++lane)
      store16(input_bytes, 320u + iter * 32u + lane * 2u,
              (uint16_t)(0x3400u + case_index * 0x100u + iter * 0x20u + lane));
  }
  store32(input_bytes, 160u, 0x7a000000u | (case_index << 8) | 0x55u);
}

static uint32_t selected_value(uint32_t case_index, uint32_t iter,
                               uint32_t lane, uint32_t active_cols) {
  (void)case_index;
  uint32_t value;
  if ((iter & 1u) == 0u)
    value = lane == 8u ? (uint32_t)input_bytes[192u + iter] : 0x5au;
  else
    value = lane == 0u ? (uint32_t)load16(input_bytes, 320u + iter * 32u + 30u)
                       : 0x55aau;
  value += lane;
...
static int verify_case(const struct test_case *tc, uint32_t case_index,
  ...
  *sentinel_mismatches = 0;
  uint32_t last_iter = tc->iters == 0u ? 0u : tc->iters - 1u;
  uint32_t sum = 0;
  for (uint32_t lane = 0; lane < 16u; ++lane)
    if (tc->iters != 0u)
      sum += selected_value(case_index, last_iter, lane, tc->active_cols);
  for (uint32_t i = 0; i < OUT_BYTES; ++i) {
    ...
    if (i >= 512u && i < 576u) {
      ...
      uint32_t value = tc->iters == 0u
                           ? 0u
                           : selected_value(case_index, last_iter, src_lane,
                                            tc->active_cols);
    ...
    if (got != want) {
      if (active) {
        if (mismatches < 8)
          printk("ERROR: p13 pingpong qpu active case=%d offset=%d got=%x want=%x\n",
                 (int)case_index, (int)i, got, want);
        ++mismatches;
      } else {
        if (*sentinel_mismatches < 8)
          printk("ERROR: p13 pingpong qpu sentinel case=%d offset=%d got=%x want=%x\n",
                 (int)case_index, (int)i, got, want);
        ++*sentinel_mismatches;
      }
    }
  ...
  for (uint32_t lane = 0; lane < 16u; ++lane) {
    ...
    uint32_t want = tc->iters == 0u
                        ? 0u
                        : selected_value(case_index, last_iter, src_lane,
                                         tc->active_cols);
    if (got != want) {
      if (mismatches < 8)
        printk("ERROR: p13 pingpong qpu compute lane case=%d lane=%d got=%x want=%x\n",
  ...
  for (uint32_t lane = 0; lane < 16u; ++lane) {
    ...
    if (got != sum) {
      if (mismatches < 8)
        printk("ERROR: p13 pingpong qpu reduce lane case=%d lane=%d got=%x want=%x\n",
  ...
  if (got_w32 != want_w32) {
    if (mismatches < 8)
      printk("ERROR: p13 pingpong qpu w32 case=%d got=%x want=%x\n",
...
void notmain(void) {
  ...
  if (vc4_program_create(&program, 0) < 0 || !program)
    panic("mixed_surface_lock_dynamic_pingpong_qpu_compute_vc4kernel program create failed");
  if (vc4_m2_malloc(program, &in_dev, sizeof(input_bytes)) < 0 ||
      vc4_m2_malloc(program, &out_dev, sizeof(output_bytes)) < 0)
    panic("mixed_surface_lock_dynamic_pingpong_qpu_compute_vc4kernel allocation failed");
  ...
  for (uint32_t i = 0; i < sizeof(cases) / sizeof(cases[0]); ++i) {
    ...
    if (vc4_m2_copy_htod(program, in_dev, input_bytes, sizeof(input_bytes)) < 0 ||
        ...
        mixed_surface_lock_dynamic_pingpong_qpu_compute_vc4kernel_launch(
  ...
  launch_failures +=
      (int)mixed_surface_lock_dynamic_pingpong_qpu_compute_vc4kernel_runtime_launch_failures();
  uint32_t launches =
      mixed_surface_lock_dynamic_pingpong_qpu_compute_vc4kernel_runtime_launches();
  ...
  const char *status = (launch_failures == 0 && total_mismatches == 0 &&
                        sentinel_mismatches == 0 && launches == case_count)
                           ? "PASS"
                           : "FAIL";
  printk("VC4_TEST_RESULT name=mixed_surface_lock_dynamic_pingpong_qpu_compute_vc4kernel status=%s cases=%d total_mismatches=%d sentinel_mismatches=%d launch_failures=%d saw_dynamic_vpm_pingpong=1 saw_checked_qpu_readback=1 saw_vdr_dynamic_selector=1 saw_vpm_qpu_dynamic_selector=1 saw_runtime_loop=1 saw_iters0=1 saw_iters4=1 saw_vdw_preserve=1 saw_nonzero_selector=1 saw_nonzero_x_or_phase_guard=1 no_tmu_to_vpm=1 saw_fragment_alu=1 saw_cmp_select=1 saw_fragment_reduce=1 saw_dynamic_rotate=1 selector_values_w8=0 selector_values_w16=0,1 x_values=0,1,2,3 checksum_accum=%u runtime_launches=%u elapsed_usec=%d\n",
