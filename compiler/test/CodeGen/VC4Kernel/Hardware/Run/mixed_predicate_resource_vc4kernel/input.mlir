module {
  vc4kernel.kernel @mixed_predicate_resource_vc4kernel(%out : i32, %n : i32, %threshold : i32, %control : i32) attributes {
    ...
    %global_lane = vc4kernel.fragment_add %base_index_v, %lanes : vector<16xi32>, vector<16xi32> -> vector<16xi32>
    %value = vc4kernel.fragment_add %tag_v, %global_lane : vector<16xi32>, vector<16xi32> -> vector<16xi32>
    %tile = vc4kernel.vpm_alloc {rows = 1 : i32, elem_bytes = 4 : i32} : !vc4kernel.vpm_tile
    vc4kernel.vpm_write_fragment %tile, %c0, %value, %mask {orientation = #vc4kernel.vpm_orientation<horizontal>, width = #vc4kernel.vpm_width<w32>, subword = #vc4kernel.vpm_subword<none>, x = 0 : i32, stride = 1 : i32} : !vc4kernel.vpm_tile, i32, vector<16xi32>, !vc4kernel.pred<16>
    %read = vc4kernel.vpm_read_fragment %tile, %c0, %tail {orientation = #vc4kernel.vpm_orientation<horizontal>, width = #vc4kernel.vpm_width<w32>, subword = #vc4kernel.vpm_subword<none>, x = 0 : i32, stride = 1 : i32} : !vc4kernel.vpm_tile, i32, !vc4kernel.pred<16> -> vector<16xi32>
    ...
    vc4kernel.vdw_store_fragment %out, %byte_offsets, %read, %tail : i32, vector<16xi32>, vector<16xi32>, !vc4kernel.pred<16>
    vc4kernel.return
  }
}
