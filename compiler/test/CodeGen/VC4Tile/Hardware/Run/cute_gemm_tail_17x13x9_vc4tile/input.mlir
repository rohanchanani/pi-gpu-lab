...
vc4tile.kernel @cute_gemm_tail_17x13x9_vc4tile(%out : i32, %a : i32, %b : i32) attributes {
  ...
  %store_mask = vc4tile.tail_mask %zero, %store_cols : i32, i32 -> vector<16xi1>
  ...
  %row_global_0 = arith.addi %tile_i_row_base, %zero : i32
  %row_live_0 = arith.cmpi ult, %row_global_0, %seventeen : i32
  scf.if %row_live_0 {
    %out_off_0 = arith.addi %out_tile_base, %zero : i32
    "vc4tile.tile_store"(%acc3, %out, %out_off_0, %store_mask) {shape = [1, 16], layout = #vc4tile.layout<row_major>, memory_space = #vc4tile.memory_space<global>, element_type = i32, storage_type = i32, precision = #vc4tile.precision<exact_32>, boundary = #vc4tile.boundary_policy<tail_predicated>, packing = #vc4tile.packing<none>} : (vector<16xi32>, i32, i32, vector<16xi1>) -> ()
  }
  %row1 = vc4tile.rotate %acc3 {amount = 4 : i32} : vector<16xi32> -> vector<16xi32>
  %row_global_1 = arith.addi %tile_i_row_base, %one : i32
  %row_live_1 = arith.cmpi ult, %row_global_1, %seventeen : i32
  scf.if %row_live_1 {
    %row_stride_1 = arith.constant 13 : i32
    %out_off_1 = arith.addi %out_tile_base, %row_stride_1 : i32
    "vc4tile.tile_store"(%row1, %out, %out_off_1, %store_mask) {shape = [1, 16], layout = #vc4tile.layout<row_major>, memory_space = #vc4tile.memory_space<global>, element_type = i32, storage_type = i32, precision = #vc4tile.precision<exact_32>, boundary = #vc4tile.boundary_policy<tail_predicated>, packing = #vc4tile.packing<none>} : (vector<16xi32>, i32, i32, vector<16xi1>) -> ()
  }
  %row2 = vc4tile.rotate %acc3 {amount = 8 : i32} : vector<16xi32> -> vector<16xi32>
  %row_global_2 = arith.addi %tile_i_row_base, %two : i32
  %row_live_2 = arith.cmpi ult, %row_global_2, %seventeen : i32
  scf.if %row_live_2 {
    %row_stride_2 = arith.constant 26 : i32
    %out_off_2 = arith.addi %out_tile_base, %row_stride_2 : i32
    "vc4tile.tile_store"(%row2, %out, %out_off_2, %store_mask) {shape = [1, 16], layout = #vc4tile.layout<row_major>, memory_space = #vc4tile.memory_space<global>, element_type = i32, storage_type = i32, precision = #vc4tile.precision<exact_32>, boundary = #vc4tile.boundary_policy<tail_predicated>, packing = #vc4tile.packing<none>} : (vector<16xi32>, i32, i32, vector<16xi1>) -> ()
  }
  %row3 = vc4tile.rotate %acc3 {amount = 12 : i32} : vector<16xi32> -> vector<16xi32>
  %row_global_3 = arith.addi %tile_i_row_base, %three : i32
  %row_live_3 = arith.cmpi ult, %row_global_3, %seventeen : i32
  scf.if %row_live_3 {
    %row_stride_3 = arith.constant 39 : i32
    %out_off_3 = arith.addi %out_tile_base, %row_stride_3 : i32
    "vc4tile.tile_store"(%row3, %out, %out_off_3, %store_mask) {shape = [1, 16], layout = #vc4tile.layout<row_major>, memory_space = #vc4tile.memory_space<global>, element_type = i32, storage_type = i32, precision = #vc4tile.precision<exact_32>, boundary = #vc4tile.boundary_policy<tail_predicated>, packing = #vc4tile.packing<none>} : (vector<16xi32>, i32, i32, vector<16xi1>) -> ()
  }
