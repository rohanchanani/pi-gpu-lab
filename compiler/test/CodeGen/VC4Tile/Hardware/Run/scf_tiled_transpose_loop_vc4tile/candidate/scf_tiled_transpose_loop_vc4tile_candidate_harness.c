...
#define VC4_OUT_WORDS 32u
#define VC4_SENTINEL 0x71710004u

static uint32_t matrix_value(uint32_t row, uint32_t col) {
    return 0x62000000u + 601u * row + 17u * col + (col << 5);
}

void notmain(void) {
    struct vc4_program *program = 0;
    vc4_deviceptr_t d_in = 0, d_out = 0;
    uint32_t host_in[VC4_IN_WORDS];
    uint32_t host_out[VC4_OUT_WORDS];
    uint32_t got[VC4_OUT_WORDS];
    ...
    for (uint32_t i = 0; i < VC4_OUT_WORDS; ++i) {
        host_out[i] = VC4_SENTINEL;
        got[i] = 0;
    }
    for (uint32_t row = 0; row < VC4_ROWS; ++row)
        for (uint32_t col = 0; col < VC4_COLS; ++col)
            host_in[VC4_IN_ACTIVE_OFFSET + row * VC4_COLS + col] = matrix_value(row, col);
    ...
    if (!failures) {
        ...
        failures += vc4_m2_check_rc(
            "scf_tiled_transpose_loop_vc4tile_launch",
            scf_tiled_transpose_loop_vc4tile_launch(
    ...
    for (uint32_t i = 0; i < VC4_OUT_WORDS; ++i) {
        uint32_t active = (i >= VC4_OUT_ACTIVE_OFFSET && i < VC4_OUT_ACTIVE_OFFSET + VC4_ROWS);
    ...
    uint32_t launch_failures = scf_tiled_transpose_loop_vc4tile_runtime_launch_failures();
    uint32_t runtime_launches = scf_tiled_transpose_loop_vc4tile_runtime_launches();
    const char *status = (!failures && total_mismatches == 0 && sentinel_mismatches == 0 &&
                          launch_failures == 0 && runtime_launches == 1) ? "PASS" : "FAIL";
    printk("VC4_TEST_RESULT name=scf_tiled_transpose_loop_vc4tile status=%s total_mismatches=%u sentinel_mismatches=%u launch_failures=%u runtime_launches=%u checksum_actual=%u checksum_expected=%u\n",
