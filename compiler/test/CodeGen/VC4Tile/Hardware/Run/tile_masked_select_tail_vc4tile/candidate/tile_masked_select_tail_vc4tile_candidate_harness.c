...
#define VC4_ACTIVE_WORDS 10u
#define VC4_SENTINEL 0x71710903u

static uint32_t x_value(uint32_t i) { return 0x2200u + 9u * i; }

void notmain(void) {
    struct vc4_program *program = 0;
    vc4_deviceptr_t d_x = 0, d_out = 0;
    uint32_t host_x[VC4_WORDS], host_out[VC4_WORDS], got[VC4_WORDS];
    ...
    uint32_t launch_failures = tile_masked_select_tail_vc4tile_runtime_launch_failures();
    uint32_t runtime_launches = tile_masked_select_tail_vc4tile_runtime_launches();
    const char *status = (!failures && total_mismatches == 0 && sentinel_mismatches == 0 &&
                          launch_failures == 0 && runtime_launches == 1) ? "PASS" : "FAIL";
    printk("VC4_TEST_RESULT name=tile_masked_select_tail_vc4tile status=%s total_mismatches=%u sentinel_mismatches=%u launch_failures=%u runtime_launches=%u checksum_actual=%u checksum_expected=%u\n",
