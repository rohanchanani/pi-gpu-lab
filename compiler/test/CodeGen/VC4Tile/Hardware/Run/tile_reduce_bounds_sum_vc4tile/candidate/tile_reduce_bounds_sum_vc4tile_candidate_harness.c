...
#define VC4_SENTINEL 0x6d0b5002u

static const uint32_t k_rows[VC4_CASES] = {0u, 1u, 2u, 3u, 4u, 4u};
static const uint32_t k_cols[VC4_CASES] = {0u, 1u, 3u, 2u, 1u, 4u};

static uint32_t input_word(uint32_t lane) {
    uint32_t row = lane / 4u;
    uint32_t col = lane % 4u;
    return 17u + row * 41u + col * 9u + lane * 3u;
}

static uint32_t reduce_expected(uint32_t active_rows, uint32_t active_cols) {
    uint32_t sum = 0;
    for (uint32_t row = 0; row < active_rows; ++row) {
        for (uint32_t col = 0; col < active_cols; ++col)
            sum += input_word(row * 4u + col);
    }
    return sum;
}
...
static uint32_t expected_word(uint32_t index) {
    ...
    return reduce_expected(k_rows[case_id], k_cols[case_id]);
}
...
void notmain(void) {
    ...
    for (uint32_t c = 0; !failures && c < VC4_CASES; ++c) {
        ...
        failures += vc4_m2_check_rc(
            "tile_reduce_bounds_sum_vc4tile_launch",
            tile_reduce_bounds_sum_vc4tile_launch(
                program, vc4_m2_dim3(1, 1, 1), vc4_m2_dim3(16, 1, 1),
                out_active, d_in, k_rows[c], k_cols[c]));
    }
    ...
    uint32_t launch_failures =
        tile_reduce_bounds_sum_vc4tile_runtime_launch_failures();
    uint32_t runtime_launches =
        tile_reduce_bounds_sum_vc4tile_runtime_launches();
    const char *status = (!failures && total_mismatches == 0 &&
                          sentinel_mismatches == 0 && launch_failures == 0 &&
                          runtime_launches == VC4_CASES) ? "PASS" : "FAIL";
    printk("VC4_TEST_RESULT name=tile_reduce_bounds_sum_vc4tile status=%s cases=%u bounds_predicated_reduce_path=1 total_mismatches=%u sentinel_mismatches=%u launch_failures=%u runtime_launches=%u checksum_actual=%u checksum_expected=%u sample_0x0_lane0=%u sample_3x2_lane7=%u sample_4x4_lane15=%u\n",
