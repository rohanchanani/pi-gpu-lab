...
static const uint32_t k_cols[VC4_CASES] = {4u, 1u, 3u, 2u, 1u, 4u};
...
void notmain(void) {
    ...
    for (uint32_t c = 0; !failures && c < VC4_CASES; ++c) {
        ...
        failures += vc4_m2_check_rc(
            "tile_matmul_bounds_output_mask_vc4tile_launch",
            tile_matmul_bounds_output_mask_vc4tile_launch(
    ...
    uint32_t launch_failures =
        tile_matmul_bounds_output_mask_vc4tile_runtime_launch_failures();
    uint32_t runtime_launches =
        tile_matmul_bounds_output_mask_vc4tile_runtime_launches();
    const char *status =
        (!failures && total_mismatches == 0 && sentinel_mismatches == 0 &&
         launch_failures == 0 && runtime_launches == VC4_CASES)
            ? "PASS"
            : "FAIL";
    printk("VC4_TEST_RESULT name=tile_matmul_bounds_output_mask_vc4tile status=%s cases=%u output_predicate_path=1 total_mismatches=%u sentinel_mismatches=%u launch_failures=%u runtime_launches=%u checksum_actual=%u checksum_expected=%u sample_0x4_lane0=%u sample_2x3_lane6=%u sample_4x4_lane15=%u\n",
           ...
           got[VC4_GUARD_WORDS + 2u * VC4_TILE_WORDS + 6u],
