...
#define VC4_SENTINEL 0x71710902u
#define VC4_SCALE 3u

static uint32_t x_value(uint32_t i) { return 11u + 2u * i; }
...
void notmain(void) {
    ...
    uint32_t launch_failures = tile_mul_store_vc4tile_runtime_launch_failures();
    uint32_t runtime_launches = tile_mul_store_vc4tile_runtime_launches();
    const char *status = (!failures && total_mismatches == 0 && sentinel_mismatches == 0 &&
                          launch_failures == 0 && runtime_launches == 1) ? "PASS" : "FAIL";
    printk("VC4_TEST_RESULT name=tile_mul_store_vc4tile status=%s total_mismatches=%u sentinel_mismatches=%u launch_failures=%u runtime_launches=%u checksum_actual=%u checksum_expected=%u\n",
