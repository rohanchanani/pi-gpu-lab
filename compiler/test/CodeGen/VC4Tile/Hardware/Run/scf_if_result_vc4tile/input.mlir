...
// RUN: bash %S/../../../Support/run_vc4tile_candidate_codegen_test.sh scf_if_result_vc4tile clean
// RUN: bash %S/../../../Support/run_vc4tile_candidate_codegen_test.sh scf_if_result_vc4tile generate
// RUN: bash -c 'd="$(bash %S/../../../Support/run_vc4tile_candidate_codegen_test.sh scf_if_result_vc4tile generated-dir)"; FileCheck --check-prefix=CORE %s < "$d/core.vc4tile.mlir"'

// RAW: legalize-vc4tile-core-cfg
// SSAVC4: ssavc4.func @scf_if_result_vc4tile
...
vc4tile.kernel @scf_if_result_vc4tile attributes {
  public_name = "scf_if_result_vc4tile",
...
^entry(%out: i32, %flag: i32):
  ...
  %total = scf.if %cond -> (i32) {
    %then_value = arith.constant 41 : i32
    scf.yield %then_value : i32
  } else {
    %else_value = arith.constant 5 : i32
