...
#define VC4_SENTINEL 0x5a5ab037u
...
static uint32_t input_word(uint32_t which, uint32_t lane) {
    return 0x41000000u + which * 257u + lane * 17u;
}
...
void notmain(void) {
    ...
    for (uint32_t c = 0; !failures && c < VC4_CASES; ++c) {
        ...
        failures += vc4_m2_check_rc(
            "tile_bounds_mask_load_vc4tile_launch",
            tile_bounds_mask_load_vc4tile_launch(
    ...
    uint32_t launch_failures =
        tile_bounds_mask_load_vc4tile_runtime_launch_failures();
    uint32_t runtime_launches = tile_bounds_mask_load_vc4tile_runtime_launches();
    const char *status =
        (!failures && total_mismatches == 0 && sentinel_mismatches == 0 &&
         launch_failures == 0 && runtime_launches == VC4_CASES)
            ? "PASS"
            : "FAIL";
    printk("VC4_TEST_RESULT name=tile_bounds_mask_load_vc4tile status=%s cases=%u total_mismatches=%u sentinel_mismatches=%u launch_failures=%u runtime_launches=%u checksum_actual=%u checksum_expected=%u sample_1x1_0=%u sample_3x2_lane9=%u sample_4x4_lane15=%u\n",
