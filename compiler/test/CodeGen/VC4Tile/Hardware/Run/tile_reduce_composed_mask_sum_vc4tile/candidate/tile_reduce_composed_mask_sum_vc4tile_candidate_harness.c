...
#define VC4_SENTINEL 0x6d0c5003u
...
static uint32_t input_word(uint32_t lane) {
    ...
    return 23u + row * 31u + col * 11u + lane;
}
...
static uint32_t reduce_expected(uint32_t active_rows, uint32_t active_cols,
    ...
    for (uint32_t lane = 0; lane < VC4_TILE_WORDS; ++lane) {
        uint32_t row = lane / 4u;
        uint32_t col = lane % 4u;
        if (row < active_rows && col < active_cols && lane < active_tail)
            sum += input_word(lane);
    }
    return sum;
}
...
void notmain(void) {
    ...
    for (uint32_t c = 0; !failures && c < VC4_CASES; ++c) {
        ...
        failures += vc4_m2_check_rc(
            "tile_reduce_composed_mask_sum_vc4tile_launch",
            tile_reduce_composed_mask_sum_vc4tile_launch(
    ...
    uint32_t launch_failures =
        tile_reduce_composed_mask_sum_vc4tile_runtime_launch_failures();
    uint32_t runtime_launches =
        tile_reduce_composed_mask_sum_vc4tile_runtime_launches();
    const char *status = (!failures && total_mismatches == 0 &&
                          sentinel_mismatches == 0 && launch_failures == 0 &&
                          runtime_launches == VC4_CASES) ? "PASS" : "FAIL";
    printk("VC4_TEST_RESULT name=tile_reduce_composed_mask_sum_vc4tile status=%s cases=%u composed_predicate_path=1 total_mismatches=%u sentinel_mismatches=%u launch_failures=%u runtime_launches=%u checksum_actual=%u checksum_expected=%u sample_0x4_tail8=%u sample_3x3_tail9=%u sample_4x1_tail13=%u\n",
