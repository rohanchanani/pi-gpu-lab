...
#define VC4_CASE_ACTIVE_OFFSET 4u
#define VC4_CASE_ACTIVE_WORDS 9u
#define VC4_CASE_SENTINEL 0xabcd0002u
...
void notmain(void) {
    ...
    if (!failures) {
        ...
        failures += vc4_m2_check_rc(
            "tile_store_1d_tail_vc4tile_launch",
            tile_store_1d_tail_vc4tile_launch(program, vc4_m2_dim3(1, 1, 1),
                                              vc4_m2_dim3(16, 1, 1), out_active,
                                              VC4_CASE_ACTIVE_WORDS));
    }
    if (!failures)
    ...
    for (uint32_t i = 0; i < VC4_CASE_WORDS; ++i) {
        ...
        uint32_t expected = active ? expected_value(i - VC4_CASE_ACTIVE_OFFSET)
                                   : VC4_CASE_SENTINEL;
    ...
    uint32_t launch_failures = tile_store_1d_tail_vc4tile_runtime_launch_failures();
    uint32_t runtime_launches = tile_store_1d_tail_vc4tile_runtime_launches();
    const char *status = (!failures && total_mismatches == 0 && sentinel_mismatches == 0 &&
                          launch_failures == 0 && runtime_launches == 1) ? "PASS" : "FAIL";
    printk("VC4_TEST_RESULT name=tile_store_1d_tail_vc4tile status=%s total_mismatches=%u sentinel_mismatches=%u launch_failures=%u runtime_launches=%u checksum_actual=%u checksum_expected=%u\n",
