...
#define VC4_SENTINEL 0x71110001u
...
void notmain(void) {
    ...
    if (!failures) {
        failures += vc4_m2_check_rc(
            "tile_dot_1x16_i32_vc4tile_launch",
            tile_dot_1x16_i32_vc4tile_launch(program, vc4_m2_dim3(1, 1, 1), vc4_m2_dim3(16, 1, 1),
                          d_out, d_lhs, d_rhs));
    }
    ...
    uint32_t launch_failures = tile_dot_1x16_i32_vc4tile_runtime_launch_failures();
    uint32_t runtime_launches = tile_dot_1x16_i32_vc4tile_runtime_launches();
    const char *status = (!failures && total_mismatches == 0 && sentinel_mismatches == 0 &&
                          launch_failures == 0 && runtime_launches == 1) ? "PASS" : "FAIL";
    printk("VC4_TEST_RESULT name=tile_dot_1x16_i32_vc4tile status=%s total_mismatches=%u sentinel_mismatches=%u launch_failures=%u runtime_launches=%u checksum_actual=%u checksum_expected=%u\n",
