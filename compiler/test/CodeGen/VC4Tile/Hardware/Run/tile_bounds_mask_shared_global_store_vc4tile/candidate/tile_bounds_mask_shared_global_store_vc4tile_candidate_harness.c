...
static uint32_t expected_word(uint32_t index) {
    ...
    return expected_case_lane(which, lane);
}
...
void notmain(void) {
    ...
    for (uint32_t c = 0; !failures && c < VC4_CASES; ++c) {
        ...
        failures += vc4_m2_check_rc(
            "tile_bounds_mask_shared_global_store_vc4tile_launch",
            tile_bounds_mask_shared_global_store_vc4tile_launch(
        ...
        if (got[i] != expected) {
            if (expected == VC4_SENTINEL) {
                ...
                if (total_mismatches < 16u) {
                    ...
                    uint32_t which = logical / VC4_OUTPUT_WORDS;
                    uint32_t lane = logical % VC4_OUTPUT_WORDS;
                    printk("mismatch case=%u row=%u col=%u got=%u expected=%u\n",
                           which, lane / 4u, lane % 4u, got[i], expected);
                }
                ++total_mismatches;
            }
        }
    ...
    uint32_t launch_failures =
        tile_bounds_mask_shared_global_store_vc4tile_runtime_launch_failures();
    uint32_t runtime_launches =
        tile_bounds_mask_shared_global_store_vc4tile_runtime_launches();
    const char *status =
        (!failures && total_mismatches == 0 && sentinel_mismatches == 0 &&
         launch_failures == 0 && runtime_launches == VC4_CASES)
            ? "PASS"
            : "FAIL";
    printk("VC4_TEST_RESULT name=tile_bounds_mask_shared_global_store_vc4tile status=%s cases=%u shared_global_predicated_path=1 total_mismatches=%u sentinel_mismatches=%u launch_failures=%u runtime_launches=%u checksum_actual=%u checksum_expected=%u sample_1x1_0_0=%u sample_1x1_0_1=%u sample_3x2_2_1=%u sample_4x4_3_3=%u\n",
           status, VC4_CASES, total_mismatches, sentinel_mismatches,
           launch_failures, runtime_launches, checksum_actual,
           checksum_expected,
           got[VC4_GUARD_WORDS],
           got[VC4_GUARD_WORDS + 1u],
           got[VC4_GUARD_WORDS + 3u * VC4_OUTPUT_WORDS + 2u * 4u + 1u],
           got[VC4_GUARD_WORDS + 5u * VC4_OUTPUT_WORDS + 3u * 4u + 3u]);
    printk("DONE!!!\n");
}
