...
#define VC4_SENTINEL 0x5a5a7311u
...
void notmain(void) {
    ...
    if (!failures) {
        vc4_deviceptr_t out_active =
            d_out + VC4_GUARD_WORDS * sizeof(uint32_t);
        failures += vc4_m2_check_rc(
            "tile_rect_mask_store_vc4tile_launch",
            tile_rect_mask_store_vc4tile_launch(
    ...
    uint32_t launch_failures =
        tile_rect_mask_store_vc4tile_runtime_launch_failures();
    uint32_t runtime_launches = tile_rect_mask_store_vc4tile_runtime_launches();
    const char *status =
        (!failures && total_mismatches == 0 && sentinel_mismatches == 0 &&
         launch_failures == 0 && runtime_launches == 1)
            ? "PASS"
            : "FAIL";
    printk("VC4_TEST_RESULT name=tile_rect_mask_store_vc4tile status=%s cases=%u total_mismatches=%u sentinel_mismatches=%u launch_failures=%u runtime_launches=%u checksum_actual=%u checksum_expected=%u sample_1x1_0=%u sample_3x2_lane9=%u sample_4x4_lane15=%u\n",
