...
void notmain(void) {
    ...
    for (uint32_t c = 0; !failures && c < VC4_CASES; ++c) {
        ...
        failures += vc4_m2_check_rc(
            "tile_bounds_mask_shared_roundtrip_vc4tile_launch",
            tile_bounds_mask_shared_roundtrip_vc4tile_launch(
    ...
    uint32_t launch_failures =
        tile_bounds_mask_shared_roundtrip_vc4tile_runtime_launch_failures();
    uint32_t runtime_launches =
        tile_bounds_mask_shared_roundtrip_vc4tile_runtime_launches();
    const char *status =
        (!failures && total_mismatches == 0 && sentinel_mismatches == 0 &&
         launch_failures == 0 && runtime_launches == VC4_CASES)
            ? "PASS"
            : "FAIL";
    printk("VC4_TEST_RESULT name=tile_bounds_mask_shared_roundtrip_vc4tile status=%s cases=%u shared_mask_path=1 total_mismatches=%u sentinel_mismatches=%u launch_failures=%u runtime_launches=%u checksum_actual=%u checksum_expected=%u sample_1x1_lane0=%u sample_1x1_lane1=%u sample_3x2_lane9=%u sample_4x4_lane15=%u\n",
           ...
           got[VC4_GUARD_WORDS + 3u * VC4_OUTPUT_WORDS + 9u],
           got[VC4_GUARD_WORDS + 5u * VC4_OUTPUT_WORDS + 15u]);
    printk("DONE!!!\n");
}
