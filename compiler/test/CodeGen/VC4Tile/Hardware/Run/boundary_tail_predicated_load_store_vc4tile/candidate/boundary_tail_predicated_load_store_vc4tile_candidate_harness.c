...
#define VC4_CASE_SENTINEL 0xcafe0801u

static uint32_t input_value(uint32_t lane) {
    return 0x28000000u + 43u * lane + (lane << 6);
}
...
void notmain(void) {
    ...
    if (!failures) {
        ...
        failures += vc4_m2_check_rc(
            "boundary_tail_predicated_load_store_vc4tile_launch",
            boundary_tail_predicated_load_store_vc4tile_launch(
    ...
    uint32_t launch_failures = boundary_tail_predicated_load_store_vc4tile_runtime_launch_failures();
    uint32_t runtime_launches = boundary_tail_predicated_load_store_vc4tile_runtime_launches();
    const char *status = (!failures && total_mismatches == 0 && sentinel_mismatches == 0 &&
                          launch_failures == 0 && runtime_launches == 1) ? "PASS" : "FAIL";
    printk("VC4_TEST_RESULT name=boundary_tail_predicated_load_store_vc4tile status=%s total_mismatches=%u sentinel_mismatches=%u launch_failures=%u runtime_launches=%u checksum_actual=%u checksum_expected=%u\n",
