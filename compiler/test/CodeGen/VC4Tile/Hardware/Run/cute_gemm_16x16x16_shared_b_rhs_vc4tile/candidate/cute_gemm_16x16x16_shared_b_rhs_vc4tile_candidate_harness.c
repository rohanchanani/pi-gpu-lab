...
#define VC4_MICRO 4u
#define VC4_TILES 4u
#define VC4_PACKED_WORDS 256u
#define VC4_PACKED_STORAGE_WORDS 304u
#define VC4_OUTPUT_WORDS 256u
#define VC4_OUTPUT_STORAGE_WORDS 320u
#define VC4_SENTINEL 0x62a51621u

static const uint32_t kAPads[VC4_CASES] = {0u, 3u, 7u, 11u, 5u, 13u};
static const uint32_t kBPads[VC4_CASES] = {2u, 6u, 10u, 1u, 9u, 14u};
static const uint32_t kOutPads[VC4_CASES] = {1u, 4u, 8u, 12u, 16u, 20u};
...
static int32_t a_value(uint32_t i, uint32_t k) {
    return positive_mod_i32((int32_t)(5u * i + 3u * k + 29u), 67) - 33;
}

static int32_t b_value(uint32_t k, uint32_t j) {
    return positive_mod_i32((int32_t)(19u * k + 2u * j + 31u), 101) - 50;
}
...
static uint32_t a_pack_offset(uint32_t tile_i, uint32_t tile_k,
    ...
    return ((tile_i * VC4_TILES + tile_k) * 16u) + local_i * 4u + local_k;
}

static uint32_t b_pack_offset(uint32_t tile_k, uint32_t tile_j,
                              uint32_t local_k, uint32_t local_j) {
    return ((tile_k * VC4_TILES + tile_j) * 16u) + local_k * 4u + local_j;
}
...
static void fill_inputs(uint32_t *a, uint32_t a_pad, uint32_t *b, uint32_t b_pad,
    ...
    for (uint32_t tile_i = 0; tile_i < VC4_TILES; ++tile_i) {
        for (uint32_t tile_k = 0; tile_k < VC4_TILES; ++tile_k) {
    ...
    for (uint32_t tile_k = 0; tile_k < VC4_TILES; ++tile_k) {
        for (uint32_t tile_j = 0; tile_j < VC4_TILES; ++tile_j) {
            for (uint32_t local_k = 0; local_k < VC4_MICRO; ++local_k) {
...
void notmain(void) {
    ...
    uint32_t sample_c00 = 0, sample_c015 = 0, sample_c57 = 0;
    uint32_t sample_c83 = 0, sample_c150 = 0, sample_c1515 = 0;
    vc4_deviceptr_t d_a = 0, d_b = 0, d_out = 0;
    int failures = 0;

    failures += vc4_m2_check_rc("vc4_program_create", vc4_program_create(&program, 8192));
    if (!failures) failures += vc4_m2_malloc(program, &d_a, VC4_PACKED_STORAGE_WORDS * sizeof(uint32_t)) < 0;
    if (!failures) failures += vc4_m2_malloc(program, &d_b, VC4_PACKED_STORAGE_WORDS * sizeof(uint32_t)) < 0;
    ...
    for (uint32_t c = 0; c < VC4_CASES; ++c) {
        ...
        if (!failures) {
            failures += vc4_m2_check_rc(
                "cute_gemm_16x16x16_shared_b_rhs_vc4tile_launch",
                cute_gemm_16x16x16_shared_b_rhs_vc4tile_launch(
                    program, vc4_m2_dim3(16, 1, 1), vc4_m2_dim3(16, 1, 1),
        ...
        for (uint32_t i = 0; i < VC4_OUTPUT_STORAGE_WORDS; ++i) {
            ...
            if (active) {
                ...
                if (c == 0u) {
                    ...
                    if (row == 0u && col == 15u) sample_c015 = got[i];
                    if (row == 5u && col == 7u) sample_c57 = got[i];
                    if (row == 8u && col == 3u) sample_c83 = got[i];
                    if (row == 15u && col == 0u) sample_c150 = got[i];
                    if (row == 15u && col == 15u) sample_c1515 = got[i];
                }
            }
            if (got[i] != expected) {
                if (active) {
    ...
    uint32_t launch_failures = cute_gemm_16x16x16_shared_b_rhs_vc4tile_runtime_launch_failures();
    uint32_t runtime_launches = cute_gemm_16x16x16_shared_b_rhs_vc4tile_runtime_launches();
    ...
    printk("VC4_TEST_RESULT name=cute_gemm_16x16x16_shared_b_rhs_vc4tile status=%s shared_rhs_path=1 rhs_shared_shape=16x16 rhs_k_tile=4 cases=%u total_mismatches=%u sentinel_mismatches=%u launch_failures=%u runtime_launches=%u checksum_actual=%u checksum_expected=%u sample_c00=%d sample_c015=%d sample_c57=%d sample_c83=%d sample_c150=%d sample_c1515=%d\n",
           status, VC4_CASES, total_mismatches, sentinel_mismatches, launch_failures,
           runtime_launches, checksum_actual, checksum_expected,
           (int32_t)sample_c00, (int32_t)sample_c015, (int32_t)sample_c57,
           (int32_t)sample_c83, (int32_t)sample_c150, (int32_t)sample_c1515);
    printk("DONE!!!\n");
}
