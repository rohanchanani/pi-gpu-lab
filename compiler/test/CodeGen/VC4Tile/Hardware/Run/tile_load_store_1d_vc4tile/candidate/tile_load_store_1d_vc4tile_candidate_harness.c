...
#define VC4_CASE_SENTINEL 0xdeadbeefu

static uint32_t input_value(uint32_t i) {
    return 0x10000000u + 37u * i + (i << 4);
}
...
void notmain(void) {
    ...
    for (uint32_t i = 0; i < VC4_CASE_ACTIVE_WORDS; ++i)
    ...
    if (!failures) {
        ...
        failures += vc4_m2_check_rc(
            "tile_load_store_1d_vc4tile_launch",
            tile_load_store_1d_vc4tile_launch(program, vc4_m2_dim3(1, 1, 1),
    ...
    for (uint32_t i = 0; i < VC4_CASE_WORDS; ++i) {
        ...
        if (i >= VC4_CASE_ACTIVE_OFFSET && i < VC4_CASE_ACTIVE_OFFSET + VC4_CASE_ACTIVE_WORDS)
            expected = input_value(i - VC4_CASE_ACTIVE_OFFSET);
    ...
    uint32_t launch_failures = tile_load_store_1d_vc4tile_runtime_launch_failures();
    uint32_t runtime_launches = tile_load_store_1d_vc4tile_runtime_launches();
    const char *status = (!failures && total_mismatches == 0 && sentinel_mismatches == 0 &&
                          launch_failures == 0 && runtime_launches == 1) ? "PASS" : "FAIL";
    printk("VC4_TEST_RESULT name=tile_load_store_1d_vc4tile status=%s total_mismatches=%u sentinel_mismatches=%u launch_failures=%u runtime_launches=%u checksum_actual=%u checksum_expected=%u\n",
