...
static int32_t a_value(uint32_t i, uint32_t k) {
    return positive_mod_i32((int32_t)(13u * i + 17u * k + 19u), 97) - 48;
}

static int32_t b_value(uint32_t k, uint32_t j) {
    return positive_mod_i32((int32_t)(11u * k + 23u) - (int32_t)(7u * j), 89) - 44;
}
...
void notmain(void) {
    ...
    for (uint32_t c = 0; c < VC4_CASES; ++c) {
        ...
        if (!failures) {
            failures += vc4_m2_check_rc(
                "cute_gemm_16x16x16_tiled_vc4tile_launch",
                cute_gemm_16x16x16_tiled_vc4tile_launch(
    ...
    uint32_t launch_failures = cute_gemm_16x16x16_tiled_vc4tile_runtime_launch_failures();
    uint32_t runtime_launches = cute_gemm_16x16x16_tiled_vc4tile_runtime_launches();
    ...
    printk("VC4_TEST_RESULT name=cute_gemm_16x16x16_tiled_vc4tile status=%s cases=%u total_mismatches=%u sentinel_mismatches=%u launch_failures=%u runtime_launches=%u checksum_actual=%u checksum_expected=%u sample_c00=%d sample_c015=%d sample_c57=%d sample_c83=%d sample_c150=%d sample_c1515=%d\n",
