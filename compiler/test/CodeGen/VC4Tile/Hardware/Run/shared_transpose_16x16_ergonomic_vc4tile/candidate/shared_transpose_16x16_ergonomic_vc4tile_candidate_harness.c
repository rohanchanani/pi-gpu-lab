...
#define VC4_WORDS 272u
#define VC4_ACTIVE_OFFSET 7u
#define VC4_SENTINEL 0x7474c7c7u

static uint32_t matrix_value(uint32_t row, uint32_t col) {
    return 0x71000000u + 257u * row + 23u * col + (row << 4);
}
...
void notmain(void) {
    ...
    if (!failures) {
        ...
        failures += vc4_m2_check_rc(
            "shared_transpose_16x16_ergonomic_vc4tile_launch",
            shared_transpose_16x16_ergonomic_vc4tile_launch(
    ...
    for (uint32_t i = 0; i < VC4_WORDS; ++i) {
        ...
        if (active) {
            ...
            uint32_t out_row = linear / VC4_COLS;
            uint32_t out_col = linear % VC4_COLS;
            expected = matrix_value(out_col, out_row);
        }
    ...
    uint32_t launch_failures = shared_transpose_16x16_ergonomic_vc4tile_runtime_launch_failures();
    uint32_t runtime_launches = shared_transpose_16x16_ergonomic_vc4tile_runtime_launches();
    const char *status = (!failures && total_mismatches == 0 && sentinel_mismatches == 0 &&
                          launch_failures == 0 && runtime_launches == 1) ? "PASS" : "FAIL";
    printk("VC4_TEST_RESULT name=shared_transpose_16x16_ergonomic_vc4tile status=%s total_mismatches=%u sentinel_mismatches=%u launch_failures=%u runtime_launches=%u checksum_actual=%u checksum_expected=%u\n",
