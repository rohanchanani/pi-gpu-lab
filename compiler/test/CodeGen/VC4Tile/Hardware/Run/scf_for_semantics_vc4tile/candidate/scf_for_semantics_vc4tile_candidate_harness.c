...
#define FIXTURE_SEMANTIC_TOTAL 76u
#define FIXTURE_ACTIVE_LANES 16u
#define FIXTURE_EXPECTED_CHECKSUM 1336u
...
void notmain(void) {
  ...
  if (vc4_program_create(&program, 0) < 0 || !program)
    panic("scf_for_semantics_vc4tile vc4_program_create failed");
  ...
  if (vc4_m2_malloc(program, &out_dev, bytes) < 0)
    panic("scf_for_semantics_vc4tile device allocation failed");
  ...
  printk("Running VC4 scf_for_semantics_vc4tile candidate bundle...\n");
  fill_host_buffer();
  int start = timer_get_usec();
  if (vc4_m2_copy_htod(program, out_dev, out_values, bytes) < 0 ||
      scf_for_semantics_vc4tile_launch(program, grid, block, out_dev) < 0 ||
      vc4_m2_copy_dtoh(program, out_values, out_dev, bytes) < 0) {
    printk("ERROR: scf_for_semantics_vc4tile launch/copy failed\n");
    launch_failures++;
  } else {
    for (unsigned lane = 0; lane < FIXTURE_ACTIVE_LANES; ++lane) {
      ...
      if (out_values[lane] != expected) {
        if (total_mismatches < 8) {
          printk("MISMATCH scf_for_semantics_vc4tile lane=%u observed=%x expected=%x\n",
    ...
    if (checksum_accum != FIXTURE_EXPECTED_CHECKSUM) {
      printk("MISMATCH scf_for_semantics_vc4tile checksum observed=%u expected=%u\n",
  ...
  const char *status =
      (total_mismatches == 0 && sentinel_mismatches == 0 &&
       launch_failures == 0)
          ? "PASS"
          : "FAIL";
  printk("VC4_TEST_RESULT name=scf_for_semantics_vc4tile status=%s cases=%d total_mismatches=%d sentinel_mismatches=%d launch_failures=%d active_qpus=%d lanes=%d active_lanes=%d semantic_total=%d checksum_accum=%u runtime_allocations=%u runtime_launches=%u elapsed_usec=%d\n",
         ...
         scf_for_semantics_vc4tile_runtime_allocations(), scf_for_semantics_vc4tile_runtime_launches(), elapsed);
