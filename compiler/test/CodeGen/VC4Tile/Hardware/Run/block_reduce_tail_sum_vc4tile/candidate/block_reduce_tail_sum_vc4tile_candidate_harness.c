...
static uint32_t x_value(uint32_t i) { return (i < VC4_ACTIVE_WORDS) ? (7u + 3u * i) : 0u; }
...
void notmain(void) {
    ...
    if (!failures) {
        failures += vc4_m2_check_rc(
            "block_reduce_tail_sum_vc4tile_launch",
            block_reduce_tail_sum_vc4tile_launch(
                program, vc4_m2_dim3(1, 1, 1), vc4_m2_dim3(16, 1, 1),
                d_out, d_in, VC4_ACTIVE_WORDS));
    }
    ...
    uint32_t launch_failures = block_reduce_tail_sum_vc4tile_runtime_launch_failures();
    uint32_t runtime_launches = block_reduce_tail_sum_vc4tile_runtime_launches();
    const char *status = (!failures && total_mismatches == 0 && sentinel_mismatches == 0 &&
                          launch_failures == 0 && runtime_launches == 1) ? "PASS" : "FAIL";
    printk("VC4_TEST_RESULT name=block_reduce_tail_sum_vc4tile status=%s total_mismatches=%u sentinel_mismatches=%u launch_failures=%u runtime_launches=%u checksum_actual=%u checksum_expected=%u\n",
