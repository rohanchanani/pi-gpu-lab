// RUN: bash %S/../../../Support/run_vc4tile_candidate_codegen_test.sh block_reduce_tail_sum_vc4tile generate

vc4tile.kernel @block_reduce_tail_sum_vc4tile(%out : i32, %in : i32, %n : i32) attributes {
  public_name = "block_reduce_tail_sum_vc4tile",
  arg_attrs = [
  ...
  %tile = "vc4tile.tile_load"(%in, %zero, %all) {shape = [1, 16], layout = #vc4tile.layout<row_major>, memory_space = #vc4tile.memory_space<global>, element_type = i32, storage_type = i32, precision = #vc4tile.precision<exact_32>, boundary = #vc4tile.boundary_policy<exact>, packing = #vc4tile.packing<none>} : (i32, i32, vector<16xi1>) -> vector<16xi32>
  %sum = "vc4tile.block_reduce"(%tile, %all) {shape = [1, 16], layout = #vc4tile.layout<row_major>, memory_space = #vc4tile.memory_space<register>, element_type = i32, storage_type = i32, precision = #vc4tile.precision<exact_32>, packing = #vc4tile.packing<none>, kind = #vc4tile.reduce_kind<add>, axis = 1 : i32, algorithm_hint = "shared_vpm_barrier"} : (vector<16xi32>, vector<16xi1>) -> vector<16xi32>
  "vc4tile.tile_store"(%sum, %out, %zero, %tail) {shape = [1, 16], layout = #vc4tile.layout<row_major>, memory_space = #vc4tile.memory_space<global>, element_type = i32, storage_type = i32, precision = #vc4tile.precision<exact_32>, boundary = #vc4tile.boundary_policy<tail_predicated>, packing = #vc4tile.packing<none>} : (vector<16xi32>, i32, i32, vector<16xi1>) -> ()
  vc4tile.return
}
