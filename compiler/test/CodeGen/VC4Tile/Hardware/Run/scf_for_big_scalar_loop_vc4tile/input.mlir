...
// RUN: bash %S/../../../Support/run_vc4tile_candidate_codegen_test.sh scf_for_big_scalar_loop_vc4tile clean
// RUN: bash %S/../../../Support/run_vc4tile_candidate_codegen_test.sh scf_for_big_scalar_loop_vc4tile generate
// RUN: bash -c 'd="$(bash %S/../../../Support/run_vc4tile_candidate_codegen_test.sh scf_for_big_scalar_loop_vc4tile generated-dir)"; FileCheck --check-prefix=CORE %s < "$d/core.vc4tile.mlir"'

// RAW: legalize-vc4tile-core-cfg
// SSAVC4: ssavc4.func @scf_for_big_scalar_loop_vc4tile
...
vc4tile.kernel @scf_for_big_scalar_loop_vc4tile attributes {
  public_name = "scf_for_big_scalar_loop_vc4tile",
  arg_attrs = [
...
^entry(%out: i32, %loop_n: i32):
  ...
  %logical_lanes = arith.addi %pid_lanes_vec, %lanes : vector<16xi32>

  %lb = arith.constant 0 : index
  %ub = arith.index_cast %loop_n : i32 to index
  %step = arith.constant 1 : index
  %value = scf.for %i = %lb to %ub step %step iter_args(%acc = %logical_lanes) -> (vector<16xi32>) {
    %ii = arith.index_cast %i : index to i32
    %term = vector.broadcast %ii : i32 to vector<16xi32>
    %next = arith.addi %acc, %term : vector<16xi32>
