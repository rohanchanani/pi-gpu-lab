...
#define VC4_SENTINEL 0x71710a03u
...
void notmain(void) {
    ...
    if (!failures) {
        failures += vc4_m2_check_rc(
            "block_reduce_sum_ergonomic_vc4tile_launch",
            block_reduce_sum_ergonomic_vc4tile_launch(
    ...
    uint32_t launch_failures = block_reduce_sum_ergonomic_vc4tile_runtime_launch_failures();
    uint32_t runtime_launches = block_reduce_sum_ergonomic_vc4tile_runtime_launches();
    const char *status = (!failures && total_mismatches == 0 && sentinel_mismatches == 0 &&
                          launch_failures == 0 && runtime_launches == 1) ? "PASS" : "FAIL";
    printk("VC4_TEST_RESULT name=block_reduce_sum_ergonomic_vc4tile status=%s total_mismatches=%u sentinel_mismatches=%u launch_failures=%u runtime_launches=%u checksum_actual=%u checksum_expected=%u\n",
