...
#define VC4_SENTINEL 0x6d51a7e1u

static const uint32_t k_rows[VC4_CASES] = {0u, 1u, 2u, 3u, 4u, 4u};
static const uint32_t k_cols[VC4_CASES] = {4u, 2u, 3u, 1u, 4u, 1u};
static const uint32_t k_tail[VC4_CASES] = {0u, 4u, 5u, 13u, 16u, 9u};

static uint32_t source_word(uint32_t which, uint32_t lane) {
    return 1000u + which * 97u + (lane / 4u) * 11u + (lane % 4u) * 3u;
}

static int active_lane(uint32_t which, uint32_t lane) {
    return (lane / 4u) < k_rows[which] &&
           (lane % 4u) < k_cols[which] &&
           lane < k_tail[which];
}
...
static uint32_t expected_word(uint32_t index) {
    ...
    if (active_lane(which, lane))
        return source_word(which, lane);
    return VC4_SENTINEL;
}
...
void notmain(void) {
    ...
    for (uint32_t c = 0; c < VC4_CASES; ++c) {
        for (uint32_t lane = 0; lane < VC4_TILE_WORDS; ++lane)
            host_in[c * VC4_TILE_WORDS + lane] = source_word(c, lane);
    }
    ...
    for (uint32_t c = 0; !failures && c < VC4_CASES; ++c) {
        ...
        failures += vc4_m2_check_rc(
            "tile_bounds_tail_shared_roundtrip_vc4tile_launch",
            tile_bounds_tail_shared_roundtrip_vc4tile_launch(
        ...
        if (got[i] != expected) {
            if (expected == VC4_SENTINEL) {
                ...
                if (total_mismatches < 12u) {
                    ...
                    uint32_t which = logical / VC4_TILE_WORDS;
                    uint32_t lane = logical % VC4_TILE_WORDS;
                    printk("mismatch case=%u lane=%u got=%u expected=%u\n",
                           which, lane, got[i], expected);
                }
                ++total_mismatches;
            }
        }
    ...
    uint32_t launch_failures =
        tile_bounds_tail_shared_roundtrip_vc4tile_runtime_launch_failures();
    uint32_t runtime_launches =
        tile_bounds_tail_shared_roundtrip_vc4tile_runtime_launches();
    const char *status =
        (!failures && total_mismatches == 0 && sentinel_mismatches == 0 &&
         launch_failures == 0 && runtime_launches == VC4_CASES)
            ? "PASS"
            : "FAIL";
    printk("VC4_TEST_RESULT name=tile_bounds_tail_shared_roundtrip_vc4tile status=%s cases=%u composed_shared_predicate_path=1 total_mismatches=%u sentinel_mismatches=%u launch_failures=%u runtime_launches=%u checksum_actual=%u checksum_expected=%u sample_empty_lane0=%u sample_case2_lane4=%u sample_case3_lane12=%u sample_case4_lane15=%u sample_case5_lane8=%u\n",
