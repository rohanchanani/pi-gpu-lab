// RUN: bash %S/../../../Support/run_vc4tile_candidate_codegen_test.sh tile_select_mask_algebra_vc4tile generate

vc4tile.kernel @tile_select_mask_algebra_vc4tile(%out : i32, %rows : i32, %cols : i32, %n : i32) attributes {
  public_name = "tile_select_mask_algebra_vc4tile",
  arg_attrs = [
  ...
  %tail_outside = vc4tile.mask_and %not_bounds, %tail
  %selected_union = "vc4tile.tile_select"(%union, %values, %fallback) {shape = [4, 4], layout = #vc4tile.layout<row_major>, memory_space = #vc4tile.memory_space<register>, element_type = i32, storage_type = i32, precision = #vc4tile.precision<exact_32>, packing = #vc4tile.packing<none>} : (!vc4tile.predicate, vector<16xi32>, vector<16xi32>) -> vector<16xi32>
  ...
  "vc4tile.tile_store"(%selected_union, %out, %zero, %all) {shape = [1, 16], dst_layout = #vc4tile.layout<row_major>, memory_space = #vc4tile.memory_space<global>, element_type = i32, storage_type = i32, precision = #vc4tile.precision<exact_32>, boundary = #vc4tile.boundary_policy<exact>, packing = #vc4tile.packing<none>, role = #vc4tile.role<output>} : (vector<16xi32>, i32, i32, !vc4tile.predicate) -> ()
  "vc4tile.tile_store"(%selected_tail_outside, %out, %sixteen, %all) {shape = [1, 16], dst_layout = #vc4tile.layout<row_major>, memory_space = #vc4tile.memory_space<global>, element_type = i32, storage_type = i32, precision = #vc4tile.precision<exact_32>, boundary = #vc4tile.boundary_policy<exact>, packing = #vc4tile.packing<none>, role = #vc4tile.role<output>} : (vector<16xi32>, i32, i32, !vc4tile.predicate) -> ()
  vc4tile.return
}
