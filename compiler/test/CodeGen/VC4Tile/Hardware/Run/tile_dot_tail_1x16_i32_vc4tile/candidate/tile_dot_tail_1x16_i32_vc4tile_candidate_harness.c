...
#define VC4_SENTINEL 0x6d0d7001u

static const uint32_t k_active[VC4_CASES] = {0u, 1u, 3u, 8u, 11u, 16u};
...
static uint32_t dot_expected(uint32_t active_lanes) {
    uint32_t sum = 0;
...
static uint32_t expected_word(uint32_t index) {
    ...
    return dot_expected(k_active[case_id]);
}
...
void notmain(void) {
    ...
    for (uint32_t c = 0; !failures && c < VC4_CASES; ++c) {
        ...
        failures += vc4_m2_check_rc(
            "tile_dot_tail_1x16_i32_vc4tile_launch",
            tile_dot_tail_1x16_i32_vc4tile_launch(
                program, vc4_m2_dim3(1, 1, 1), vc4_m2_dim3(16, 1, 1),
                out_active, d_lhs, d_rhs, k_active[c]));
    }
    ...
    uint32_t launch_failures =
        tile_dot_tail_1x16_i32_vc4tile_runtime_launch_failures();
    uint32_t runtime_launches =
        tile_dot_tail_1x16_i32_vc4tile_runtime_launches();
    const char *status = (!failures && total_mismatches == 0 &&
                          sentinel_mismatches == 0 && launch_failures == 0 &&
                          runtime_launches == VC4_CASES) ? "PASS" : "FAIL";
    printk("VC4_TEST_RESULT name=tile_dot_tail_1x16_i32_vc4tile status=%s cases=%u predicated_dot_path=1 total_mismatches=%u sentinel_mismatches=%u launch_failures=%u runtime_launches=%u checksum_actual=%u checksum_expected=%u sample_n0_lane0=%u sample_n8_lane7=%u sample_n16_lane15=%u\n",
