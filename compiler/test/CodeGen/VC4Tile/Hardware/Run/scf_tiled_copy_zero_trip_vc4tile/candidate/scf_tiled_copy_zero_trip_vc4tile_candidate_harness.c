...
#define VC4_SENTINEL 0x71710002u
...
void notmain(void) {
    ...
    if (!failures) {
        ...
        failures += vc4_m2_check_rc(
            "scf_tiled_copy_zero_trip_vc4tile_launch",
            scf_tiled_copy_zero_trip_vc4tile_launch(program, vc4_m2_dim3(1, 1, 1), vc4_m2_dim3(16, 1, 1), out_active, in_active));
    }
    ...
    uint32_t launch_failures = scf_tiled_copy_zero_trip_vc4tile_runtime_launch_failures();
    uint32_t runtime_launches = scf_tiled_copy_zero_trip_vc4tile_runtime_launches();
    const char *status = (!failures && total_mismatches == 0 && sentinel_mismatches == 0 &&
                          launch_failures == 0 && runtime_launches == 1) ? "PASS" : "FAIL";
    printk("VC4_TEST_RESULT name=scf_tiled_copy_zero_trip_vc4tile status=%s total_mismatches=%u sentinel_mismatches=%u launch_failures=%u runtime_launches=%u checksum_actual=%u checksum_expected=%u\n",
