...
#define VC4_SENTINEL 0x57c0de05u
...
static uint32_t expected_word(uint32_t index) {
    ...
    if (!in_bounds(lane, k_rows[case_id], k_cols[case_id]) &&
        lane < k_tail[case_id])
        return lane;
    return VC4_SENTINEL;
}
...
void notmain(void) {
    ...
    for (uint32_t c = 0; !failures && c < VC4_CASES; ++c) {
        ...
        failures += vc4_m2_check_rc(
            "tile_store_composed_mask_vc4tile_launch",
            tile_store_composed_mask_vc4tile_launch(
    ...
    uint32_t launch_failures =
        tile_store_composed_mask_vc4tile_runtime_launch_failures();
    uint32_t runtime_launches =
        tile_store_composed_mask_vc4tile_runtime_launches();
    const char *status = (!failures && total_mismatches == 0 &&
                          sentinel_mismatches == 0 && launch_failures == 0 &&
                          runtime_launches == VC4_CASES) ? "PASS" : "FAIL";
    printk("VC4_TEST_RESULT name=tile_store_composed_mask_vc4tile status=%s cases=%u composed_store_predicate_path=1 total_mismatches=%u sentinel_mismatches=%u launch_failures=%u runtime_launches=%u checksum_actual=%u checksum_expected=%u sample_empty_lane0=%u sample_hole_lane3=%u sample_tail_lane12=%u sample_inactive_lane8=%u\n",
