...
// RUN: vc4-opt %s --vc4-verify-value-surface --convert-vc4-value-to-vc4kernel -o %t/value_attention_apply_v0_scaled.vc4kernel.mlir
// RUN: FileCheck %s --check-prefix=VC4KERNEL --input-file=%t/value_attention_apply_v0_scaled.vc4kernel.mlir
// RUN: vc4-opt %t/value_attention_apply_v0_scaled.vc4kernel.mlir --verify-vc4kernel --convert-vc4kernel-to-ssavc4 -o %t/value_attention_apply_v0_scaled.ssavc4.mlir
// RUN: vc4-opt %t/value_attention_apply_v0_scaled.ssavc4.mlir --convert-ssavc4-to-vc4 --vc4-verify-emit-contract --vc4-verify-scheduled-hardware-rules --vc4-verify-scheduled-adjacent-hazards --vc4-verify-scheduled-io-spacing --vc4-verify-scheduled-peripheral-accesses -o %t/value_attention_apply_v0_scaled.vc4.mlir
// RUN: FileCheck %s --check-prefix=VC4 --input-file=%t/value_attention_apply_v0_scaled.vc4.mlir

func.func @value_attention_apply_v0_scaled_f32_b16_vc4value(
  ...
  %scaled_scores = arith.mulf %scores_v, %scale_v : vector<16xf32>
  %low = arith.constant dense<-8.000000e+01> : vector<16xf32>
  %zeros = arith.constant dense<0.000000e+00> : vector<16xf32>
  %active_scores = arith.select %mask, %scaled_scores, %low : vector<16xi1>, vector<16xf32>
...
// VC4KERNEL-LABEL: vc4kernel.kernel @value_attention_apply_v0_scaled_f32_b16_vc4value
// VC4KERNEL: vc4kernel.splat %{{.*}} : f32 -> vector<16xf32>
