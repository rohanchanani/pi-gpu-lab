...
// RUN: vc4-opt %s --vc4-verify-value-surface --convert-vc4-value-to-vc4kernel -o %t/mixed_f16_gemv.vc4kernel.mlir
// RUN: FileCheck %s --check-prefix=VC4KERNEL --input-file=%t/mixed_f16_gemv.vc4kernel.mlir
// RUN: vc4-opt %t/mixed_f16_gemv.vc4kernel.mlir --verify-vc4kernel --convert-vc4kernel-to-ssavc4 -o %t/mixed_f16_gemv.ssavc4.mlir
// RUN: FileCheck %s --check-prefix=SSAVC4 --input-file=%t/mixed_f16_gemv.ssavc4.mlir
// RUN: vc4-opt %t/mixed_f16_gemv.ssavc4.mlir --convert-ssavc4-to-vc4 --vc4-verify-emit-contract --vc4-verify-scheduled-hardware-rules --vc4-verify-scheduled-adjacent-hazards --vc4-verify-scheduled-io-spacing --vc4-verify-scheduled-peripheral-accesses -o %t/mixed_f16_gemv.vc4.mlir
// RUN: FileCheck %s --check-prefix=VC4 --input-file=%t/mixed_f16_gemv.vc4.mlir
...
func.func @mixed_value_f16_storage_gemv_axes_mask_cf_reduction_vc4value(
  ...
  %av_h = vector.transfer_read %a[%row, %col], %zero_h, %mask {in_bounds = [true]} : memref<?x?xf16, strided<[?, 1], offset: 0>, #vc4value.global>, vector<16xf16>
  %xv_h = vector.transfer_read %x[%col], %zero_h, %mask {in_bounds = [true]} : memref<?xf16, #vc4value.global>, vector<16xf16>
...
// VC4KERNEL-LABEL: vc4kernel.kernel @mixed_value_f16_storage_gemv_axes_mask_cf_reduction_vc4value
...
// SSAVC4-LABEL: ssavc4.func @mixed_value_f16_storage_gemv_axes_mask_cf_reduction_vc4value
// SSAVC4: program_id_x
// SSAVC4: program_id_y
