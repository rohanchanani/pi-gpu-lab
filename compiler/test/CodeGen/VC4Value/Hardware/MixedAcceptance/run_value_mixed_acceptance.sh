...
manifest="${script_dir}/mixed_value_acceptance_manifest.json"
repo_root="$(cd "${script_dir}/../../../../../../" && pwd)"
state_root="${VC4_CODEGEN_STATE_ROOT:-.vc4_auto/codegen_vc4value_mixed}"
list_only=0
fixtures=()

usage() {
  cat <<'EOF'
usage: run_value_mixed_acceptance.sh [--manifest PATH] [--repo-root PATH]
...
if [[ "${list_only}" -eq 1 ]]; then
  if [[ -z "${entries}" ]]; then
    echo "no VC4Value mixed acceptance fixtures selected"
    exit 0
  fi
...
while IFS=$'\t' read -r name status runner path dialect; do
  ...
  if [[ "${status}" != "implemented" ]]; then
    echo "unsupported VC4Value mixed acceptance fixture status for ${name}: ${status}" >&2
    exit 1
  fi
  if [[ "${runner}" != "vc4value_candidate" ]]; then
    echo "unsupported runner_kind for ${name}: ${runner}" >&2
    exit 1
  fi
  if [[ "${dialect}" != "vc4value" ]]; then
  ...
  if [[ ! -d "${fixture_dir}" ]]; then
    echo "missing VC4Value mixed acceptance fixture directory: ${fixture_dir}" >&2
    exit 1
  fi

  log_path="${state_root}/logs/${name}.log"
  echo "RUN VC4Value mixed acceptance fixture: ${name}"
  VC4_CODEGEN_STATE_ROOT="${state_root}" \
    "${repo_root}/compiler/test/CodeGen/VC4Value/Support/run_vc4value_candidate_codegen_test.sh" \
  ...
  if [[ "${result_line}" != *"status=PASS"* ]]; then
    echo "VC4Value mixed acceptance fixture failed: ${name}" >&2
    exit 1
  fi
  for zero_field in total_mismatches sentinel_mismatches launch_failures; do
    if [[ "${result_line}" == *"${zero_field}="* && "${result_line}" != *"${zero_field}=0"* ]]; then
      echo "VC4Value mixed acceptance fixture has nonzero ${zero_field}: ${name}" >&2
      exit 1
    fi
  done
done <<< "${entries}"
