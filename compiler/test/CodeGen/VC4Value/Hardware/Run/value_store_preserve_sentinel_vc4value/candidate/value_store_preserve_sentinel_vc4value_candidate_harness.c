...
#define SENTINEL 0x8badf00du

static const uint32_t n_cases[] = {0u, 1u, 15u, 16u, 17u, 31u, 32u, 33u};
...
static int verify_results(uint32_t n) {
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (out_values[index] != expected) {
            if (mismatches < 8)
                printk("ERROR: value_store_preserve active n=%d i=%d got=%x expected=%x\n",
...
static int verify_sentinels(uint32_t n) {
    ...
    for (uint32_t i = 0; i < BUFFER_N; i++) {
        ...
        if ((uint32_t)out_values[i] != SENTINEL) {
            if (mismatches < 8)
                printk("ERROR: value_store_preserve sentinel n=%d i=%d got=%x expected=%x\n",
...
void notmain(void) {
    ...
    if (vc4_m2_malloc(program, &out_dev, bytes) < 0)
        panic("value_store_preserve allocation failed");
    ...
    for (uint32_t case_id = 0; case_id < sizeof(n_cases) / sizeof(n_cases[0]); case_id++) {
        ...
        if (vc4_m2_copy_htod(program, out_dev, out_values, bytes) < 0 ||
            value_store_preserve_sentinel_vc4value_launch(program, grid, block, out_active, n) < 0 ||
            vc4_m2_copy_dtoh(program, out_values, out_dev, bytes) < 0) {
            printk("ERROR: value_store_preserve launch/copy failed n=%d\n", (int)n);
            launch_failures++;
            continue;
        }
        ...
        printk("VALUE_STORE_PRESERVE_CASE case=%d n=%d waves=%d mismatches=%d sentinel_mismatches=%d hash=%x\n",
    ...
    printk("VC4_TEST_RESULT name=value_store_preserve_sentinel_vc4value status=%s cases=%d elements_checked=%d total_mismatches=%d sentinel_mismatches=%d launch_failures=%d active_qpus=%d lanes=%d max_n=%d max_coverage_n=%d buffer_n=%d output_hash=%u runtime_allocations=%d runtime_launches=%d elapsed_usec=%d\n",
