...
#define A_SENTINEL_BITS 0xc6510000u
#define Y_SENTINEL_BITS 0xc6810000u
#define F32_TOLERANCE 0.001f

struct case_desc { uint32_t rows, k, lda; };
static const struct case_desc cases[] = {
    {1u, 0u, 5u}, {1u, 1u, 5u}, {2u, 2u, 8u}, {2u, 15u, 22u},
    {7u, 16u, 31u}, {7u, 1u, 19u}, {7u, 15u, 23u}
};
...
static float a_value(uint32_t case_id, uint32_t row, uint32_t col) {
    int32_t whole = (int32_t)((case_id * 11u + row * 17u + col * 5u) % 47u) - 23;
    float value = (float)whole * 0.125f + (float)((row + col + case_id) & 3u) * 0.03125f;
    if (((row + col + case_id) & 1u) != 0u) value = -value;
    return value;
}

static float x_value(uint32_t case_id, uint32_t col) {
    int32_t whole = (int32_t)((case_id * 7u + col * 13u) % 31u) - 15;
...
static int verify_rows(uint32_t case_id, uint32_t rows, uint32_t k, float *max_abs_diff) {
    ...
    for (uint32_t r = 0; r < rows; r++) {
        ...
        if (diff > F32_TOLERANCE) {
            if (mismatches < 8)
                printk("ERROR: f32 gemv row case=%d row=%d k=%d got=%f expected=%f diff=%f\n",
...
static int verify_sentinels(uint32_t rows, uint32_t k, uint32_t lda) {
    ...
    for (uint32_t i = 0; i < Y_BUFFER_N; i++) {
        ...
        if (float_to_bits(y_values[i]) != Y_SENTINEL_BITS) {
            if (mismatches < 8) printk("ERROR: f32 gemv y sentinel i=%d bits=%x\n", (int)i, float_to_bits(y_values[i]));
            mismatches++;
        }
    }
    for (uint32_t r = 0; r < rows; r++) {
        for (uint32_t c = k; c < lda; c++) {
            ...
            if (float_to_bits(a_values[idx]) != A_SENTINEL_BITS) {
                if (mismatches < 8) printk("ERROR: f32 gemv A padding changed row=%d col=%d bits=%x\n", (int)r, (int)c, float_to_bits(a_values[idx]));
                mismatches++;
            }
        }
    }
...
void notmain(void) {
    ...
    if (vc4_program_create(&program, 0) < 0 || !program) panic("value_gemv_f32 create failed");
    vc4_deviceptr_t a_dev = 0, x_dev = 0, y_dev = 0;
    uint32_t a_bytes = A_BUFFER_N * sizeof(float), x_bytes = X_BUFFER_N * sizeof(float), y_bytes = Y_BUFFER_N * sizeof(float);
    if (vc4_m2_malloc(program, &a_dev, a_bytes) < 0 || vc4_m2_malloc(program, &x_dev, x_bytes) < 0 ||
        vc4_m2_malloc(program, &y_dev, y_bytes) < 0) panic("value_gemv_f32 allocation failed");
    ...
    for (uint32_t case_id = 0; case_id < sizeof(cases) / sizeof(cases[0]); case_id++) {
        uint32_t rows = cases[case_id].rows, k = cases[case_id].k, lda = cases[case_id].lda;
        ...
        if (vc4_m2_copy_htod(program, a_dev, a_values, a_bytes) < 0 ||
            ...
            value_gemv_row_dot_f32_tail_vc4value_launch(program, grid, block, a_active, x_active, y_active, rows, k, lda) < 0 ||
            vc4_m2_copy_dtoh(program, y_values, y_dev, y_bytes) < 0 ||
            vc4_m2_copy_dtoh(program, a_values, a_dev, a_bytes) < 0) {
            printk("ERROR: f32 gemv launch/copy failed case=%d rows=%d k=%d lda=%d\n", (int)case_id, (int)rows, (int)k, (int)lda);
            launch_failures++;
            continue;
        }
        ...
        total_mismatches += mismatches; sentinel_mismatches += sentinels;
        output_hash ^= hash + 0x9e3779b9u + (case_id << 6) + (case_id >> 2);
        output_hash = rotl32_local(output_hash, 7u);
        printk("VALUE_GEMV_F32_CASE case=%d rows=%d k=%d lda=%d mismatches=%d sentinel_mismatches=%d hash=%x max_abs_diff=%f\n",
    ...
    printk("VC4_TEST_RESULT name=value_gemv_row_dot_f32_tail_vc4value status=%s cases=%d total_mismatches=%d sentinel_mismatches=%d launch_failures=%d active_qpus=%d lanes=%d max_rows=%d max_k=%d max_lda=%d output_hash=%u output_hash_nonzero=%d max_abs_diff=%f saw_value_gemv_f32_row_dot=1 saw_f32_dot_finite_tree_policy=1 saw_value_scalar_dot_store=1 saw_tail_inactive_zero_dot_inputs=1 saw_value_row_strided_A=1 saw_contiguous_x_vector=1 runtime_allocations=%d runtime_launches=%d elapsed_usec=%d\n",
    ...
    vc4Free(program, a_dev); vc4Free(program, x_dev); vc4Free(program, y_dev); vc4_program_destroy(program);
}
