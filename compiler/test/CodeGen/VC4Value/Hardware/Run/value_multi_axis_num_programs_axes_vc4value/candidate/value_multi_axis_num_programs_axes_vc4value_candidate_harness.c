...
static int32_t expected_value(uint32_t grid_x, uint32_t grid_y, uint32_t grid_z, uint32_t lane) {
    return (int32_t)(grid_x * 10000u + grid_y * 100u + grid_z + lane);
}
...
static int verify_results(uint32_t grid_x, uint32_t grid_y, uint32_t grid_z) {
    ...
    for (uint32_t i = 0; i < elements; i++) {
        ...
        if (out_values[index] != expected) {
            if (mismatches < 8)
                printk("ERROR: num_programs grid=%d,%d,%d i=%d lane=%d got=%x expected=%x\n",
...
static int verify_sentinels(uint32_t elements) {
    ...
    for (uint32_t i = 0; i < BUFFER_N; i++) {
        ...
        if ((uint32_t)out_values[i] != SENTINEL) {
            if (mismatches < 8)
                printk("ERROR: num_programs sentinel elements=%d i=%d got=%x expected=%x\n",
...
void notmain(void) {
    ...
    if (vc4_m2_malloc(program, &out_dev, bytes) < 0)
        panic("num_programs allocation failed");
    ...
    for (uint32_t case_id = 0; case_id < sizeof(cases) / sizeof(cases[0]); case_id++) {
        ...
        if (vc4_m2_copy_htod(program, out_dev, out_values, bytes) < 0 ||
            value_multi_axis_num_programs_axes_vc4value_launch(program, grid, block, out_active, elements) < 0 ||
            vc4_m2_copy_dtoh(program, out_values, out_dev, bytes) < 0) {
            printk("ERROR: num_programs launch/copy failed case=%d grid=%d,%d,%d\n",
        ...
        printk("VALUE_MULTI_AXIS_NUM_PROGRAMS_CASE case=%d grid_x=%d grid_y=%d grid_z=%d elements=%d mismatches=%d sentinel_mismatches=%d hash=%x\n",
               (int)case_id, (int)grid_x, (int)grid_y, (int)grid_z,
    ...
    printk("VC4_TEST_RESULT name=value_multi_axis_num_programs_axes_vc4value status=%s cases=%d elements_checked=%d total_mismatches=%d sentinel_mismatches=%d launch_failures=%d active_qpus=%d lanes=%d max_grid_blocks=%d max_elements=%d buffer_n=%d output_hash=%u saw_value_multi_axis_launch=1 saw_value_program_id_axis0=1 saw_value_program_id_axis1=1 saw_value_program_id_axis2=1 saw_value_num_programs_axis0=1 saw_value_num_programs_axis1=1 saw_value_num_programs_axis2=1 runtime_allocations=%d runtime_launches=%d elapsed_usec=%d\n",
