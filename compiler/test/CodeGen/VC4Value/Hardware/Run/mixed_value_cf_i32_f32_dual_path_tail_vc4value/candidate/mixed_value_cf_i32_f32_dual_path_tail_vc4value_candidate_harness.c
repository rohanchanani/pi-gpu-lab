...
static void fill_buffers(uint32_t n, float a, uint32_t trip, int32_t use_i32_path) {
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (use_i32_path) {
            ...
            expected_values[index] = factor * yf_values[index] + xf_values[index];
        }
    }
}
...
static int verify_results(uint32_t n, float *max_abs_diff) {
    ...
    *max_abs_diff = 0.0f;
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (ad > EPSILON) {
            if (mismatches < 8)
                printk("ERROR: mixed_cf_i32_f32 n=%d i=%d xi=%d gpu=%f cpu=%f diff=%f\n",
...
static int verify_sentinels(uint32_t n) {
    ...
    for (uint32_t i = 0; i < BUFFER_N; i++) {
        ...
        if (got != SENTINEL_BITS) {
            if (mismatches < 8)
                printk("ERROR: mixed_cf_i32_f32 sentinel n=%d i=%d got=%x expected=%x\n",
...
void notmain(void) {
    ...
    if (vc4_m2_malloc(program, &xi_dev, bytes_i) < 0 ||
        ...
        panic("mixed_cf_i32_f32 allocation failed");
        ...
        for (uint32_t trip_id = 0; trip_id < sizeof(trip_cases) / sizeof(trip_cases[0]); trip_id++) {
            for (uint32_t flag_id = 0; flag_id < sizeof(flag_cases) / sizeof(flag_cases[0]); flag_id++) {
                for (uint32_t n_id = 0; n_id < sizeof(n_cases) / sizeof(n_cases[0]); n_id++) {
                    ...
                    fill_buffers(n, a, trip, use_i32_path);
                    ...
                    if (vc4_m2_copy_htod(program, xi_dev, xi_values, bytes_i) < 0 ||
                        ...
                        mixed_value_cf_i32_f32_dual_path_tail_vc4value_launch(
                            program, grid, block, xi_active, xf_active, yf_active,
                            out_active, THRESHOLD_I, a, trip, use_i32_path, n) < 0 ||
                        vc4_m2_copy_dtoh(program, out_values, out_dev, bytes_f) < 0) {
                        printk("ERROR: mixed_cf_i32_f32 launch/copy failed a_id=%d trip=%d flag=%d n=%d\n",
                    ...
                    case_count++;
                    printk("MIXED_VALUE_CF_I32_F32_CASE a_id=%d trip=%d flag=%d n=%d waves=%d mismatches=%d sentinel_mismatches=%d hash=%x max_abs_diff=%f\n",
                           (int)a_id, (int)trip, (int)use_i32_path, (int)n, (int)waves,
    ...
    printk("VC4_TEST_RESULT name=mixed_value_cf_i32_f32_dual_path_tail_vc4value status=%s cases=%d elements_checked=%d total_mismatches=%d sentinel_mismatches=%d launch_failures=%d active_qpus=%d lanes=%d max_n=%d max_coverage_n=%d buffer_n=%d output_hash=%u max_abs_diff=%f saw_value_cf_loop=1 saw_value_cf_cond_br=1 saw_value_vector_block_arg=1 saw_value_tail_mask=1 saw_value_tmu_load=1 saw_value_vdw_preserve=1 saw_value_mixed_i32_f32=1 saw_value_i32_cmp=1 saw_value_f32_alu=1 runtime_allocations=%d runtime_launches=%d elapsed_usec=%d\n",
