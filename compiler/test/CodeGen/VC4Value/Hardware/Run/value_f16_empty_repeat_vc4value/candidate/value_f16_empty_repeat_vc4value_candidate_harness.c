...
static int verify_output(uint32_t case_id, uint32_t n, float *max_abs_diff) {
    int mismatches = 0;
    for (uint32_t i = 0; i < n; i++) {
        float expected = f16_to_f32(input_half(case_id, i));
        float got = out_values[GUARD + i];
        float diff = absf_local(got - expected);
        if (diff > *max_abs_diff) *max_abs_diff = diff;
        if (diff > F32_TOLERANCE) {
            if (mismatches < 8)
                printk("ERROR: f16 repeat output case=%d i=%d got=%f expected=%f diff=%f\n",
...
static int verify_sentinels(uint32_t n) {
    ...
    for (uint32_t i = 0; i < BUFFER_N; i++) {
        ...
        if (float_to_bits(out_values[i]) != OUT_SENTINEL_BITS) {
            if (mismatches < 8) printk("ERROR: f16 repeat sentinel i=%d bits=%x\n", (int)i, float_to_bits(out_values[i]));
            mismatches++;
        }
    }
    return mismatches;
...
void notmain(void) {
    ...
    if (vc4_program_create(&program, 0) < 0 || !program) panic("value_f16_repeat create failed");
    ...
    if (vc4_m2_malloc(program, &in_dev, in_bytes) < 0 ||
        ...
        panic("value_f16_repeat allocation failed");
    ...
    for (uint32_t case_id = 0; case_id < sizeof(cases) / sizeof(cases[0]); case_id++) {
        ...
        vc4_dim3 grid = vc4_m2_dim3(n == 0u ? 1u : (n + ELEMENTS_PER_WAVE - 1u) / ELEMENTS_PER_WAVE, 1u, 1u);
        ...
        if (vc4_m2_copy_htod(program, in_dev, in_values, in_bytes) < 0 ||
            ...
            value_f16_empty_repeat_vc4value_launch(program, grid, block, in_active, out_active, n) < 0 ||
            vc4_m2_copy_dtoh(program, out_values, out_dev, out_bytes) < 0) {
            printk("ERROR: f16 repeat launch/copy failed case=%d n=%d\n", (int)case_id, (int)n);
            launch_failures++;
            continue;
        }
        ...
        printk("VALUE_F16_REPEAT_CASE case=%d n=%d mismatches=%d sentinel_mismatches=%d hash=%x max_abs_diff=%f\n",
    ...
    printk("VC4_TEST_RESULT name=value_f16_empty_repeat_vc4value status=%s cases=%d elements_checked=%d total_mismatches=%d sentinel_mismatches=%d launch_failures=%d active_qpus=%d lanes=%d max_n=%d max_coverage_n=%d buffer_n=%d output_hash=%u output_hash_nonzero=%d max_abs_diff=%f saw_repeat_invocation=1 runtime_allocations=%d runtime_launches=%d elapsed_usec=%d\n",
           status, (int)(sizeof(cases) / sizeof(cases[0])), elements_checked, total_mismatches, sentinel_mismatches, launch_failures,
           ACTIVE_QPUS, LANES, MAX_N, MAX_COVERAGE_N, BUFFER_N, output_hash, output_hash != 0u ? 1 : 0, max_abs_diff, 2,
