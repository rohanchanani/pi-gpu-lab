...
static const struct case_config cases[] = {
    {0u, 0u, 0u, 1u}, {1u, 0u, 1u, 1u}, {2u, 1u, 5u, 2u},
    {3u, 2u, 11u, 3u}, {5u, 5u, 5u, 1u}, {17u, 0u, 16u, 4u},
    {193u, 3u, 25u, 5u}, {1000u, 7u, 43u, 6u},
};
...
static int32_t expected_acc(uint32_t start, uint32_t end, uint32_t step) {
    int32_t acc = 0;
    for (uint32_t tile = start; tile < end; tile += step)
        acc += (int32_t)(tile + 1u);
    return acc;
}
...
static int verify_results(uint32_t n, uint32_t start, uint32_t end, uint32_t step) {
    int mismatches = 0;
    int32_t expected = expected_acc(start, end, step);
    for (uint32_t i = 0; i < n; i++) {
        uint32_t index = GUARD + i;
        if (out_values[index] != expected) {
            if (mismatches < 8)
                printk("ERROR: tl_range_style n=%d start=%d end=%d step=%d i=%d got=%x expected=%x\n",
                       (int)n, (int)start, (int)end, (int)step, (int)i,
                       (uint32_t)out_values[index], (uint32_t)expected);
            mismatches++;
        }
    }
    return mismatches;
...
static int verify_sentinels(uint32_t n) {
    ...
    for (uint32_t i = 0; i < BUFFER_N; i++) {
        ...
        if ((uint32_t)out_values[i] != SENTINEL) {
            if (mismatches < 8)
                printk("ERROR: tl_range_style sentinel n=%d i=%d got=%x expected=%x\n",
...
void notmain(void) {
    ...
    if (vc4_m2_malloc(program, &out_dev, bytes) < 0)
        panic("tl_range_style allocation failed");
    ...
    for (uint32_t case_id = 0; case_id < sizeof(cases) / sizeof(cases[0]); case_id++) {
        ...
        uint32_t start = cases[case_id].start;
        uint32_t end = cases[case_id].end;
        uint32_t step = cases[case_id].step;
        ...
        if (vc4_m2_copy_htod(program, out_dev, out_values, bytes) < 0 ||
            value_tl_range_style_loop_skeleton_vc4value_launch(program, grid, block, out_active, start, end, step, n) < 0 ||
            vc4_m2_copy_dtoh(program, out_values, out_dev, bytes) < 0) {
            printk("ERROR: tl_range_style launch/copy failed n=%d start=%d end=%d step=%d\n",
                   (int)n, (int)start, (int)end, (int)step);
            launch_failures++;
            continue;
        }
        int mismatches = verify_results(n, start, end, step);
        ...
        printk("VALUE_TL_RANGE_STYLE_CASE case=%d n=%d start=%d end=%d step=%d waves=%d mismatches=%d sentinel_mismatches=%d hash=%x\n",
               (int)case_id, (int)n, (int)start, (int)end, (int)step,
               (int)waves, mismatches, sentinels, case_hash);
    }

    int elapsed = timer_get_usec() - start_time;
    const char *status = (total_mismatches == 0 && sentinel_mismatches == 0 &&
                          launch_failures == 0) ? "PASS" : "FAIL";
    printk("VC4_TEST_RESULT name=value_tl_range_style_loop_skeleton_vc4value status=%s cases=%d elements_checked=%d total_mismatches=%d sentinel_mismatches=%d launch_failures=%d active_qpus=%d lanes=%d max_n=%d max_coverage_n=%d buffer_n=%d output_hash=%u saw_tl_range_style_loop=1 saw_persistent_loop_skeleton=1 saw_scalar_loop_carried=1 saw_scf_to_cf_boundary=1 runtime_allocations=%d runtime_launches=%d elapsed_usec=%d\n",
