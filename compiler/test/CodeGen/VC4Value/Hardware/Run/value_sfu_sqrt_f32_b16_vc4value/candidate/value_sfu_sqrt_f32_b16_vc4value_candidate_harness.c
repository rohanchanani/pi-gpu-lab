...
#define SQRT_ABS_TOL 0.0060f
#define SQRT_REL_TOL 0.0120f
...
static uint32_t input_index(uint32_t case_id, uint32_t i) {
    static const uint32_t idxs[] = {0u, 1u, 2u, 3u, 4u, 5u};
    return idxs[(i + case_id * 3u) % (sizeof(idxs) / sizeof(idxs[0]))];
}

static float input_value(uint32_t case_id, uint32_t i) {
    static const float vals[] = {0.0625f, 0.25f, 1.0f, 4.0f, 9.0f, 16.0f};
    return vals[input_index(case_id, i)];
}

static float sqrt_ref(uint32_t case_id, uint32_t i) {
    static const float vals[] = {0.25f, 0.5f, 1.0f, 2.0f, 3.0f, 4.0f};
    return vals[input_index(case_id, i)];
}

static float raw_rsqrt_oracle(uint32_t case_id, uint32_t i) {
    return 1.0f / sqrt_ref(case_id, i);
}

static float allowed_error(float expected) {
    return maxf_local(SQRT_ABS_TOL, SQRT_REL_TOL * absf_local(expected));
}
...
static int verify_results(uint32_t case_id, uint32_t n, float *max_abs_diff, float *max_rel_diff) {
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        float expected = sqrt_ref(case_id, i);
        float raw_rsqrt = raw_rsqrt_oracle(case_id, i);
        if (absf_local(expected - raw_rsqrt) > allowed_error(expected) * 8.0f)
        ...
        if (diff > allowed_error(expected)) {
            if (mismatches < 8)
                printk("ERROR: value_sfu_sqrt case=%d n=%d i=%d x=%f gpu=%f expected=%f raw_rsqrt=%f diff=%f rel=%f allowed=%f\n",
                       (int)case_id, (int)n, (int)i, input_value(case_id, i),
                       out_values[index], expected, raw_rsqrt, diff, rel,
                       allowed_error(expected));
            mismatches++;
        }
    }
    if (!saw_distinguishing_value) {
        printk("ERROR: value_sfu_sqrt case=%d n=%d did not distinguish sqrt from raw rsqrt oracle\n",
               (int)case_id, (int)n);
        mismatches++;
    }
    return mismatches;
...
static int verify_sentinels(uint32_t n) {
    ...
    for (uint32_t i = 0; i < BUFFER_N; i++) {
        ...
        if (float_to_bits(out_values[i]) != SENTINEL_BITS) {
            if (mismatches < 8)
                printk("ERROR: value_sfu_sqrt sentinel n=%d i=%d bits=%x expected=%x\n",
...
void notmain(void) {
    ...
    if (vc4_program_create(&program, 0) < 0 || !program)
        panic("value_sfu_sqrt_f32_b16 program create failed");
    ...
    if (vc4_m2_malloc(program, &in_dev, bytes) < 0 ||
        ...
        panic("value_sfu_sqrt_f32_b16 allocation failed");
    ...
    for (uint32_t case_id = 0; case_id < sizeof(n_cases) / sizeof(n_cases[0]); case_id++) {
        ...
        if (vc4_m2_copy_htod(program, in_dev, in_values, bytes) < 0 ||
            ...
            value_sfu_sqrt_f32_b16_vc4value_launch(program, grid, block, in_active, out_active, n) < 0 ||
            vc4_m2_copy_dtoh(program, out_values, out_dev, bytes) < 0) {
            printk("ERROR: value_sfu_sqrt launch/copy failed case=%d n=%d\n",
                   (int)case_id, (int)n);
            launch_failures++;
            continue;
        }
        ...
        printk("VALUE_SFU_SQRT_CASE case=%d n=%d waves=%d mismatches=%d sentinel_mismatches=%d hash=%x max_abs_diff=%f max_rel_diff=%f\n",
    ...
    printk("VC4_TEST_RESULT name=value_sfu_sqrt_f32_b16_vc4value status=%s cases=%d total_mismatches=%d sentinel_mismatches=%d launch_failures=%d active_qpus=%d lanes=%d max_n=%d wave_boundary_n=%d output_hash=%u output_hash_nonzero=%d max_abs_diff=%f max_rel_diff=%f saw_value_math_sqrt=1 saw_target_sfu_rsqrt_times_x=1 saw_positive_finite_domain=1 saw_approx_math_policy=1 runtime_allocations=%d runtime_launches=%d elapsed_usec=%d\n",
