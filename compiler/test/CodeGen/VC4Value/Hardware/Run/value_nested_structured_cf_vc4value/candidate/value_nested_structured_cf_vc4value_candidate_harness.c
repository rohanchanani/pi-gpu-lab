...
static const struct case_config cases[] = {
    {0u, 0u, 0u}, {1u, 1u, 0u}, {2u, 2u, 1u}, {3u, 3u, 0u},
    {5u, 5u, 1u}, {16u, 0u, 1u}, {17u, 1u, 1u}, {65u, 4u, 0u},
    {193u, 3u, 1u}, {1000u, 6u, 1u},
};
...
static int32_t expected_acc(uint32_t trip, uint32_t selector) {
    int32_t acc = 0;
...
static int verify_results(uint32_t n, uint32_t trip, uint32_t selector) {
    int mismatches = 0;
    int32_t expected = expected_acc(trip, selector);
    for (uint32_t i = 0; i < n; i++) {
        uint32_t index = GUARD + i;
        if (out_values[index] != expected) {
            if (mismatches < 8)
                printk("ERROR: nested_structured_cf n=%d trip=%d selector=%d i=%d got=%x expected=%x\n",
                       (int)n, (int)trip, (int)selector, (int)i,
...
static int verify_sentinels(uint32_t n) {
    ...
    for (uint32_t i = 0; i < BUFFER_N; i++) {
        ...
        if ((uint32_t)out_values[i] != SENTINEL) {
            if (mismatches < 8)
                printk("ERROR: nested_structured_cf sentinel n=%d i=%d got=%x expected=%x\n",
...
void notmain(void) {
    ...
    if (vc4_m2_malloc(program, &out_dev, bytes) < 0)
        panic("nested_structured_cf allocation failed");
    ...
    for (uint32_t case_id = 0; case_id < sizeof(cases) / sizeof(cases[0]); case_id++) {
        ...
        uint32_t trip = cases[case_id].trip;
        uint32_t selector = cases[case_id].selector;
        ...
        if (vc4_m2_copy_htod(program, out_dev, out_values, bytes) < 0 ||
            value_nested_structured_cf_vc4value_launch(program, grid, block, out_active, trip, selector, n) < 0 ||
            vc4_m2_copy_dtoh(program, out_values, out_dev, bytes) < 0) {
            printk("ERROR: nested_structured_cf launch/copy failed n=%d trip=%d selector=%d\n",
                   (int)n, (int)trip, (int)selector);
            launch_failures++;
            continue;
        }
        int mismatches = verify_results(n, trip, selector);
        ...
        printk("VALUE_NESTED_STRUCTURED_CF_CASE case=%d n=%d trip=%d selector=%d waves=%d mismatches=%d sentinel_mismatches=%d hash=%x\n",
               (int)case_id, (int)n, (int)trip, (int)selector, (int)waves,
    ...
    printk("VC4_TEST_RESULT name=value_nested_structured_cf_vc4value status=%s cases=%d elements_checked=%d total_mismatches=%d sentinel_mismatches=%d launch_failures=%d active_qpus=%d lanes=%d max_n=%d max_coverage_n=%d buffer_n=%d output_hash=%u saw_nested_structured_cf=1 saw_value_cf_cond_br=1 saw_value_cf_loop=1 saw_scf_to_cf_boundary=1 runtime_allocations=%d runtime_launches=%d elapsed_usec=%d\n",
