...
static const struct case_desc cases[] = {
    ...
    {1u, 5u, 15u, 16u, 19u, 7u, -0.50f},
    {7u, 1u, 16u, 23u, 16u, 3u, 1.25f},
    {2u, 5u, 1u, 17u, 19u, 6u, 1.50f},
    {7u, 2u, 16u, 23u, 19u, 5u, 0.25f},
    {1u, 2u, 7u, 16u, 16u, 4u, -0.75f},
    {2u, 1u, 15u, 17u, 19u, 3u, 0.90f},
    {7u, 5u, 2u, 23u, 16u, 8u, 1.10f},
    {1u, 5u, 16u, 16u, 19u, 8u, 0.60f},
    {2u, 2u, 7u, 17u, 16u, 5u, 1.35f}
};
    ...
    for (uint32_t q = 0; q < tc->q_rows; q++) {
        for (uint32_t d = 0; d < tc->out_dims; d++) {
            ...
            if (diff > maxf_local(MIXED_ABS_TOL, MIXED_REL_TOL * absf_local(expected))) {
                if (mismatches < 8)
                    printk("ERROR: mixed_attention output case=%d q=%d d=%d got=%f expected=%f diff=%f rel=%f\n",
            ...
            if (den_diff > maxf_local(MIXED_ABS_TOL, MIXED_REL_TOL * absf_local(expected_den))) {
                if (mismatches < 8)
                    printk("ERROR: mixed_attention denom case=%d q=%d d=%d got=%f expected=%f diff=%f rel=%f\n",
...
static int verify_sentinels(const struct case_desc *tc) {
    ...
    for (uint32_t i = 0; i < SCORE_BUFFER_N; i++) {
        ...
        if (!active && scores_values[i] != SENTINEL_H) {
            if (mismatches < 8)
                printk("ERROR: mixed_attention score sentinel i=%d half=%x\n", (int)i, scores_values[i]);
            mismatches++;
        }
    }
    for (uint32_t i = 0; i < VALUE_BUFFER_N; i++) {
        ...
        if (!active && vt_values[i] != SENTINEL_H) {
            if (mismatches < 8)
                printk("ERROR: mixed_attention vt sentinel i=%d half=%x\n", (int)i, vt_values[i]);
            mismatches++;
        }
        if (!active && float_to_bits(residual_values[i]) != SENTINEL_BITS) {
            if (mismatches < 8)
                printk("ERROR: mixed_attention residual sentinel i=%d bits=%x\n", (int)i, float_to_bits(residual_values[i]));
            mismatches++;
        }
        if (!active && gate_values[i] != SENTINEL_I) {
            if (mismatches < 8)
                printk("ERROR: mixed_attention gate sentinel i=%d got=%x\n", (int)i, gate_values[i]);
            mismatches++;
        }
    }
    for (uint32_t i = 0; i < OUT_BUFFER_N; i++) {
...
void notmain(void) {
    ...
    if (vc4_program_create(&program, 0) < 0 || !program)
        panic("mixed_value_attention_apply program create failed");
    ...
    if (vc4_m2_malloc(program, &scores_dev, scores_bytes) < 0 ||
        ...
        panic("mixed_value_attention_apply allocation failed");
    ...
    for (uint32_t case_id = 0; case_id < sizeof(cases) / sizeof(cases[0]); case_id++) {
        ...
        if (vc4_m2_copy_htod(program, scores_dev, scores_values, scores_bytes) < 0 ||
            ...
            mixed_value_attention_apply_v0_axes_mask_cf_f16_storage_vc4value_launch(
            ...
            printk("ERROR: mixed_attention launch/copy failed case=%d q_rows=%d out_dims=%d k=%d\n",
        ...
        printk("MIXED_VALUE_ATTENTION_APPLY_AXIS_MASK_CF_F16_CASE case=%d q_rows=%d out_dims=%d k=%d lds=%d ldv=%d ldo=%d scale=%f mismatches=%d sentinel_mismatches=%d hash=%x max_abs_diff=%f max_rel_diff=%f\n",
               (int)case_id, (int)tc->q_rows, (int)tc->out_dims, (int)tc->k,
