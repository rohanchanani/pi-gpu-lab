func.func @mixed_value_multi_axis_cf_tail_vc4value(
  ...
  %total_yz = arith.muli %num1, %num2 : index
  %logical_extent = arith.muli %num0, %total_yz : index
  ...
  %num2v = vector.broadcast %num2_i32 : i32 to vector<16xi32>
...
^i32_path(%factor_i32_path: vector<16xf32>):
  ...
  %selected_f = arith.select %cond, %candidate, %yfv : vector<16xi1>, vector<16xf32>
  %biased = arith.addi %xiv, %num2v : vector<16xi32>
  %selected_i = arith.select %cond, %biased, %xiv : vector<16xi1>, vector<16xi32>
  cf.br ^merge(%selected_f, %selected_i : vector<16xf32>, vector<16xi32>)
...
^float_path(%factor_float_path: vector<16xf32>):
  ...
  %selected_i_alt = arith.addi %xiv, %num2v : vector<16xi32>
  cf.br ^merge(%candidate_alt, %selected_i_alt : vector<16xf32>, vector<16xi32>)

^merge(%result_f: vector<16xf32>, %result_i: vector<16xi32>):
  %in_extent = arith.cmpi ult, %block_id, %logical_extent : index
  cf.cond_br %in_extent, ^store(%result_f, %result_i : vector<16xf32>, vector<16xi32>), ^exit

^store(%store_f: vector<16xf32>, %store_i: vector<16xi32>):
  vector.transfer_write %store_f, %out_f[%base], %mask : vector<16xf32>, memref<?xf32, #vc4value.global>
  vector.transfer_write %store_i, %out_i[%base], %mask : vector<16xi32>, memref<?xi32, #vc4value.global>
  cf.br ^exit
