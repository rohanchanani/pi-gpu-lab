...
#define OUT_BUFFER_N (1u + 2u * GUARD)
...
static float expected_max(uint32_t case_id, uint32_t n) {
    float maxv = -80.0f;
    for (uint32_t lane = 0; lane < n; lane++) {
        float value = input_value(case_id, lane);
        if (value > maxv)
            maxv = value;
    }
    return maxv;
...
static int verify_case(uint32_t case_id, uint32_t n, float *max_abs_diff) {
    float expected = expected_max(case_id, n);
    float got = out_values[GUARD];
    float diff = absf_local(got - expected);
    if (diff > *max_abs_diff)
        *max_abs_diff = diff;
    if (diff != 0.0f) {
        printk("ERROR: value_reduce_max case=%d n=%d got=%f expected=%f diff=%f\n",
               (int)case_id, (int)n, got, expected, diff);
        return 1;
    }
    return 0;
}
...
static int verify_sentinels(void) {
    ...
    for (uint32_t i = 0; i < OUT_BUFFER_N; i++) {
        ...
        if (float_to_bits(out_values[i]) != SENTINEL_BITS) {
            if (mismatches < 8)
                printk("ERROR: value_reduce_max sentinel i=%d bits=%x expected=%x\n",
...
static uint32_t hash_case(uint32_t n) {
    ...
    hash ^= float_to_bits(out_values[GUARD]) + 0x9e3779b9u;
    return rotl32_local(hash, 5u) * 16777619u;
}

void notmain(void) {
    struct vc4_program *program = 0;
    if (vc4_program_create(&program, 0) < 0 || !program)
        panic("value_reduce_max_f32_b16 program create failed");
    ...
    if (vc4_m2_malloc(program, &in_dev, in_bytes) < 0 ||
        ...
        panic("value_reduce_max_f32_b16 allocation failed");
    ...
    for (uint32_t case_id = 0; case_id < sizeof(active_counts) / sizeof(active_counts[0]); case_id++) {
        ...
        if (vc4_m2_copy_htod(program, in_dev, in_values, in_bytes) < 0 ||
            ...
            value_reduce_max_f32_b16_vc4value_launch(program, grid, block, in_active, out_active, n, 1u) < 0 ||
            vc4_m2_copy_dtoh(program, out_values, out_dev, out_bytes) < 0) {
            printk("ERROR: value_reduce_max launch/copy failed case=%d n=%d\n",
                   (int)case_id, (int)n);
            launch_failures++;
            continue;
        }
        ...
        printk("VALUE_REDUCE_MAX_CASE case=%d n=%d mismatches=%d sentinel_mismatches=%d hash=%x max_abs_diff=%f\n",
