    ...
    for (uint32_t q = 0; q < q_rows; q++) {
        for (uint32_t d = 0; d < out_dims; d++) {
            ...
            if (diff > maxf_local(ATTN_ABS_TOL, ATTN_REL_TOL * absf_local(expected))) {
                if (mismatches < 8)
                    printk("ERROR: attention_f32 case=%d q=%d d=%d k=%d got=%f expected=%f diff=%f rel=%f\n",
...
void notmain(void) {
    ...
    if (vc4_program_create(&program, 0) < 0 || !program)
        panic("value_attention_apply_v0_f32 program create failed");
    ...
    if (vc4_m2_malloc(program, &scores_dev, scores_bytes) < 0 ||
        ...
        panic("value_attention_apply_v0_f32 allocation failed");
    ...
    for (uint32_t vi = 0; vi < sizeof(vt_stride_cases) / sizeof(vt_stride_cases[0]); vi++) {
        ...
        if (vc4_m2_copy_htod(program, scores_dev, scores_values, scores_bytes) < 0 ||
            ...
            value_attention_apply_v0_f32_b16_vc4value_launch(
        ...
        printk("VALUE_ATTENTION_APPLY_F32_CASE case=%d q_rows=%d out_dims=%d k=%d lds=%d ldv=%d ldo=%d mismatches=%d sentinel_mismatches=%d hash=%x max_abs_diff=%f max_rel_diff=%f\n",
