...
#define BIAS (-3.0f)
#define THRESHOLD 12.5f
#define EPSILON 0.001f

static const uint32_t n_cases[] = {
    0u, 1u, 15u, 16u, 17u, 31u, 32u, 33u, 64u, 65u, 193u, 1000u
};
static const float a_cases[] = {0.5f, -1.25f, 2.0f};
static const uint32_t trip_cases[] = {0u, 1u, 3u};
...
static float looped_value(float x, float y, float a, uint32_t trip) {
    float acc = y;
    float scaled = a * x;
...
static void fill_buffers(uint32_t n, float a, uint32_t trip, int32_t use_select) {
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        float looped = looped_value(x_values[index], y_values[index], a, trip);
        expected_values[index] =
            use_select ? (looped > THRESHOLD ? looped : fallback_values[index])
                       : (looped + BIAS);
    }
}
...
static int verify_results(uint32_t n, float *max_abs_diff) {
    ...
    *max_abs_diff = 0.0f;
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (ad > EPSILON) {
            if (mismatches < 8)
                printk("ERROR: mixed_cf_saxpy n=%d i=%d gpu=%f cpu=%f diff=%f\n",
...
static int verify_sentinels(uint32_t n) {
    ...
    for (uint32_t i = 0; i < BUFFER_N; i++) {
        ...
        if (got != SENTINEL_BITS) {
            if (mismatches < 8)
                printk("ERROR: mixed_cf_saxpy sentinel n=%d i=%d got=%x expected=%x\n",
...
void notmain(void) {
    ...
    if (vc4_m2_malloc(program, &x_dev, bytes) < 0 ||
        ...
        panic("mixed_cf_saxpy allocation failed");
            ...
            for (uint32_t flag_id = 0; flag_id < sizeof(flag_cases) / sizeof(flag_cases[0]); flag_id++) {
                for (uint32_t n_id = 0; n_id < sizeof(n_cases) / sizeof(n_cases[0]); n_id++) {
                    ...
                    if (vc4_m2_copy_htod(program, x_dev, x_values, bytes) < 0 ||
                        ...
                        mixed_value_cf_saxpy_loop_select_tail_vc4value_launch(
                            program, grid, block, x_active, y_active, fallback_active,
                            out_active, a, BIAS, THRESHOLD, trip, use_select, n) < 0 ||
                        vc4_m2_copy_dtoh(program, out_values, out_dev, bytes) < 0) {
                        printk("ERROR: mixed_cf_saxpy launch/copy failed a_id=%d trip=%d flag=%d n=%d\n",
                               (int)a_id, (int)trip, (int)use_select, (int)n);
                        launch_failures++;
                        case_count++;
                        continue;
                    }
                    ...
                    output_hash ^= case_hash + 0x9e3779b9u + (a_id << 16) +
                                   (trip_id << 10) + (flag_id << 6) + n_id;
                    ...
                    case_count++;
                    printk("MIXED_VALUE_CF_SAXPY_CASE a_id=%d trip=%d flag=%d n=%d waves=%d mismatches=%d sentinel_mismatches=%d hash=%x max_abs_diff=%f\n",
                           (int)a_id, (int)trip, (int)use_select, (int)n, (int)waves,
    ...
    printk("VC4_TEST_RESULT name=mixed_value_cf_saxpy_loop_select_tail_vc4value status=%s cases=%d elements_checked=%d total_mismatches=%d sentinel_mismatches=%d launch_failures=%d active_qpus=%d lanes=%d max_n=%d max_coverage_n=%d buffer_n=%d output_hash=%u max_abs_diff=%f saw_value_cf_loop=1 saw_value_cf_cond_br=1 saw_value_vector_block_arg=1 saw_value_tail_mask=1 saw_value_tmu_load=1 saw_value_vdw_preserve=1 saw_value_f32_cmp_select=1 runtime_allocations=%d runtime_launches=%d elapsed_usec=%d\n",
           status, case_count, elements_checked, total_mismatches,
           sentinel_mismatches, launch_failures, ACTIVE_QPUS, LANES, MAX_N,
           MAX_COVERAGE_N, BUFFER_N, output_hash, max_abs_diff_overall, 4,
           case_count, elapsed);
