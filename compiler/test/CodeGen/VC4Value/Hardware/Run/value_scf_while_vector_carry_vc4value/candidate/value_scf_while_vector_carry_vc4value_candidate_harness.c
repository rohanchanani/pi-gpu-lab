...
static int verify_results(uint32_t n, uint32_t trip) {
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (out_values[index] != expected) {
            if (mismatches < 8)
                printk("ERROR: scf_while_vector_carry active n=%d trip=%d i=%d got=%x expected=%x\n",
...
static int verify_sentinels(uint32_t n) {
    ...
    for (uint32_t i = 0; i < BUFFER_N; i++) {
        ...
        if ((uint32_t)out_values[i] != SENTINEL) {
            if (mismatches < 8)
                printk("ERROR: scf_while_vector_carry sentinel n=%d i=%d got=%x expected=%x\n",
...
void notmain(void) {
    ...
    if (vc4_m2_malloc(program, &out_dev, bytes) < 0)
        panic("scf_while_vector_carry allocation failed");
    ...
    for (uint32_t case_id = 0; case_id < sizeof(cases) / sizeof(cases[0]); case_id++) {
        ...
        if (vc4_m2_copy_htod(program, out_dev, out_values, bytes) < 0 ||
            value_scf_while_vector_carry_vc4value_launch(program, grid, block, out_active, trip, n) < 0 ||
            vc4_m2_copy_dtoh(program, out_values, out_dev, bytes) < 0) {
            printk("ERROR: scf_while_vector_carry launch/copy failed n=%d trip=%d\n", (int)n, (int)trip);
            launch_failures++;
            continue;
        }
        ...
        printk("VALUE_SCF_WHILE_VECTOR_CARRY_CASE case=%d n=%d trip=%d waves=%d mismatches=%d sentinel_mismatches=%d hash=%x\n",
    ...
    printk("VC4_TEST_RESULT name=value_scf_while_vector_carry_vc4value status=%s cases=%d elements_checked=%d total_mismatches=%d sentinel_mismatches=%d launch_failures=%d active_qpus=%d lanes=%d max_n=%d max_coverage_n=%d buffer_n=%d output_hash=%u saw_value_scf_while=1 saw_vector_loop_carried=1 saw_value_tail_mask=1 saw_scf_to_cf_boundary=1 runtime_allocations=%d runtime_launches=%d elapsed_usec=%d\n",
