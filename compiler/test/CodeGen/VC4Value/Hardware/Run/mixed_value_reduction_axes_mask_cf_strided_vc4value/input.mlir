func.func @mixed_value_reduction_axes_mask_cf_strided_vc4value(
    ...
    %rank_f: memref<?x?xf32, strided<[?, 1], offset: 0>, #vc4value.global> {vc4value.arg_name = "rank_f",
                                                                            ...
                                                                            vc4value.shape_args = ["rows", "cols"],
                                                                            vc4value.stride_args = ["row_stride"]},
    %row_out_i: memref<?xi32, #vc4value.global> {vc4value.arg_name = "row_out_i",
                                                  vc4value.direction = "inout",
                                                  vc4value.shape_args = ["rows"]},
    %row_out_f: memref<?xf32, #vc4value.global> {vc4value.arg_name = "row_out_f",
                                                  vc4value.direction = "inout",
                                                  vc4value.shape_args = ["rows"]},
    %tail_out_i: memref<?x?xi32, strided<[?, 1], offset: 0>, #vc4value.global> {vc4value.arg_name = "tail_out_i",
                                                                                ...
                                                                                vc4value.shape_args = ["rows", "cols"],
                                                                                vc4value.stride_args = ["row_stride"]},
    %tail_out_f: memref<?x?xf32, strided<[?, 1], offset: 0>, #vc4value.global> {vc4value.arg_name = "tail_out_f",
                                                                                vc4value.direction = "inout",
                                                                                vc4value.shape_args = ["rows", "cols"],
                                                                                vc4value.stride_args = ["row_stride"]},
...
^copy:
  ...
  %zero_iv = vector.broadcast %zero_i32 : i32 to vector<16xi32>
  %zero_fv = vector.broadcast %zero_f : f32 to vector<16xf32>
...
^i32_path(%i_bias_a: vector<16xi32>, %f_bias_a: vector<16xf32>):
  ...
  %candidate_f = arith.addf %rank_tail_f, %f_bias_a : vector<16xf32>
...
^f32_path(%i_bias_b: vector<16xi32>, %f_bias_b: vector<16xf32>):
  ...
  %candidate_f_alt = arith.addf %rank_tail_f, %f_bias_b : vector<16xf32>
...
^store(%store_i: vector<16xi32>, %store_f: vector<16xf32>):
  ...
  vector.transfer_write %store_f, %tail_out_f[%pid1, %col], %mask {in_bounds = [true]} : vector<16xf32>, memref<?x?xf32, strided<[?, 1], offset: 0>, #vc4value.global>
  %reduce_i = arith.select %mask, %store_i, %zero_iv : vector<16xi1>, vector<16xi32>
  %reduce_f = arith.select %mask, %store_f, %zero_fv : vector<16xi1>, vector<16xf32>
  %row_sum_i = vector.reduction <add>, %reduce_i : vector<16xi32> into i32
  %row_sum_f = vector.reduction <add>, %reduce_f : vector<16xf32> into f32
  memref.store %row_sum_i, %row_out_i[%pid1] : memref<?xi32, #vc4value.global>
  memref.store %row_sum_f, %row_out_f[%pid1] : memref<?xf32, #vc4value.global>
  cf.br ^exit
