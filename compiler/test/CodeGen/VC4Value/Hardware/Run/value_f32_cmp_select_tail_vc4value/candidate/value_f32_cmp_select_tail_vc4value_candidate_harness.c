...
static float lo_values[BUFFER_N];
static float hi_values[BUFFER_N];
...
static float x_value(uint32_t i) {
    int32_t whole = (int32_t)((i * 19u + 5u) % 83u) - 41;
    return (float)whole * 0.5f + (float)((i * 7u) & 3u) * 0.25f;
}

static float lo_value(uint32_t i) {
    return -1000.0f - (float)((i * 3u) % 127u) * 0.5f;
}

static float hi_value(uint32_t i) {
    return 2000.0f + (float)((i * 5u) % 113u) * 0.25f;
}
...
static void fill_buffers(uint32_t n) {
    ...
    for (uint32_t i = 0; i < BUFFER_N; i++) {
        ...
        lo_values[i] = lo_value(active);
        hi_values[i] = hi_value(active);
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        expected_bits[index] =
            float_to_bits(x_values[index] < THRESHOLD ? lo_values[index] : hi_values[index]);
    }
}
...
static int verify_results(uint32_t n, float *max_abs_diff) {
    ...
    *max_abs_diff = 0.0f;
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (got != expected) {
            float diff = out_values[index] - bits_to_float(expected);
            float ad = diff < 0.0f ? -diff : diff;
            if (ad > *max_abs_diff)
                *max_abs_diff = ad;
            if (mismatches < 8)
                printk("ERROR: value_f32_cmp_select n=%d i=%d x=%f gpu_bits=%x expected_bits=%x\n",
                       (int)n, (int)i, x_values[index], got, expected);
            mismatches++;
        }
    }
    return mismatches;
...
static int verify_sentinels(uint32_t n) {
    ...
    for (uint32_t i = 0; i < BUFFER_N; i++) {
        ...
        if (got != SENTINEL_BITS) {
            if (mismatches < 8)
                printk("ERROR: value_f32_cmp_select sentinel n=%d i=%d got=%x expected=%x\n",
...
void notmain(void) {
    ...
    vc4_deviceptr_t x_dev = 0, lo_dev = 0, hi_dev = 0, out_dev = 0;
    if (vc4_m2_malloc(program, &x_dev, bytes) < 0 ||
        vc4_m2_malloc(program, &lo_dev, bytes) < 0 ||
        vc4_m2_malloc(program, &hi_dev, bytes) < 0 ||
        vc4_m2_malloc(program, &out_dev, bytes) < 0)
        panic("value_f32_cmp_select allocation failed");
    ...
    for (uint32_t case_id = 0; case_id < sizeof(n_cases) / sizeof(n_cases[0]); case_id++) {
        ...
        vc4_deviceptr_t lo_active = lo_dev + GUARD * sizeof(float);
        vc4_deviceptr_t hi_active = hi_dev + GUARD * sizeof(float);
        vc4_deviceptr_t out_active = out_dev + GUARD * sizeof(float);
        if (vc4_m2_copy_htod(program, x_dev, x_values, bytes) < 0 ||
            vc4_m2_copy_htod(program, lo_dev, lo_values, bytes) < 0 ||
            vc4_m2_copy_htod(program, hi_dev, hi_values, bytes) < 0 ||
            vc4_m2_copy_htod(program, out_dev, out_values, bytes) < 0 ||
            value_f32_cmp_select_tail_vc4value_launch(
                program, grid, block, x_active, lo_active, hi_active, out_active,
                THRESHOLD, n) < 0 ||
            vc4_m2_copy_dtoh(program, out_values, out_dev, bytes) < 0) {
            printk("ERROR: value_f32_cmp_select launch/copy failed n=%d\n", (int)n);
            launch_failures++;
            continue;
        }
        ...
        printk("VALUE_F32_CMP_SELECT_CASE case=%d n=%d waves=%d mismatches=%d sentinel_mismatches=%d hash=%x max_abs_diff=%f\n",
    ...
    printk("VC4_TEST_RESULT name=value_f32_cmp_select_tail_vc4value status=%s cases=%d elements_checked=%d total_mismatches=%d sentinel_mismatches=%d launch_failures=%d active_qpus=%d lanes=%d max_n=%d max_coverage_n=%d buffer_n=%d output_hash=%u max_abs_diff=%f runtime_allocations=%d runtime_launches=%d elapsed_usec=%d\n",
    ...
    vc4Free(program, lo_dev);
    vc4Free(program, hi_dev);
