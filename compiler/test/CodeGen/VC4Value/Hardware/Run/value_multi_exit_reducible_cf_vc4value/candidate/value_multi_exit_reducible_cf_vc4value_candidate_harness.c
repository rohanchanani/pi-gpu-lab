...
static const struct case_config cases[] = {
    {0u, 0u, 0u}, {1u, 0u, 5u}, {2u, 1u, 0u}, {3u, 3u, 2u},
    {5u, 5u, 9u}, {17u, 7u, 3u}, {193u, 4u, 4u}, {1000u, 11u, 6u},
};
...
static int32_t expected_acc(uint32_t trip, uint32_t early) {
    uint32_t i = 0;
    int32_t acc = 0;
    while (1) {
        if (i >= trip)
...
static int verify_results(uint32_t n, uint32_t trip, uint32_t early) {
    int mismatches = 0;
    int32_t expected = expected_acc(trip, early);
    for (uint32_t i = 0; i < n; i++) {
        uint32_t index = GUARD + i;
        if (out_values[index] != expected) {
            if (mismatches < 8)
                printk("ERROR: multi_exit n=%d trip=%d early=%d i=%d got=%x expected=%x\n",
                       (int)n, (int)trip, (int)early, (int)i,
...
static int verify_sentinels(uint32_t n) {
    ...
    for (uint32_t i = 0; i < BUFFER_N; i++) {
        ...
        if ((uint32_t)out_values[i] != SENTINEL) {
            if (mismatches < 8)
                printk("ERROR: multi_exit sentinel n=%d i=%d got=%x expected=%x\n",
...
void notmain(void) {
    ...
    if (vc4_m2_malloc(program, &out_dev, bytes) < 0)
        panic("multi_exit allocation failed");
    ...
    for (uint32_t case_id = 0; case_id < sizeof(cases) / sizeof(cases[0]); case_id++) {
        ...
        uint32_t early = cases[case_id].early;
        ...
        if (vc4_m2_copy_htod(program, out_dev, out_values, bytes) < 0 ||
            value_multi_exit_reducible_cf_vc4value_launch(program, grid, block, out_active, trip, early, n) < 0 ||
            vc4_m2_copy_dtoh(program, out_values, out_dev, bytes) < 0) {
            printk("ERROR: multi_exit launch/copy failed n=%d trip=%d early=%d\n",
                   (int)n, (int)trip, (int)early);
            launch_failures++;
            continue;
        }
        int mismatches = verify_results(n, trip, early);
        ...
        printk("VALUE_MULTI_EXIT_REDUCIBLE_CF_CASE case=%d n=%d trip=%d early=%d waves=%d mismatches=%d sentinel_mismatches=%d hash=%x\n",
               (int)case_id, (int)n, (int)trip, (int)early, (int)waves,
