...
#define ATTN_ABS_TOL 0.0300f
#define ATTN_REL_TOL 0.0400f
...
static void fill_buffers(uint32_t case_id, uint32_t q_rows, uint32_t out_dims,
    ...
    for (uint32_t q = 0; q < q_rows; q++)
        for (uint32_t j = 0; j < k; j++)
            scores_values[GUARD + q * lds + j] = score_half(case_id, q, j);

    for (uint32_t d = 0; d < out_dims; d++)
        for (uint32_t j = 0; j < k; j++)
            vt_values[GUARD + d * ldv + j] = vt_half(case_id, d, j);
    (void)ldo;
}
...
static float expected_attention(uint32_t case_id, uint32_t q, uint32_t d, uint32_t k) {
    float maxv = -80.0f;
    for (uint32_t j = 0; j < k; j++) {
        float s = score_value(case_id, q, j);
        if (s > maxv)
            maxv = s;
    }
    float denom = 0.0f;
    for (uint32_t j = 0; j < k; j++)
        denom += natural_exp_ref(score_value(case_id, q, j) - maxv);

    float acc = 0.0f;
    for (uint32_t j = 0; j < k; j++) {
        float prob = natural_exp_ref(score_value(case_id, q, j) - maxv) / denom;
    ...
    for (uint32_t q = 0; q < q_rows; q++) {
        for (uint32_t d = 0; d < out_dims; d++) {
            ...
            if (diff > maxf_local(ATTN_ABS_TOL, ATTN_REL_TOL * absf_local(expected))) {
                if (mismatches < 8)
                    printk("ERROR: attention_f16_storage case=%d q=%d d=%d k=%d got=%f expected=%f diff=%f rel=%f\n",
...
static int verify_sentinels(uint32_t q_rows, uint32_t out_dims, uint32_t k,
    ...
    for (uint32_t i = 0; i < SCORE_BUFFER_N; i++) {
        ...
        if (!active && scores_values[i] != SENTINEL_H) {
            if (mismatches < 8)
                printk("ERROR: attention_f16_storage score sentinel i=%d half=%x\n", (int)i, scores_values[i]);
            mismatches++;
        }
    }
    for (uint32_t i = 0; i < VT_BUFFER_N; i++) {
        ...
        if (!active && vt_values[i] != SENTINEL_H) {
            if (mismatches < 8)
                printk("ERROR: attention_f16_storage vt sentinel i=%d half=%x\n", (int)i, vt_values[i]);
            mismatches++;
        }
    }
    for (uint32_t i = 0; i < OUT_BUFFER_N; i++) {
        ...
        if (!active && float_to_bits(out_values[i]) != SENTINEL_BITS) {
            if (mismatches < 8)
                printk("ERROR: attention_f16_storage out sentinel i=%d bits=%x\n", (int)i, float_to_bits(out_values[i]));
            mismatches++;
        }
    }
    return mismatches;
...
void notmain(void) {
    ...
    if (vc4_program_create(&program, 0) < 0 || !program)
        panic("value_attention_apply_v0_f16_storage program create failed");
    ...
    if (vc4_m2_malloc(program, &scores_dev, scores_bytes) < 0 ||
        ...
        panic("value_attention_apply_v0_f16_storage allocation failed");
    ...
    for (uint32_t ki = 0; ki < sizeof(k_cases) / sizeof(k_cases[0]); ki++) {
        ...
        uint32_t lds = (case_id % 2u) ? 17u : 23u;
        uint32_t ldv = (case_id % 3u) ? 19u : 16u;
        ...
        if (vc4_m2_copy_htod(program, scores_dev, scores_values, scores_bytes) < 0 ||
            ...
            value_attention_apply_v0_f16_storage_vc4value_launch(
            ...
            printk("ERROR: attention_f16_storage launch/copy failed case=%d q_rows=%d out_dims=%d k=%d lds=%d ldv=%d ldo=%d\n",
        ...
        printk("VALUE_ATTENTION_APPLY_F16_STORAGE_CASE case=%d q_rows=%d out_dims=%d k=%d lds=%d ldv=%d ldo=%d mismatches=%d sentinel_mismatches=%d hash=%x max_abs_diff=%f max_rel_diff=%f\n",
    ...
    printk("VC4_TEST_RESULT name=value_attention_apply_v0_f16_storage_vc4value status=%s cases=%d total_mismatches=%d sentinel_mismatches=%d launch_failures=%d active_qpus=%d lanes=%d max_q_rows=%d max_out_dims=%d max_k=%d output_hash=%u output_hash_nonzero=%d max_abs_diff=%f max_rel_diff=%f saw_value_attention_apply_v0=1 saw_value_precomputed_scores=1 saw_value_transposed_v_layout=1 saw_value_softmax_v0=1 saw_value_weighted_sum_reduction=1 saw_value_scalar_result_store=1 saw_softmax_uses_natural_exp=1 saw_value_approx_sfu_exp=1 saw_value_approx_sfu_recip_div=1 saw_value_f16_storage_load=1 saw_value_f32_compute_after_f16_load=1 saw_f16_storage_finite_policy=1 saw_no_scalar_global_load=1 saw_no_nontransposed_v_gather=1 runtime_allocations=%d runtime_launches=%d elapsed_usec=%d\n",
