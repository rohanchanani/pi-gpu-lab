...
static int verify_rows(uint32_t case_id, uint32_t rows, uint32_t cols, float *max_abs_diff) {
    ...
    for (uint32_t r = 0; r < rows; r++) {
        ...
        if (diff > F32_TOLERANCE) {
            if (mismatches < 8)
                printk("ERROR: row reduction case=%d row=%d cols=%d got=%f expected=%f diff=%f\n",
...
static int verify_sentinels(uint32_t rows, uint32_t cols, uint32_t lda) {
    ...
    for (uint32_t i = 0; i < OUT_BUFFER_N; i++) {
        ...
        if (float_to_bits(out_values[i]) != OUT_SENTINEL_BITS) {
            if (mismatches < 8)
                printk("ERROR: row reduction output sentinel rows=%d i=%d bits=%x\n",
    ...
    for (uint32_t r = 0; r < rows; r++) {
        for (uint32_t c = cols; c < lda; c++) {
            ...
            if (float_to_bits(in_values[index]) != IN_SENTINEL_BITS) {
                if (mismatches < 8)
                    printk("ERROR: row reduction input padding changed row=%d col=%d lda=%d bits=%x\n",
...
void notmain(void) {
    ...
    if (vc4_program_create(&program, 0) < 0 || !program)
        panic("value_reduce_row_strided_sum program create failed");
    ...
    if (vc4_m2_malloc(program, &in_dev, in_bytes) < 0 ||
        ...
        panic("value_reduce_row_strided_sum allocation failed");
    ...
    for (uint32_t case_id = 0; case_id < sizeof(cases) / sizeof(cases[0]); case_id++) {
        ...
        if (vc4_m2_copy_htod(program, in_dev, in_values, in_bytes) < 0 ||
            ...
            value_reduce_row_strided_sum_vc4value_launch(program, grid, block, in_active,
                                                         out_active, rows, cols, lda) < 0 ||
            vc4_m2_copy_dtoh(program, out_values, out_dev, out_bytes) < 0 ||
            vc4_m2_copy_dtoh(program, in_values, in_dev, in_bytes) < 0) {
            printk("ERROR: row reduction launch/copy failed case=%d rows=%d cols=%d lda=%d\n",
        ...
        printk("VALUE_REDUCE_ROW_STRIDED_CASE case=%d rows=%d cols=%d lda=%d mismatches=%d sentinel_mismatches=%d hash=%x max_abs_diff=%f\n",
               (int)case_id, (int)rows, (int)cols, (int)lda, mismatches, sentinels, hash, max_abs_diff);
    }

    int elapsed = timer_get_usec() - start;
    const char *status = (total_mismatches == 0 && sentinel_mismatches == 0 &&
                          launch_failures == 0) ? "PASS" : "FAIL";
    printk("VC4_TEST_RESULT name=value_reduce_row_strided_sum_vc4value status=%s cases=%d total_mismatches=%d sentinel_mismatches=%d launch_failures=%d active_qpus=%d lanes=%d max_rows=%d max_cols=%d output_hash=%u output_hash_nonzero=%d max_abs_diff=%f saw_value_row_strided_reduction=1 saw_value_reduction_f32_finite_add=1 saw_f32_finite_tree_policy=1 saw_row_padding_sentinels=1 runtime_allocations=%d runtime_launches=%d elapsed_usec=%d\n",
           ...
           2, (int)(sizeof(cases) / sizeof(cases[0])), elapsed);
