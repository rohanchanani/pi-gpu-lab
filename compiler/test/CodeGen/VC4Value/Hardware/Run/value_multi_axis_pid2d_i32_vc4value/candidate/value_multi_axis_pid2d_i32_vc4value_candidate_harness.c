...
static int32_t expected_value(uint32_t pid0, uint32_t pid1, uint32_t lane) {
    return (int32_t)(pid1 * 1000u + pid0 * 100u + lane);
}
        ...
        for (uint32_t pid0 = 0; pid0 < grid_x; pid0++) {
            ...
            for (uint32_t lane = 0; lane < LANES; lane++) {
                ...
                if (out_values[index] != expected) {
                    if (mismatches < 8)
                        printk("ERROR: pid2d x=%d y=%d pid0=%d pid1=%d lane=%d got=%x expected=%x\n",
...
static int verify_sentinels(uint32_t elements) {
    ...
    for (uint32_t i = 0; i < BUFFER_N; i++) {
        ...
        if ((uint32_t)out_values[i] != SENTINEL) {
            if (mismatches < 8)
                printk("ERROR: pid2d sentinel elements=%d i=%d got=%x expected=%x\n",
...
void notmain(void) {
    ...
    for (uint32_t case_id = 0; case_id < sizeof(cases) / sizeof(cases[0]); case_id++) {
        ...
        if (vc4_m2_copy_htod(program, out_dev, out_values, bytes) < 0 ||
            value_multi_axis_pid2d_i32_vc4value_launch(program, grid, block, out_active, elements) < 0 ||
            vc4_m2_copy_dtoh(program, out_values, out_dev, bytes) < 0) {
            printk("ERROR: pid2d launch/copy failed case=%d grid=%d,%d\n",
        ...
        printk("VALUE_MULTI_AXIS_PID2D_CASE case=%d grid_x=%d grid_y=%d elements=%d mismatches=%d sentinel_mismatches=%d hash=%x\n",
    ...
    printk("VC4_TEST_RESULT name=value_multi_axis_pid2d_i32_vc4value status=%s cases=%d elements_checked=%d total_mismatches=%d sentinel_mismatches=%d launch_failures=%d active_qpus=%d lanes=%d max_grid_blocks=%d max_elements=%d buffer_n=%d output_hash=%u saw_value_multi_axis_launch=1 saw_value_program_id_axis0=1 saw_value_program_id_axis1=1 saw_value_num_programs_axis0=1 runtime_allocations=%d runtime_launches=%d elapsed_usec=%d\n",
