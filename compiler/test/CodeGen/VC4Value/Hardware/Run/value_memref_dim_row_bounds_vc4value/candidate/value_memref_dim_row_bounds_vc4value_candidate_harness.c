...
static int32_t input_value(uint32_t case_id, uint32_t row, uint32_t col) {
    return (int32_t)(0x44000000u + case_id * 0x10000u + row * 733u + col * 31u);
}

static void fill_buffers(uint32_t case_id, uint32_t rows, uint32_t cols, uint32_t lda) {
    for (uint32_t i = 0; i < BUFFER_N; i++) {
        in_values[i] = (int32_t)(0x70000000u + i);
    ...
    for (uint32_t r = 0; r < rows; r++) {
        for (uint32_t c = 0; c < cols; c++) {
            ...
            if (out_values[index] != expected) {
                if (mismatches < 8)
                    printk("ERROR: memref.dim row=%d col=%d lda=%d got=%x expected=%x\n",
...
static int verify_sentinels(uint32_t rows, uint32_t cols, uint32_t lda) {
    ...
    for (uint32_t i = 0; i < BUFFER_N; i++) {
        ...
        if ((uint32_t)out_values[i] != SENTINEL) {
            if (mismatches < 8)
                printk("ERROR: memref.dim sentinel rows=%d cols=%d lda=%d i=%d got=%x\n",
...
void notmain(void) {
    ...
    if (vc4_program_create(&program, 0) < 0 || !program)
        panic("value_memref_dim_row_bounds program create failed");
    ...
    if (vc4_m2_malloc(program, &in_dev, bytes) < 0 ||
        ...
        panic("value_memref_dim_row_bounds allocation failed");
    ...
    for (uint32_t case_id = 0; case_id < sizeof(cases) / sizeof(cases[0]); case_id++) {
        ...
        if (vc4_m2_copy_htod(program, in_dev, in_values, bytes) < 0 ||
            ...
            value_memref_dim_row_bounds_vc4value_launch(
                program, grid, block, in_active, out_active, rows, cols, lda) < 0 ||
            vc4_m2_copy_dtoh(program, out_values, out_dev, bytes) < 0) {
            printk("ERROR: memref.dim launch/copy failed case=%d rows=%d cols=%d lda=%d\n",
        ...
        printk("VALUE_MEMREF_DIM_CASE case=%d rows=%d cols=%d lda=%d grid_rows=%d mismatches=%d sentinel_mismatches=%d hash=%x\n",
    ...
    printk("VC4_TEST_RESULT name=value_memref_dim_row_bounds_vc4value status=%s cases=%d total_mismatches=%d sentinel_mismatches=%d launch_failures=%d active_qpus=%d lanes=%d max_rows=%d max_cols=%d output_hash=%u output_hash_nonzero=%d saw_value_memref_dim_metadata=1 saw_value_shape_args=1 saw_no_hidden_descriptor=1 saw_row_bounds=1 saw_value_tail_mask=1 runtime_allocations=%d runtime_launches=%d elapsed_usec=%d\n",
