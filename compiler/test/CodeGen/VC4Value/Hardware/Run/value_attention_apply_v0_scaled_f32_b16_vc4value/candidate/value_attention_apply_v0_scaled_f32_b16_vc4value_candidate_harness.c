...
void notmain(void) {
    ...
    for (uint32_t vi = 0; vi < sizeof(vt_stride_cases) / sizeof(vt_stride_cases[0]); vi++) {
        ...
        if (vc4_m2_copy_htod(program, scores_dev, scores_values, scores_bytes) < 0 ||
            ...
            value_attention_apply_v0_scaled_f32_b16_vc4value_launch(
    ...
    printk("VC4_TEST_RESULT name=value_attention_apply_v0_scaled_f32_b16_vc4value status=%s cases=%d total_mismatches=%d sentinel_mismatches=%d launch_failures=%d active_qpus=%d lanes=%d max_q_rows=%d max_out_dims=%d max_k=%d output_hash=%u output_hash_nonzero=%d max_abs_diff=%f max_rel_diff=%f saw_value_attention_apply_v0=1 saw_value_attention_apply_scaled_scores=1 saw_scalar_scale_arg=1 saw_no_scalar_global_load=1 saw_value_precomputed_scores=1 saw_value_transposed_v_layout=1 saw_value_softmax_v0=1 saw_value_weighted_sum_reduction=1 saw_value_scalar_result_store=1 saw_softmax_uses_natural_exp=1 saw_value_approx_sfu_exp=1 saw_value_approx_sfu_recip_div=1 saw_value_finite_f32_max_reduction=1 saw_value_reduction_f32_finite_add=1 saw_no_nontransposed_v_gather=1 runtime_allocations=%d runtime_launches=%d elapsed_usec=%d\n",
