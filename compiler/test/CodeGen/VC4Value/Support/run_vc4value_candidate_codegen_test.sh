#!/usr/bin/env bash
# Generate, assemble, build, or run a VC4Value codegen candidate program bundle for
...
usage() {
  cat >&2 <<'USAGE'
usage:
  run_vc4value_candidate_codegen_test.sh --self-test
  run_vc4value_candidate_codegen_test.sh TEST_NAME [generate|assemble|build|run|all|workdir|generated-dir|clean]
USAGE
}
...
TEST_ROOT="$REPO_ROOT/compiler/test/CodeGen/VC4Value/Hardware/Run/$TEST_NAME"
...
AUTO_ROOT_RAW="${VC4_CODEGEN_STATE_ROOT:-.vc4_auto/codegen_vc4value}"
...
log() { printf '[vc4value-candidate] %s\n' "$*"; }
fail() { printf '[vc4value-candidate] ERROR: %s\n' "$*" >&2; exit 1; }
...
check_fixture() {
  ...
  python3 - "$INPUT_MLIR" <<'PY_CHECK_VALUE_INPUT'
...
if "vc4value.kernel" not in text:
    raise SystemExit(f"{path}: VC4Value hardware input must contain vc4value.kernel")
if "vc4kernel." in text:
    raise SystemExit(f"{path}: VC4Value hardware input must not contain vc4kernel.*")
if "ssavc4." in text:
    raise SystemExit(f"{path}: VC4Value hardware input must not contain ssavc4.*")
if re.search(r'(?<![A-Za-z0-9_])"?vc4\.(?!value)', text):
    raise SystemExit(f"{path}: VC4Value hardware input must not contain scheduled vc4.*")
PY_CHECK_VALUE_INPUT
}
...
if data.get('schema_version') != 2:
    raise SystemExit('manifest schema_version 2 is required for VC4Value candidate assembly')

kernels = data.get('kernels')
if not isinstance(kernels, list) or not kernels:
    raise SystemExit('manifest must contain non-empty kernels[] for VC4Value candidate assembly')
...
for i, k in enumerate(kernels):
    ...
    if qasm == 'kernel.qasm':
        raise SystemExit('VC4Value bundles must use kernels[].qasm_path, not a root-level singleton QASM artifact')
...
run_vc4_codegen() {
  ...
  cp "$input_value" "$GENERATED_DIR/input.value.mlir"
...
select_harness_path() {
  ...
  if [[ "${VC4_ALLOW_REFERENCE_HARNESS_FALLBACK:-0}" != "1" ]]; then
    fail "VC4Value candidate fixture $TEST_NAME needs $(relpath "$candidate") using kernel_launch.h and vc4Malloc/vc4Memcpy APIs; reference harness fallback is disabled"
  fi

  log "WARNING: VC4_ALLOW_REFERENCE_HARNESS_FALLBACK=1; using legacy harness fallback for $TEST_NAME. This is debug-only and FORBIDDEN for normal VC4Value candidate verification."
...
lines = [
    "/* Generated by run_vc4value_candidate_codegen_test.sh; do not commit. */",
...
# Generated VC4Value program-bundle smoke workdir

Generated by compiler/test/CodeGen/VC4Value/Support/run_vc4value_candidate_codegen_test.sh.
...
try:
    ...
    try:
        ...
        try:
            ...
            pass
        process.wait()
    print(f"[vc4value-candidate] WARNING: Pi power cycle command timed out after {timeout}s", file=sys.stderr)
    sys.exit(124)
sys.exit(rc)
PY_POWER_CYCLE
      rc=$?
...
run_candidate() {
  ...
  while true; do
    ...
    if [[ "$run_rc" -ne 0 ]] && retry_reason="$(vc4_candidate_transient_preboot_failure_reason "$attempt_log")" && [[ "$attempt" -lt "$max_attempts" ]]; then
      ...
      printf '[vc4value-candidate] transient pre-boot failure matched: %s\n' "$retry_reason" >> "$attempt_log"
      attempt=$((attempt + 1))
      continue
    fi
...
try:
    ...
    print(
        f"[vc4value-candidate] ERROR: bash run.sh timed out after {int(timeout)} seconds; terminating hardware process group",
        file=sys.stderr,
    )
    try:
        os.killpg(process.pid, signal.SIGTERM)
    ...
    try:
        ...
        print(
            "[vc4value-candidate] ERROR: bash run.sh did not exit after SIGTERM; sending SIGKILL",
            file=sys.stderr,
        )
        try:
            os.killpg(process.pid, signal.SIGKILL)
...
EOF
  ...
  echo "run_vc4value_candidate_codegen_test.sh self-test PASS"
}
