...
CLAIMS_PATH = RUN_ROOT / "phase12_reduction_isolation_claims.json"
...
def fail(message: str) -> None:
    raise SystemExit(f"phase12 reduction isolation claim audit failed: {message}")
...
def audit_fixture(repo: Path, fixture: dict) -> None:
    ...
    for token in ("tt.func", "tt.load", "tt.store", "tt.reduce", "tt.make_range"):
    ...
    if "ttir_reduce_sum_i32" in name and "arith.addi" not in ttir_text:
        fail(f"{name}: i32 reduction snapshot does not contain integer add combiner")
    if "ttir_reduce_sum_i32" not in name and "arith.addf" not in ttir_text:
        fail(f"{name}: f32 reduction snapshot does not contain f32 add combiner")
    if "row_strided" in name and "arith.muli" not in ttir_text:
        fail(f"{name}: row-strided reduction snapshot does not contain scalar stride multiply")
    ...
    if "dot" in harness.lower() and "saw_no_dot_gemv=1" not in harness:
        fail(f"{name}: harness must not add dot/GEMV behavior")
...
def main(argv: list[str]) -> int:
    ...
    if not isinstance(fixtures, list) or len(fixtures) != 4:
        fail("claims file must list exactly four Phase 12 reduction isolation fixtures")
    ...
    if not isinstance(static_negative, list) or len(static_negative) != 3:
        fail("claims file must list non-add, rank>1, and dot/GEMV static negatives")
    for entry in static_negative:
        audit_static_negative(repo, entry)

    print("TTIR_REDUCTION_CLAIM_AUDIT=PASS")
