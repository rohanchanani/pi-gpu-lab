...
static int verify_results(uint32_t n, float *max_abs_diff) {
    ...
    for (uint32_t i = 0; i < n; i++) {
        uint32_t index = GUARD + i;
        float expected = f16_to_f32(in_values[index]) + 1.25f;
        float diff = out_values[index] - expected;
        float ad = absf_local(diff);
        if (ad > *max_abs_diff) *max_abs_diff = ad;
        if (ad > F32_TOLERANCE) {
            if (mismatches < 8)
                printk("ERROR: ttir f16 load f32 compute n=%d i=%d gpu=%f cpu=%f diff=%f\n",
                       (int)n, (int)i, out_values[index], expected, diff);
            mismatches++;
        }
    }
    return mismatches;
...
static int verify_sentinels(uint32_t n) {
    ...
    for (uint32_t i = 0; i < BUFFER_N; i++) {
        ...
        if (float_to_bits(out_values[i]) != SENTINEL_BITS) {
            if (mismatches < 8)
                printk("ERROR: ttir f16 load f32 sentinel n=%d i=%d bits=%x expected=%x\n",
...
void notmain(void) {
    ...
    if (vc4_program_create(&program, 0) < 0 || !program) panic("ttir f16 load create failed");
    ...
    if (vc4_m2_malloc(program, &in_dev, in_bytes) < 0 ||
        ...
        panic("ttir f16 load allocation failed");
