...
CLAIMS_PATH = RUN_ROOT / "phase17_online_softmax_isolation_claims.json"
...
def fail(message: str) -> None:
    raise SystemExit(f"phase17 TTIR online-softmax claim audit failed: {message}")
...
def audit_fixture(repo: Path, fixture: dict) -> None:
    ...
    require_contains(source_text, "tl.range", f"{name} Triton source")
    ...
    if "tl.load(Scale" in source_text or "tl.load(SCALE" in source_text:
    ...
    for token in ("tt.func", "tt.load", "tt.store", "tt.reduce", "math.exp", "scf.for"):
...
def main(argv: list[str]) -> int:
    ...
    if not isinstance(fixtures, list) or len(fixtures) != 3:
        fail("claims file must list exactly three Phase 17 TTIR isolation fixtures")
    ...
    if not isinstance(static_negative, list) or len(static_negative) != 5:
        fail("claims file must list scalar-load, nontransposed-V, K=0, QK, and dot negatives")
    for entry in static_negative:
        audit_static_negative(repo, entry)

    print("TTIR_ONLINE_SOFTMAX_CLAIM_AUDIT=PASS")
    print(f"fixtures={len(fixtures)}")
    print(f"static_negative={len(static_negative)}")
    print("ACTIVE_QPUS=12")
    print("TTIR_HARNESS_BLOCK_X=16")
