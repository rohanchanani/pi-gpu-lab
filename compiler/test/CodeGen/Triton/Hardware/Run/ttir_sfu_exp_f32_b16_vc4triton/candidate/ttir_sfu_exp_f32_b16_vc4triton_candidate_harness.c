...
#define EXP_ABS_TOL 0.0080f
#define EXP_REL_TOL 0.0120f
...
static float input_value(uint32_t case_id, uint32_t i) {
    static const float vals[] = {
        -4.0f, -3.5f, -3.0f, -2.0f, -1.25f, -0.5f, 0.0f, 0.5f,
        1.0f, 1.5f, 2.0f, 2.75f, 3.0f, 3.5f, 4.0f
    };
    return vals[(i + case_id * 5u) % (sizeof(vals) / sizeof(vals[0]))];
}
...
static float natural_exp_ref(float x) {
    int n = (int)(x * INV_LN2 + (x >= 0.0f ? 0.5f : -0.5f));
    ...
    return pow2_int_local(n) * sum;
}

static float raw_exp2_oracle(float x) {
    return pow2_int_local((int)x);
}

static float allowed_error(float expected) {
    return maxf_local(EXP_ABS_TOL, EXP_REL_TOL * absf_local(expected));
}
...
static int verify_results(uint32_t case_id, uint32_t n, float *max_abs_diff, float *max_rel_diff) {
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        float x = input_value(case_id, i);
        float expected = natural_exp_ref(x);
        float raw_exp2 = raw_exp2_oracle(x);
        if (absf_local(expected - raw_exp2) > allowed_error(expected))
        ...
        if (diff > allowed_error(expected)) {
            if (mismatches < 8)
                printk("ERROR: ttir_sfu_exp case=%d n=%d i=%d x=%f gpu=%f expected=%f raw_exp2=%f diff=%f rel=%f allowed=%f\n",
                       (int)case_id, (int)n, (int)i, x, out_values[index],
                       expected, raw_exp2, diff, rel, allowed_error(expected));
            mismatches++;
        }
    }
    if (!saw_distinguishing_value) {
        printk("ERROR: ttir_sfu_exp case=%d n=%d did not distinguish natural exp from raw exp2 oracle\n",
               (int)case_id, (int)n);
        mismatches++;
    }
    return mismatches;
...
static int verify_sentinels(uint32_t n) {
    ...
    for (uint32_t i = 0; i < BUFFER_N; i++) {
        ...
        if (float_to_bits(out_values[i]) != SENTINEL_BITS) {
            if (mismatches < 8)
                printk("ERROR: ttir_sfu_exp sentinel n=%d i=%d bits=%x expected=%x\n",
...
void notmain(void) {
    ...
    if (vc4_program_create(&program, 0) < 0 || !program)
        panic("ttir_sfu_exp_f32_b16 program create failed");
    ...
    if (vc4_m2_malloc(program, &in_dev, bytes) < 0 ||
        ...
        panic("ttir_sfu_exp_f32_b16 allocation failed");
    ...
    for (uint32_t case_id = 0; case_id < sizeof(n_cases) / sizeof(n_cases[0]); case_id++) {
        ...
        if (vc4_m2_copy_htod(program, in_dev, in_values, bytes) < 0 ||
            ...
            ttir_sfu_exp_f32_b16_launch(program, grid, block, in_active, out_active, n) < 0 ||
            vc4_m2_copy_dtoh(program, out_values, out_dev, bytes) < 0) {
            printk("ERROR: ttir_sfu_exp launch/copy failed case=%d n=%d\n",
                   (int)case_id, (int)n);
            launch_failures++;
            continue;
        }
        ...
        printk("TTIR_SFU_EXP_CASE case=%d n=%d waves=%d mismatches=%d sentinel_mismatches=%d hash=%x max_abs_diff=%f max_rel_diff=%f\n",
    ...
    printk("VC4_TEST_RESULT name=ttir_sfu_exp_f32_b16_vc4triton status=%s cases=%d total_mismatches=%d sentinel_mismatches=%d launch_failures=%d active_qpus=%d lanes=%d max_n=%d wave_boundary_n=%d output_hash=%u output_hash_nonzero=%d max_abs_diff=%f max_rel_diff=%f saw_real_ttir_snapshot=1 saw_cpp_ttir_importer=%d saw_ttir_approx_sfu=1 saw_value_approx_sfu=1 saw_ttir_approx_sfu_exp=1 saw_value_approx_sfu_exp=1 saw_ttir_tl_exp_natural=1 saw_value_math_exp_natural=1 saw_target_sfu_exp2_scale_log2e=1 saw_not_raw_exp2_oracle=1 saw_approx_math_policy=1 saw_no_exact_default_math=1 runtime_allocations=%d runtime_launches=%d elapsed_usec=%d\n",
           ...
           max_abs_diff, max_rel_diff, VC4_CASE_SAW_CPP_TTIR_IMPORTER, 2,
