...
static const uint32_t score_stride_cases[] = {16u, 17u, 23u, 67u};
static const uint32_t vt_stride_cases[] = {16u, 19u, 64u};
    ...
    for (uint32_t q = 0; q < q_rows; q++) {
        for (uint32_t d = 0; d < out_dims; d++) {
            ...
            if (diff > maxf_local(ATTN_ABS_TOL, ATTN_REL_TOL * absf_local(expected))) {
                if (mismatches < 8)
                    printk("ERROR: attention_f32 case=%d q=%d d=%d k=%d lds=%d ldv=%d got=%f expected=%f diff=%f rel=%f\n",
...
static int verify_sentinels(uint32_t q_rows, uint32_t out_dims, uint32_t k,
    ...
    for (uint32_t i = 0; i < SCORE_BUFFER_N; i++) {
        ...
        if (!active && float_to_bits(scores_values[i]) != SENTINEL_BITS) {
            if (mismatches < 8)
                printk("ERROR: attention_f32 score sentinel i=%d bits=%x\n", (int)i, float_to_bits(scores_values[i]));
            mismatches++;
        }
    }
    for (uint32_t i = 0; i < VT_BUFFER_N; i++) {
        ...
        if (!active && float_to_bits(vt_values[i]) != SENTINEL_BITS) {
            if (mismatches < 8)
                printk("ERROR: attention_f32 vt sentinel i=%d bits=%x\n", (int)i, float_to_bits(vt_values[i]));
            mismatches++;
        }
    }
    for (uint32_t i = 0; i < OUT_BUFFER_N; i++) {
        ...
        if (!active && float_to_bits(out_values[i]) != SENTINEL_BITS) {
            if (mismatches < 8)
                printk("ERROR: attention_f32 out sentinel i=%d bits=%x\n", (int)i, float_to_bits(out_values[i]));
            mismatches++;
        }
    }
    return mismatches;
...
void notmain(void) {
    ...
    if (vc4_program_create(&program, 0) < 0 || !program)
        panic("ttir_online_attention_apply_v0_f32 program create failed");
    ...
    if (vc4_m2_malloc(program, &scores_dev, scores_bytes) < 0 ||
        ...
        panic("ttir_online_attention_apply_v0_f32 allocation failed");
    ...
    for (uint32_t vi = 0; vi < sizeof(vt_stride_cases) / sizeof(vt_stride_cases[0]); vi++) {
        ...
        if (vc4_m2_copy_htod(program, scores_dev, scores_values, scores_bytes) < 0 ||
            ...
            ttir_online_attention_apply_v0_f32_b16_kernel_launch(
            ...
            printk("ERROR: attention_f32 launch/copy failed case=%d q_rows=%d out_dims=%d k=%d lds=%d ldv=%d ldo=%d\n",
        ...
        printk("TTIR_ONLINE_ATTENTION_APPLY_F32_CASE case=%d q_rows=%d out_dims=%d k=%d lds=%d ldv=%d ldo=%d mismatches=%d sentinel_mismatches=%d hash=%x max_abs_diff=%f max_rel_diff=%f\n",
    ...
    printk("VC4_TEST_RESULT name=ttir_online_attention_apply_v0_f32_b16_vc4triton status=%s cases=%d total_mismatches=%d sentinel_mismatches=%d launch_failures=%d active_qpus=%d lanes=%d block_x=%d max_q_rows=%d max_out_dims=%d max_k=%d output_hash=%u output_hash_nonzero=%d max_abs_diff=%f max_rel_diff=%f saw_real_ttir_snapshot=1 saw_cpp_ttir_importer=%d saw_ttir_online_softmax_state=1 saw_ttir_online_attention_apply=1 saw_ttir_precomputed_scores=1 saw_ttir_transposed_v_layout=1 saw_ttir_loop_carried_m_l_acc_state=1 saw_ttir_k_gt_16=1 saw_ttir_weighted_sum_reduction=1 saw_ttir_scalar_result_store=1 saw_softmax_uses_natural_exp=1 saw_ttir_approx_sfu_exp=1 saw_ttir_approx_sfu_recip_div=1 saw_no_scalar_global_load=1 saw_no_nontransposed_v_gather=1 saw_no_qk_score_generation=1 saw_no_full_attention=1 runtime_allocations=%d runtime_launches=%d elapsed_usec=%d\n",
