...
#define MAX_COVERAGE_N (((MAX_N + ELEMENTS_PER_WAVE - 1u) / ELEMENTS_PER_WAVE) * ELEMENTS_PER_WAVE)
...
static const uint32_t n_cases[] = {
    0u, 1u, 2u, 15u, 16u, 17u, 31u, 32u, 33u, 192u, 1000u
};

static float in_values[BUFFER_N];
static uint16_t out_values[BUFFER_N];
static uint16_t scratch_out[SCRATCH_N];
...
static void fill_buffers(uint32_t n) {
    for (uint32_t i = 0; i < BUFFER_N; i++) {
        ...
        in_values[i] = f16_to_f32(base) - 2.5f;
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        in_values[index] = f16_to_f32(expected) - 2.5f;
    }
}
...
static int verify_results(uint32_t n) {
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (out_values[index] != expected) {
            if (mismatches < 8)
                printk("ERROR: ttir f32 compute f16 store n=%d i=%d got=%x expected=%x\n",
                       (int)n, (int)i, out_values[index], expected);
            mismatches++;
        }
    }
    return mismatches;
...
static int verify_sentinels(uint32_t n) {
    ...
    for (uint32_t i = 0; i < BUFFER_N; i++) {
        ...
        if (out_values[i] != OUT_SENTINEL) {
            if (mismatches < 8)
                printk("ERROR: ttir f32 compute f16 sentinel n=%d i=%d bits=%x\n",
...
void notmain(void) {
    ...
    if (vc4_program_create(&program, 0) < 0 || !program) panic("ttir f16 store create failed");
    ...
    uint32_t scratch_bytes = SCRATCH_N * sizeof(uint16_t);
    if (vc4_m2_malloc(program, &in_dev, in_bytes) < 0 ||
        ...
        panic("ttir f16 store allocation failed");

    int total_mismatches = 0, sentinel_mismatches = 0, launch_failures = 0;
    int elements_checked = 0, runtime_launches = 0;
    ...
    for (uint32_t n_id = 0; n_id < sizeof(n_cases) / sizeof(n_cases[0]); n_id++) {
        ...
        if (n == 0u) {
            if (ttir_f32_compute_store_f16_b16_launch(program, grid, block,
                                                     in_dev, scratch_dev) < 0)
                launch_failures++;
            runtime_launches++;
        } else {
            for (uint32_t base = 0; base < coverage; base += ELEMENTS_PER_WAVE) {
                vc4_deviceptr_t in_active = in_dev + (GUARD + base) * sizeof(float);
                vc4_deviceptr_t out_active = out_dev + (GUARD + base) * sizeof(uint16_t);
                if (ttir_f32_compute_store_f16_b16_launch(program, grid, block,
        ...
        printk("TTIR_F32_COMPUTE_F16_STORE_CASE n=%d coverage=%d mismatches=%d sentinel_mismatches=%d hash=%x\n",
               (int)n, (int)coverage, mismatches, sentinels, case_hash);
    }

    int elapsed = timer_get_usec() - start;
    const char *status = (total_mismatches == 0 && sentinel_mismatches == 0 &&
                          launch_failures == 0 && output_hash != 0u) ? "PASS" : "FAIL";
    printk("VC4_TEST_RESULT name=ttir_f32_compute_store_f16_b16_vc4triton status=%s cases=%d elements_checked=%d total_mismatches=%d sentinel_mismatches=%d launch_failures=%d active_qpus=%d lanes=%d max_n=%d max_coverage_n=%d buffer_n=%d output_hash=%u output_hash_nonzero=%d saw_real_ttir_snapshot=1 saw_cpp_ttir_importer=%d saw_ttir_f16_storage=1 saw_value_f16_storage=1 saw_ttir_f16_storage_store=1 saw_value_f16_storage_store=1 saw_f16_storage_finite_policy=1 saw_no_native_f16_arithmetic=1 saw_no_softmax_sfu=1 runtime_allocations=%d runtime_launches=%d elapsed_usec=%d\n",
           ...
           output_hash != 0u ? 1 : 0, VC4_CASE_SAW_CPP_TTIR_IMPORTER,
           3, runtime_launches, elapsed);
