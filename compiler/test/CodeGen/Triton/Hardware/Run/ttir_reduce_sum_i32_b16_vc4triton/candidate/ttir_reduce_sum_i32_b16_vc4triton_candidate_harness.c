...
static int verify_case(uint32_t case_id, uint32_t n) {
    ...
    for (uint32_t b = 0; b < blocks; b++) {
        ...
        if (out_values[index] != expected) {
            if (mismatches < 8)
                printk("ERROR: ttir i32 reduction case=%d n=%d block=%d got=%d expected=%d\n",
...
static int verify_sentinels(uint32_t n) {
    ...
    for (uint32_t i = 0; i < OUT_BUFFER_N; i++) {
        ...
        if ((uint32_t)out_values[i] != SENTINEL) {
            if (mismatches < 8)
                printk("ERROR: ttir i32 reduction sentinel n=%d i=%d got=%x\n",
...
void notmain(void) {
    ...
    if (vc4_program_create(&program, 0) < 0 || !program)
        panic("ttir_reduce_sum_i32_b16 program create failed");
    ...
    if (vc4_m2_malloc(program, &in_dev, in_bytes) < 0 ||
        ...
        panic("ttir_reduce_sum_i32_b16 allocation failed");
    ...
    for (uint32_t case_id = 0; case_id < sizeof(n_cases) / sizeof(n_cases[0]); case_id++) {
        ...
        if (vc4_m2_copy_htod(program, in_dev, in_values, in_bytes) < 0 ||
            ...
            ttir_reduce_sum_i32_b16_kernel_launch(program, grid, block,
                                                  in_active, out_active, n) < 0 ||
            vc4_m2_copy_dtoh(program, out_values, out_dev, out_bytes) < 0) {
            printk("ERROR: ttir i32 reduction launch/copy failed case=%d n=%d\n",
                   (int)case_id, (int)n);
            launch_failures++;
            continue;
        }
        ...
        printk("TTIR_REDUCE_I32_CASE case=%d n=%d logical_blocks=%d launch_blocks=%d mismatches=%d sentinel_mismatches=%d hash=%x\n",
    ...
    printk("VC4_TEST_RESULT name=ttir_reduce_sum_i32_b16_vc4triton status=%s cases=%d total_mismatches=%d sentinel_mismatches=%d launch_failures=%d active_qpus=%d lanes=%d max_n=%d wave_boundary_n=%d output_hash=%u output_hash_nonzero=%d saw_real_ttir_snapshot=1 saw_cpp_ttir_importer=%d saw_ttir_reduction=1 saw_value_reduction=1 saw_ttir_reduce_i32_add=1 saw_ttir_scalar_reduction_store=1 saw_tail_inactive_zero_input=1 saw_no_dot_gemv=1 runtime_allocations=%d runtime_launches=%d elapsed_usec=%d\n",
           ...
           output_hash != 0u ? 1 : 0, VC4_CASE_SAW_CPP_TTIR_IMPORTER, 2,
