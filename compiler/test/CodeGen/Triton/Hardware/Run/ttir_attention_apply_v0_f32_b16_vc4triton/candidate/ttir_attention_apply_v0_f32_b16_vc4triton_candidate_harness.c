...
void notmain(void) {
    ...
    if (vc4_program_create(&program, 0) < 0 || !program)
        panic("ttir_attention_apply_v0_f32 program create failed");
    ...
    if (vc4_m2_malloc(program, &scores_dev, scores_bytes) < 0 ||
        ...
        panic("ttir_attention_apply_v0_f32 allocation failed");
    ...
    for (uint32_t vi = 0; vi < sizeof(vt_stride_cases) / sizeof(vt_stride_cases[0]); vi++) {
        ...
        if (vc4_m2_copy_htod(program, scores_dev, scores_values, scores_bytes) < 0 ||
            ...
            ttir_attention_apply_v0_f32_b16_kernel_launch(
        ...
        printk("TTIR_ATTENTION_APPLY_F32_CASE case=%d q_rows=%d out_dims=%d k=%d lds=%d ldv=%d ldo=%d block_x=%d mismatches=%d sentinel_mismatches=%d hash=%x max_abs_diff=%f max_rel_diff=%f\n",
               (int)case_id, (int)q_rows, (int)out_dims, (int)k, (int)lds,
               (int)ldv, (int)ldo, (int)LANES, mismatches, sentinels, hash,
    ...
    printk("VC4_TEST_RESULT name=ttir_attention_apply_v0_f32_b16_vc4triton status=%s cases=%d total_mismatches=%d sentinel_mismatches=%d launch_failures=%d active_qpus=%d lanes=%d block_x=%d max_q_rows=%d max_out_dims=%d max_k=%d output_hash=%u output_hash_nonzero=%d max_abs_diff=%f max_rel_diff=%f saw_real_ttir_snapshot=1 saw_cpp_ttir_importer=%d saw_ttir_attention_apply_v0=1 saw_ttir_precomputed_scores=1 saw_ttir_transposed_v_layout=1 saw_ttir_softmax_v0=1 saw_ttir_weighted_sum_reduction=1 saw_ttir_scalar_result_store=1 saw_softmax_uses_natural_exp=1 saw_no_scalar_global_load=1 saw_no_nontransposed_v_gather=1 saw_no_qk_score_generation=1 saw_no_full_attention=1 runtime_allocations=%d runtime_launches=%d elapsed_usec=%d\n",
