...
static float expected_attention(uint32_t case_id, uint32_t q, uint32_t d, uint32_t k,
    ...
    float maxv = -80.0f;
    for (uint32_t j = 0; j < k; j++) {
        float s = scores_values[GUARD + q * lds + j] * scale;
        if (s > maxv)
            maxv = s;
    }
    float denom = 0.0f;
    for (uint32_t j = 0; j < k; j++)
        denom += natural_exp_ref(scores_values[GUARD + q * lds + j] * scale - maxv);

    float acc = 0.0f;
    for (uint32_t j = 0; j < k; j++) {
        float prob = natural_exp_ref(scores_values[GUARD + q * lds + j] * scale - maxv) / denom;
    ...
    for (uint32_t q = 0; q < q_rows; q++) {
        for (uint32_t d = 0; d < out_dims; d++) {
            ...
            if (diff > maxf_local(ATTN_ABS_TOL, ATTN_REL_TOL * absf_local(expected))) {
                if (mismatches < 8)
                    printk("ERROR: attention_scaled case=%d q=%d d=%d k=%d lds=%d ldv=%d got=%f expected=%f diff=%f rel=%f\n",
...
void notmain(void) {
    ...
    if (vc4_program_create(&program, 0) < 0 || !program)
        panic("ttir_online_attention_apply_v0_scaled program create failed");
    ...
    if (vc4_m2_malloc(program, &scores_dev, scores_bytes) < 0 ||
        ...
        panic("ttir_online_attention_apply_v0_scaled allocation failed");
    ...
    for (uint32_t vi = 0; vi < sizeof(vt_stride_cases) / sizeof(vt_stride_cases[0]); vi++) {
        ...
        if (vc4_m2_copy_htod(program, scores_dev, scores_values, scores_bytes) < 0 ||
            ...
            ttir_online_attention_apply_v0_scaled_f32_b16_kernel_launch(
        ...
        printk("TTIR_ONLINE_ATTENTION_APPLY_SCALED_F32_CASE case=%d q_rows=%d out_dims=%d k=%d lds=%d ldv=%d ldo=%d scale=%f mismatches=%d sentinel_mismatches=%d hash=%x max_abs_diff=%f max_rel_diff=%f\n",
               (int)case_id, (int)q_rows, (int)out_dims, (int)k, (int)lds,
               (int)ldv, (int)ldo, scale, mismatches, sentinels, hash, max_abs_diff,
               max_rel_diff);
        case_id++;
    }
    ...
    printk("VC4_TEST_RESULT name=ttir_online_attention_apply_v0_scaled_f32_b16_vc4triton status=%s cases=%d total_mismatches=%d sentinel_mismatches=%d launch_failures=%d active_qpus=%d lanes=%d block_x=%d max_q_rows=%d max_out_dims=%d max_k=%d output_hash=%u output_hash_nonzero=%d max_abs_diff=%f max_rel_diff=%f saw_real_ttir_snapshot=1 saw_cpp_ttir_importer=%d saw_ttir_online_softmax_state=1 saw_ttir_online_attention_apply=1 saw_ttir_attention_apply_scaled_scores=1 saw_scalar_scale_arg=1 saw_no_scalar_global_load=1 saw_ttir_precomputed_scores=1 saw_ttir_transposed_v_layout=1 saw_ttir_loop_carried_m_l_acc_state=1 saw_ttir_k_gt_16=1 saw_ttir_weighted_sum_reduction=1 saw_ttir_scalar_result_store=1 saw_softmax_uses_natural_exp=1 saw_ttir_approx_sfu_exp=1 saw_ttir_approx_sfu_recip_div=1 saw_no_nontransposed_v_gather=1 saw_no_qk_score_generation=1 saw_no_full_attention=1 runtime_allocations=%d runtime_launches=%d elapsed_usec=%d\n",
