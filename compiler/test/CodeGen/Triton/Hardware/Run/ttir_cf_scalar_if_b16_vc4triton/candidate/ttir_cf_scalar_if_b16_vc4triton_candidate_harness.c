...
static const int32_t flag_cases[] = {-3, 0, 7};
...
static void fill_buffers(uint32_t n, int32_t flag) {
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        float expected = flag > 0 ? x_values[index] + y_values[index]
...
static int verify_results(uint32_t n, int32_t flag) {
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (got != expected) {
            if (mismatches < 8)
                printk("ERROR: ttir_cf_scalar_if n=%d flag=%d i=%d gpu_bits=%x expected_bits=%x gpu=%f expected=%f\n",
                       (int)n, (int)flag, (int)i, got, expected,
...
static int verify_sentinels(uint32_t n) {
    ...
    for (uint32_t i = 0; i < BUFFER_N; i++) {
        ...
        if (got != SENTINEL_BITS) {
            if (mismatches < 8)
                printk("ERROR: ttir_cf_scalar_if sentinel n=%d i=%d bits=%x expected=%x\n",
...
void notmain(void) {
    ...
    if (vc4_m2_malloc(program, &x_dev, bytes) < 0 ||
        ...
        panic("ttir_cf_scalar_if allocation failed");
    ...
    for (uint32_t flag_id = 0; flag_id < sizeof(flag_cases) / sizeof(flag_cases[0]); flag_id++) {
        for (uint32_t n_id = 0; n_id < sizeof(n_cases) / sizeof(n_cases[0]); n_id++) {
            int32_t flag = flag_cases[flag_id];
            ...
            if (vc4_m2_copy_htod(program, x_dev, x_values, bytes) < 0 ||
                ...
                ttir_cf_scalar_if_b16_kernel_launch(program, grid, block, x_active,
                                                    y_active, out_active, n, flag) < 0 ||
                vc4_m2_copy_dtoh(program, out_values, out_dev, bytes) < 0) {
                printk("ERROR: ttir_cf_scalar_if launch/copy failed flag=%d n=%d\n",
                       (int)flag, (int)n);
                launch_failures++;
                continue;
            }
            ...
            output_hash ^= case_hash + 0x9e3779b9u + (flag_id << 12) + (n_id << 4);
            output_hash = rotl32_local(output_hash, 7u);
            total_mismatches += mismatches;
            sentinel_mismatches += sentinels;
            elements_checked += (int)n;
            printk("TTIR_CF_SCALAR_IF_CASE flag=%d n=%d waves=%d coverage=%d mismatches=%d sentinel_mismatches=%d hash=%x\n",
                   (int)flag, (int)n, (int)waves, (int)coverage,
                   mismatches, sentinels, case_hash);
        }
    }

    int elapsed = timer_get_usec() - start;
    int cases = (int)((sizeof(flag_cases) / sizeof(flag_cases[0])) *
                      (sizeof(n_cases) / sizeof(n_cases[0])));
    const char *status = (total_mismatches == 0 && sentinel_mismatches == 0 &&
                          launch_failures == 0 && output_hash != 0u) ? "PASS" : "FAIL";
    printk("VC4_TEST_RESULT name=ttir_cf_scalar_if_b16_vc4triton status=%s cases=%d elements_checked=%d total_mismatches=%d sentinel_mismatches=%d launch_failures=%d active_qpus=%d lanes=%d max_n=%d max_coverage_n=%d buffer_n=%d output_hash=%u saw_nonzero_output_hash=%d saw_real_ttir_snapshot=%d saw_cpp_ttir_importer=%d saw_value_scf_cf=%d saw_vc4value_to_vc4kernel=%d saw_ttir_scf_if=%d saw_tail_mask_load_store=%d saw_cpu_oracle_sentinel=%d saw_no_arith_sitofp=%d runtime_allocations=%d runtime_launches=%d elapsed_usec=%d\n",
