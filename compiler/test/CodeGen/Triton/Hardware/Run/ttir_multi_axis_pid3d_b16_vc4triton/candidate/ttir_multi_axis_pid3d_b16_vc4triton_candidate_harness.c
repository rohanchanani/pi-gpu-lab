...
static int32_t expected_value(uint32_t pid0, uint32_t pid1, uint32_t pid2, uint32_t lane) {
    return (int32_t)(pid2 * 1000000u + pid1 * 10000u + pid0 * 100u + lane);
}
            ...
            for (uint32_t pid0 = 0; pid0 < grid_x; pid0++) {
                ...
                for (uint32_t lane = 0; lane < LANES; lane++) {
                    ...
                    if (out_values[index] != expected) {
                        if (mismatches < 8)
                            printk("ERROR: ttir pid3d grid=%d,%d,%d pid0=%d pid1=%d pid2=%d lane=%d got=%x expected=%x\n",
...
static int verify_sentinels(uint32_t elements) {
    ...
    for (uint32_t i = 0; i < BUFFER_N; i++) {
        ...
        if ((uint32_t)out_values[i] != SENTINEL) {
            if (mismatches < 8)
                printk("ERROR: ttir pid3d sentinel elements=%d i=%d got=%x expected=%x\n",
...
void notmain(void) {
    ...
    if (vc4_m2_malloc(program, &out_dev, bytes) < 0)
        panic("ttir pid3d allocation failed");
    ...
    for (uint32_t case_id = 0; case_id < sizeof(cases) / sizeof(cases[0]); case_id++) {
        ...
        if (vc4_m2_copy_htod(program, out_dev, out_values, bytes) < 0 ||
            ttir_multi_axis_pid3d_b16_kernel_launch(program, grid, block, out_active, elements) < 0 ||
            vc4_m2_copy_dtoh(program, out_values, out_dev, bytes) < 0) {
            printk("ERROR: ttir pid3d launch/copy failed case=%d grid=%d,%d,%d\n",
        ...
        printk("TTIR_MULTI_AXIS_PID3D_CASE case=%d grid_x=%d grid_y=%d grid_z=%d elements=%d mismatches=%d sentinel_mismatches=%d hash=%x\n",
               (int)case_id, (int)grid_x, (int)grid_y, (int)grid_z,
