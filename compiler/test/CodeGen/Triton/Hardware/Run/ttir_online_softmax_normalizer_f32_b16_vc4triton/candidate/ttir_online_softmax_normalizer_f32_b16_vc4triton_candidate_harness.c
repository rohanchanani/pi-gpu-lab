...
static const uint32_t stride_cases[] = {16u, 17u, 23u, 67u};
...
static float expected_inv_l(uint32_t case_id, uint32_t row, uint32_t k, uint32_t stride) {
    float maxv = -80.0f;
    for (uint32_t j = 0; j < k; j++) {
...
static int verify_outputs(uint32_t case_id, uint32_t rows, uint32_t k,
    ...
    for (uint32_t r = 0; r < rows; r++) {
        ...
        float expected = expected_inv_l(case_id, r, k, stride);
        ...
        if (diff > maxf_local(DENOM_ABS_TOL, DENOM_REL_TOL * absf_local(expected))) {
            if (mismatches < 8)
                printk("ERROR: ttir_online_normalizer case=%d row=%d k=%d stride=%d got=%f expected=%f diff=%f rel=%f\n",
...
static int verify_sentinels(uint32_t rows, uint32_t k, uint32_t stride) {
    ...
    for (uint32_t i = 0; i < SCORE_BUFFER_N; i++) {
        ...
        if (!active && float_to_bits(scores_values[i]) != SENTINEL_BITS) {
            if (mismatches < 8)
                printk("ERROR: ttir_online_normalizer score sentinel i=%d bits=%x\n", (int)i, float_to_bits(scores_values[i]));
            mismatches++;
        }
    }
    for (uint32_t i = 0; i < OUT_BUFFER_N; i++) {
        ...
        if (!active && float_to_bits(out_values[i]) != SENTINEL_BITS) {
            if (mismatches < 8)
                printk("ERROR: ttir_online_normalizer out sentinel i=%d bits=%x\n", (int)i, float_to_bits(out_values[i]));
            mismatches++;
        }
    }
    return mismatches;
...
void notmain(void) {
    ...
    if (vc4_program_create(&program, 0) < 0 || !program)
        panic("ttir_online_softmax_normalizer program create failed");
    ...
    if (vc4_m2_malloc(program, &scores_dev, scores_bytes) < 0 ||
        ...
        panic("ttir_online_softmax_normalizer allocation failed");
    ...
    for (uint32_t si = 0; si < sizeof(stride_cases) / sizeof(stride_cases[0]); si++) {
        ...
        if (vc4_m2_copy_htod(program, scores_dev, scores_values, scores_bytes) < 0 ||
            ...
            ttir_online_softmax_normalizer_f32_b16_kernel_launch(
            ...
            printk("ERROR: ttir_online_normalizer launch/copy failed case=%d rows=%d k=%d stride=%d\n",
        ...
        printk("TTIR_ONLINE_SOFTMAX_NORMALIZER_CASE case=%d rows=%d k=%d stride=%d mismatches=%d sentinel_mismatches=%d hash=%x max_abs_diff=%f max_rel_diff=%f\n",
    ...
    printk("VC4_TEST_RESULT name=ttir_online_softmax_normalizer_f32_b16_vc4triton status=%s cases=%d total_mismatches=%d sentinel_mismatches=%d launch_failures=%d active_qpus=%d lanes=%d block_x=%d max_rows=%d max_k=%d output_hash=%u output_hash_nonzero=%d max_abs_diff=%f max_rel_diff=%f saw_real_ttir_snapshot=1 saw_cpp_ttir_importer=%d saw_ttir_online_softmax_state=1 saw_ttir_loop_carried_m_l_state=1 saw_ttir_k_gt_16=1 saw_softmax_uses_natural_exp=1 saw_ttir_finite_f32_max_reduction=1 saw_ttir_reduction_f32_finite_add=1 saw_ttir_scalar_result_store=1 saw_no_scalar_global_load=1 saw_no_nontransposed_v_gather=1 saw_no_qk_score_generation=1 saw_no_full_attention=1 runtime_allocations=%d runtime_launches=%d elapsed_usec=%d\n",
           status, (int)case_id, total_mismatches, sentinel_mismatches,
           launch_failures, ACTIVE_QPUS, LANES, LANES, MAX_ROWS, MAX_K, output_hash,
           output_hash != 0u ? 1 : 0, max_abs_diff, max_rel_diff,
           VC4_CASE_SAW_CPP_TTIR_IMPORTER, 2, (int)case_id, elapsed);
