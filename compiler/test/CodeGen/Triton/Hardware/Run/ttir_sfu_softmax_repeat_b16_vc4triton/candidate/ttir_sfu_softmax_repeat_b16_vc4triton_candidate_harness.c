    ...
    for (uint32_t r = 0; r < launched_rows(tc); r++) {
        ...
        for (uint32_t c = 0; c < tc->ncols; c++) {
            ...
            if (diff > maxf_local(SOFTMAX_ABS_TOL, SOFTMAX_REL_TOL * absf_local(expected))) {
                if (mismatches < 8)
                    printk("ERROR: ttir_sfu_softmax_repeat case=%d row=%d col=%d got=%f expected=%f diff=%f rel=%f\n",
        ...
        if (sum_diff > ROW_SUM_TOL) {
            if (mismatches < 8)
                printk("ERROR: ttir_sfu_softmax_repeat row_sum case=%d row=%d sum=%f diff=%f\n",
...
static int verify_sentinels(const struct case_desc *tc) {
    ...
    for (uint32_t i = 0; i < BUFFER_N; i++) {
        ...
        if (float_to_bits(y_values[i]) != SENTINEL_BITS) {
            if (mismatches < 8)
                printk("ERROR: ttir_sfu_softmax_repeat sentinel rows=%d ncols=%d stride=%d i=%d bits=%x expected=%x\n",
...
void notmain(void) {
    ...
    if (vc4_program_create(&program, 0) < 0 || !program)
        panic("ttir_sfu_softmax_repeat_b16 program create failed");
    ...
    if (vc4_m2_malloc(program, &x_dev, bytes) < 0 ||
        ...
        panic("ttir_sfu_softmax_repeat_b16 allocation failed");
    ...
    for (uint32_t case_id = 0; case_id < sizeof(cases) / sizeof(cases[0]); case_id++) {
        ...
        if (vc4_m2_copy_htod(program, x_dev, x_values, bytes) < 0 ||
            ...
            printk("ERROR: ttir_sfu_softmax_repeat launch/copy failed case=%d rows=%d ncols=%d stride=%d\n",
        ...
        printk("TTIR_SFU_SOFTMAX_REPEAT_CASE case=%d rows=%d ncols=%d stride=%d mismatches=%d sentinel_mismatches=%d hash=%x max_abs_diff=%f max_rel_diff=%f max_row_sum_diff=%f\n",
               (int)case_id, (int)tc->rows, (int)tc->ncols, (int)tc->stride,
    ...
    printk("VC4_TEST_RESULT name=ttir_sfu_softmax_repeat_b16_vc4triton status=%s cases=%d total_mismatches=%d sentinel_mismatches=%d launch_failures=%d active_qpus=%d lanes=%d max_rows=%d max_ncols=%d output_hash=%u output_hash_nonzero=%d max_abs_diff=%f max_rel_diff=%f max_row_sum_diff=%f saw_real_ttir_snapshot=1 saw_cpp_ttir_importer=%d saw_ttir_approx_sfu=1 saw_value_approx_sfu=1 saw_ttir_softmax_v0=1 saw_value_softmax_v0=1 saw_ttir_approx_sfu_exp=1 saw_value_approx_sfu_exp=1 saw_ttir_approx_sfu_recip_div=1 saw_value_approx_sfu_recip_div=1 saw_ttir_finite_f32_max_reduction=1 saw_value_finite_f32_max_reduction=1 saw_scalar_to_vector_f32_broadcast=1 saw_approx_math_policy=1 saw_no_exact_default_math=1 saw_zero_active_softmax_staged=1 saw_softmax_uses_natural_exp=1 saw_target_sfu_exp2_scale_log2e=1 saw_repeat_invocation=1 runtime_allocations=%d runtime_launches=%d elapsed_usec=%d\n",
