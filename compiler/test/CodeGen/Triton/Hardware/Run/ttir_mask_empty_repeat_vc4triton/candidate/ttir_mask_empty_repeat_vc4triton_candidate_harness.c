...
static const uint32_t n_cases[] = {0u, 17u, 0u, 33u, 192u, 0u, 193u};
...
static float x_value(uint32_t i, uint32_t case_id) {
    int32_t whole = (int32_t)(((i + case_id * 13u) * 31u + 9u) % 137u) - 68;
    uint32_t quarter = (i * 7u + case_id) & 3u;
...
static void fill_buffers(uint32_t case_id) {
    ...
    for (uint32_t i = 0; i < BUFFER_N; i++) {
        ...
        x_values[i] = x_value(logical, case_id);
...
static int verify_results(uint32_t n) {
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (got != expected) {
            if (mismatches < 8)
                printk("ERROR: ttir_empty_repeat n=%d i=%d gpu_bits=%x expected_bits=%x\n",
...
static int verify_sentinels(uint32_t n) {
    ...
    for (uint32_t i = 0; i < BUFFER_N; i++) {
        ...
        if (got != SENTINEL_BITS) {
            if (mismatches < 8)
                printk("ERROR: ttir_empty_repeat sentinel n=%d i=%d bits=%x expected=%x\n",
...
void notmain(void) {
    ...
    if (vc4_m2_malloc(program, &x_dev, bytes) < 0 ||
        ...
        panic("ttir_empty_repeat allocation failed");
    ...
    for (uint32_t case_id = 0; case_id < sizeof(n_cases) / sizeof(n_cases[0]); case_id++) {
        ...
        if (vc4_m2_copy_htod(program, x_dev, x_values, bytes) < 0 ||
            ...
            printk("ERROR: ttir_empty_repeat launch/copy failed case=%d n=%d\n",
                   (int)case_id, (int)n);
            launch_failures++;
            continue;
        }
        ...
        printk("TTIR_MASK_EMPTY_REPEAT_CASE case=%d n=%d waves=%d coverage=%d mismatches=%d sentinel_mismatches=%d hash=%x\n",
    ...
    printk("VC4_TEST_RESULT name=ttir_mask_empty_repeat_vc4triton status=%s cases=%d elements_checked=%d total_mismatches=%d sentinel_mismatches=%d launch_failures=%d active_qpus=%d lanes=%d max_n=%d max_coverage_n=%d buffer_n=%d output_hash=%u output_hash_nonzero=%d saw_real_ttir_snapshot=%d saw_cpp_ttir_importer=%d saw_ttir_mask_memory=%d saw_value_mask_classifier=%d saw_ttir_empty_mask=%d saw_repeat_invocation=%d saw_value_load_inactive_zero=%d saw_value_store_inactive_preserve=%d runtime_allocations=%d runtime_launches=%d elapsed_usec=%d\n",
           ...
           1, VC4_CASE_SAW_CPP_TTIR_IMPORTER, 1, 1, saw_empty,
           saw_nonempty_after_empty, 1, 1, 2,
