...
static int verify_case(uint32_t case_id, uint32_t n, float *max_abs_diff) {
    ...
    for (uint32_t b = 0; b < blocks; b++) {
        ...
        if (diff > F32_TOLERANCE) {
            if (mismatches < 8)
                printk("ERROR: ttir f32 reduction case=%d n=%d block=%d got=%f expected=%f diff=%f\n",
...
static int verify_sentinels(uint32_t n) {
    int mismatches = 0;
    uint32_t blocks = launch_blocks(n);
    for (uint32_t i = 0; i < OUT_BUFFER_N; i++) {
        if (i >= GUARD && i < GUARD + blocks)
            continue;
        if (float_to_bits(out_values[i]) != SENTINEL_BITS) {
            if (mismatches < 8)
                printk("ERROR: ttir f32 reduction sentinel n=%d i=%d got_bits=%x\n",
...
void notmain(void) {
    ...
    if (vc4_program_create(&program, 0) < 0 || !program)
        panic("ttir_reduce_sum_f32_b16 program create failed");
    ...
    if (vc4_m2_malloc(program, &in_dev, in_bytes) < 0 ||
        ...
        panic("ttir_reduce_sum_f32_b16 allocation failed");
    ...
    for (uint32_t case_id = 0; case_id < sizeof(n_cases) / sizeof(n_cases[0]); case_id++) {
        ...
        if (vc4_m2_copy_htod(program, in_dev, in_values, in_bytes) < 0 ||
            ...
            ttir_reduce_sum_f32_b16_kernel_launch(program, grid, block,
                                                  in_active, out_active, n) < 0 ||
            vc4_m2_copy_dtoh(program, out_values, out_dev, out_bytes) < 0) {
            printk("ERROR: ttir f32 reduction launch/copy failed case=%d n=%d\n",
                   (int)case_id, (int)n);
            launch_failures++;
            continue;
        }
        ...
        printk("TTIR_REDUCE_F32_CASE case=%d n=%d logical_blocks=%d launch_blocks=%d mismatches=%d sentinel_mismatches=%d hash=%x max_abs_diff=%f\n",
               (int)case_id, (int)n, (int)logical_blocks(n), (int)launch_blocks(n),
    ...
    printk("VC4_TEST_RESULT name=ttir_reduce_sum_f32_b16_vc4triton status=%s cases=%d total_mismatches=%d sentinel_mismatches=%d launch_failures=%d active_qpus=%d lanes=%d max_n=%d wave_boundary_n=%d output_hash=%u output_hash_nonzero=%d max_abs_diff=%f saw_real_ttir_snapshot=1 saw_cpp_ttir_importer=%d saw_ttir_reduction=1 saw_value_reduction=1 saw_ttir_reduce_f32_finite_add=1 saw_ttir_scalar_reduction_store=1 saw_f32_finite_tree_policy=1 saw_tail_inactive_zero_input=1 saw_no_nan_inf_inputs=1 saw_no_dot_gemv=1 runtime_allocations=%d runtime_launches=%d elapsed_usec=%d\n",
