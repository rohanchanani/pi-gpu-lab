...
static int verify_results(uint32_t case_id, uint32_t n, float *max_abs_diff, float *max_rel_diff) {
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (diff > allowed_error(expected)) {
            if (mismatches < 8)
                printk("ERROR: ttir_sfu_recip_div case=%d n=%d i=%d gpu=%f expected=%f diff=%f rel=%f allowed=%f\n",
...
static int verify_sentinels(uint32_t n) {
    ...
    for (uint32_t i = 0; i < BUFFER_N; i++) {
        ...
        if (float_to_bits(out_values[i]) != SENTINEL_BITS) {
            if (mismatches < 8)
                printk("ERROR: ttir_sfu_recip_div sentinel n=%d i=%d bits=%x expected=%x\n",
...
void notmain(void) {
    ...
    if (vc4_program_create(&program, 0) < 0 || !program)
        panic("ttir_sfu_recip_div_f32_b16 program create failed");
    ...
    if (vc4_m2_malloc(program, &num_dev, bytes) < 0 ||
        ...
        panic("ttir_sfu_recip_div_f32_b16 allocation failed");
    ...
    for (uint32_t case_id = 0; case_id < sizeof(n_cases) / sizeof(n_cases[0]); case_id++) {
        ...
        if (vc4_m2_copy_htod(program, num_dev, num_values, bytes) < 0 ||
            ...
            ttir_sfu_recip_div_f32_b16_launch(program, grid, block, num_active, den_active, out_active, n) < 0 ||
            vc4_m2_copy_dtoh(program, out_values, out_dev, bytes) < 0) {
            printk("ERROR: ttir_sfu_recip_div launch/copy failed case=%d n=%d\n",
                   (int)case_id, (int)n);
            launch_failures++;
            continue;
        }
        ...
        printk("TTIR_SFU_RECIP_DIV_CASE case=%d n=%d waves=%d mismatches=%d sentinel_mismatches=%d hash=%x max_abs_diff=%f max_rel_diff=%f\n",
    ...
    printk("VC4_TEST_RESULT name=ttir_sfu_recip_div_f32_b16_vc4triton status=%s cases=%d total_mismatches=%d sentinel_mismatches=%d launch_failures=%d active_qpus=%d lanes=%d max_n=%d wave_boundary_n=%d output_hash=%u output_hash_nonzero=%d max_abs_diff=%f max_rel_diff=%f saw_real_ttir_snapshot=1 saw_cpp_ttir_importer=%d saw_ttir_approx_sfu=1 saw_value_approx_sfu=1 saw_ttir_approx_sfu_recip_div=1 saw_value_approx_sfu_recip_div=1 saw_approx_math_policy=1 saw_no_exact_default_math=1 runtime_allocations=%d runtime_launches=%d elapsed_usec=%d\n",
           ...
           max_abs_diff, max_rel_diff, VC4_CASE_SAW_CPP_TTIR_IMPORTER, 3,
