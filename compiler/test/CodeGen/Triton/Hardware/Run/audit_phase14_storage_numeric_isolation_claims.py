...
CLAIMS_PATH = RUN_ROOT / "phase14_storage_numeric_isolation_claims.json"
...
COMMON_RESULT_FIELDS = {
    ...
    "saw_ttir_f16_storage",
    "saw_value_f16_storage",
    "saw_no_native_f16_arithmetic",
    "saw_no_softmax_sfu",
}
...
def fail(message: str) -> None:
    raise SystemExit(f"phase14 storage/numeric isolation claim audit failed: {message}")
...
def main(argv: list[str]) -> int:
    ...
    if not isinstance(fixtures, list) or len(fixtures) != 4:
        fail("claims file must list exactly four Phase 14 storage/numeric isolation fixtures")
    ...
    if not isinstance(static_negative, list) or len(static_negative) != 5:
        fail("claims file must list native-f16, fp-to-int, bf16/fp8, int8, and i32-to-f32 static negatives")
    for entry in static_negative:
        audit_static_negative(repo, entry)

    print("TTIR_STORAGE_NUMERIC_CLAIM_AUDIT=PASS")
    print(f"fixtures={len(fixtures)}")
    print("TTIR_I32_TO_F32_CAST_HARDWARE=NOT_REQUIRED_STAGED_BY_LOWER_HALF_GAP")
