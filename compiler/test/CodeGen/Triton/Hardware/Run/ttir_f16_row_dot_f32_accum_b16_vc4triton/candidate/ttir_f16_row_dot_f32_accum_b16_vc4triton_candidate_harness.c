...
#define MAX_K 16u
#define LDA 16u
#define A_BUFFER_N (MAX_ROWS * LDA + 2u * GUARD)
#define X_BUFFER_N (MAX_K + 2u * GUARD)
#define Y_BUFFER_N (MAX_ROWS + 2u * GUARD)
#define Y_STATIC_WRITE_ROWS 16u
...
struct case_desc { uint32_t rows, k; };
static const struct case_desc cases[] = {
    {1u, 0u}, {1u, 1u}, {1u, 2u}, {1u, 15u}, {1u, 16u},
    {2u, 0u}, {2u, 1u}, {2u, 2u}, {2u, 15u}, {2u, 16u},
    {7u, 0u}, {7u, 1u}, {7u, 2u}, {7u, 15u}, {7u, 16u}
};
...
static void fill_buffers(uint32_t case_id, uint32_t rows, uint32_t k) {
...
static int verify_rows(uint32_t case_id, uint32_t rows, uint32_t k, float *max_abs_diff) {
    ...
    for (uint32_t r = 0; r < rows; r++) {
        ...
        if (diff > F32_TOLERANCE) {
            if (mismatches < 8)
                printk("ERROR: ttir f16 gemv row case=%d row=%d k=%d got=%f expected=%f diff=%f\n",
...
static int verify_sentinels(uint32_t rows, uint32_t k) {
    int mismatches = 0;
    for (uint32_t i = 0; i < Y_BUFFER_N; i++) {
        if (i >= GUARD && i < GUARD + Y_STATIC_WRITE_ROWS) continue;
        if (float_to_bits(y_values[i]) != Y_SENTINEL_BITS) {
            if (mismatches < 8) printk("ERROR: ttir f16 gemv y sentinel i=%d bits=%x\n", (int)i, float_to_bits(y_values[i]));
            mismatches++;
        }
    }
    for (uint32_t r = 0; r < rows; r++) {
        for (uint32_t c = k; c < LDA; c++) {
            ...
            if (a_values[idx] != A_PADDING_SENTINEL) {
                if (mismatches < 8) printk("ERROR: ttir f16 gemv A padding changed row=%d col=%d bits=%x\n", (int)r, (int)c, a_values[idx]);
                mismatches++;
            }
        }
    }
    for (uint32_t c = k; c < MAX_K; c++) {
        if (x_values[GUARD + c] != X_PADDING_SENTINEL) {
            if (mismatches < 8) printk("ERROR: ttir f16 gemv X padding changed col=%d bits=%x\n", (int)c, x_values[GUARD + c]);
            mismatches++;
        }
    }
    return mismatches;
...
void notmain(void) {
    ...
    if (vc4_program_create(&program, 0) < 0 || !program) panic("ttir f16 gemv create failed");
    ...
    if (vc4_m2_malloc(program, &a_dev, a_bytes) < 0 ||
        ...
        panic("ttir f16 gemv allocation failed");
    ...
    for (uint32_t case_id = 0; case_id < sizeof(cases) / sizeof(cases[0]); case_id++) {
        ...
        if (vc4_m2_copy_htod(program, a_dev, a_values, a_bytes) < 0 ||
            ...
            ttir_f16_row_dot_f32_accum_b16_launch(program, grid, block, a_active, x_active, y_active) < 0 ||
            ...
            printk("ERROR: ttir f16 gemv launch/copy failed case=%d rows=%d k=%d lda=%d\n",
                   (int)case_id, (int)rows, (int)k, (int)LDA);
            launch_failures++;
            continue;
        }
        int mismatches = verify_rows(case_id, rows, k, &max_abs_diff);
        int sentinels = verify_sentinels(rows, k);
        uint32_t hash = hash_case(rows);
        total_mismatches += mismatches;
        sentinel_mismatches += sentinels;
        output_hash ^= hash + 0x9e3779b9u + (case_id << 6) + (case_id >> 2);
        output_hash = rotl32_local(output_hash, 7u);
        printk("TTIR_F16_GEMV_CASE case=%d rows=%d k=%d lda=%d mismatches=%d sentinel_mismatches=%d hash=%x max_abs_diff=%f\n",
    ...
    printk("VC4_TEST_RESULT name=ttir_f16_row_dot_f32_accum_b16_vc4triton status=%s cases=%d total_mismatches=%d sentinel_mismatches=%d launch_failures=%d active_qpus=%d lanes=%d max_rows=%d max_k=%d max_lda=%d output_hash=%u output_hash_nonzero=%d max_abs_diff=%f saw_real_ttir_snapshot=1 saw_cpp_ttir_importer=%d saw_ttir_f16_storage=1 saw_value_f16_storage=1 saw_ttir_f16_gemv_input_storage=1 saw_ttir_gemv_f32_accum=1 saw_value_f16_gemv_input_storage=1 saw_value_gemv_f32_accum=1 saw_f16_storage_finite_policy=1 saw_no_native_f16_arithmetic=1 saw_no_softmax_sfu=1 runtime_allocations=%d runtime_launches=%d elapsed_usec=%d\n",
           status, (int)(sizeof(cases) / sizeof(cases[0])), total_mismatches, sentinel_mismatches,
           launch_failures, ACTIVE_QPUS, LANES, MAX_ROWS, MAX_K, LDA, output_hash,
           output_hash != 0u ? 1 : 0, max_abs_diff, VC4_CASE_SAW_CPP_TTIR_IMPORTER,
