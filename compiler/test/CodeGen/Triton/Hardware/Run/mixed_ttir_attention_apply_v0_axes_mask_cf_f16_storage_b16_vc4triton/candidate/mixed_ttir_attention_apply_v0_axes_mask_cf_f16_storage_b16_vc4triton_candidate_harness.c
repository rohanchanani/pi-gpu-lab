...
void notmain(void) {
    ...
    if (vc4_program_create(&program, 0) < 0 || !program)
        panic("mixed_ttir_attention_apply program create failed");
    ...
    if (vc4_m2_malloc(program, &scores_dev, scores_bytes) < 0 ||
        ...
        panic("mixed_ttir_attention_apply allocation failed");
    ...
    for (uint32_t case_id = 0; case_id < sizeof(cases) / sizeof(cases[0]); case_id++) {
        ...
        if (vc4_m2_copy_htod(program, scores_dev, scores_values, scores_bytes) < 0 ||
            ...
            mixed_ttir_attention_apply_v0_axes_mask_cf_f16_storage_b16_kernel_launch(
        ...
        printk("MIXED_TTIR_ATTENTION_APPLY_AXIS_MASK_CF_F16_CASE case=%d q_rows=%d out_dims=%d k=%d lds=%d ldv=%d ldo=%d scale=%f grid_z=%d block_x=%d mismatches=%d sentinel_mismatches=%d hash=%x max_abs_diff=%f max_rel_diff=%f max_audit_diff=%f\n",
               (int)case_id, (int)tc->q_rows, (int)tc->out_dims, (int)tc->k,
    ...
    printk("VC4_TEST_RESULT name=mixed_ttir_attention_apply_v0_axes_mask_cf_f16_storage_b16_vc4triton status=%s cases=%d total_mismatches=%d sentinel_mismatches=%d launch_failures=%d active_qpus=%d lanes=%d block_x=%d output_hash=%u output_hash_nonzero=%d max_abs_diff=%f max_rel_diff=%f max_audit_diff=%f runtime_allocations=%d runtime_launches=%d saw_real_ttir_snapshot=1 saw_cpp_ttir_importer=%d saw_value_surface_verification=1 saw_value_to_vc4kernel=1 saw_value_scf_cf=1 saw_ttir_elementwise=1 saw_ttir_control_flow=1 saw_ttir_multi_axis=1 saw_ttir_program_id_axis1=1 saw_ttir_program_id_axis2=1 saw_ttir_mask_tail=1 saw_ttir_row_strided_memory=1 saw_ttir_reduction_f32_finite_add=1 saw_ttir_scalar_reduction_store=1 saw_ttir_f16_storage_load=1 saw_ttir_f32_compute_after_f16_load=1 saw_value_f16_storage=1 saw_ttir_approx_sfu_exp=1 saw_ttir_approx_sfu_recip_div=1 saw_ttir_finite_f32_max_reduction=1 saw_ttir_softmax_v0=1 saw_ttir_attention_apply_v0=1 saw_ttir_precomputed_scores=1 saw_ttir_transposed_v_layout=1 saw_ttir_weighted_sum_reduction=1 saw_ttir_scalar_result_store=1 saw_value_approx_sfu=1 saw_value_softmax_v0=1 saw_scalar_to_vector_f32_broadcast=1 saw_approx_math_policy=1 saw_softmax_uses_natural_exp=1 saw_no_exact_default_math=1 saw_no_multiblock_softmax=1 saw_no_scalar_global_load=1 saw_no_nontransposed_v_gather=1 saw_no_qk_score_generation=1 saw_no_full_attention=1 saw_no_tl_dot_tt_dot=1 saw_no_vector_contract=1 saw_no_gemm=1 saw_row_padding_sentinels=1 saw_output_padding_sentinels=1 elapsed_usec=%d\n",
