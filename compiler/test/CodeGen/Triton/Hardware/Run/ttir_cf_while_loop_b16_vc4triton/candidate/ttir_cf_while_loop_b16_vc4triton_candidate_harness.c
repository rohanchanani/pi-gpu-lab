...
static const struct loop_case loop_cases[] = {
    {0u, 0}, {1u, 1}, {2u, 2}, {15u, 5}, {16u, 7},
    {17u, 0}, {31u, 1}, {32u, 2}, {33u, 5}, {63u, 7},
    {64u, 0}, {65u, 1}, {127u, 2}, {128u, 5}, {129u, 7},
    {193u, 0}, {384u, 1}, {385u, 2}, {999u, 5}, {1000u, 7}
};
...
static int verify_results(uint32_t n, int32_t trip_count) {
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (got != expected) {
            if (mismatches < 8)
                printk("ERROR: ttir_cf_while n=%d trip=%d i=%d gpu_bits=%x expected_bits=%x gpu=%f expected=%f\n",
...
static int verify_sentinels(uint32_t n) {
    ...
    for (uint32_t i = 0; i < BUFFER_N; i++) {
        ...
        if (got != SENTINEL_BITS) {
            if (mismatches < 8)
                printk("ERROR: ttir_cf_while sentinel n=%d i=%d bits=%x expected=%x\n",
...
void notmain(void) {
    ...
    if (vc4_m2_malloc(program, &x_dev, bytes) < 0 ||
        ...
        panic("ttir_cf_while allocation failed");
    ...
    for (uint32_t case_id = 0; case_id < sizeof(loop_cases) / sizeof(loop_cases[0]); case_id++) {
        ...
        if (vc4_m2_copy_htod(program, x_dev, x_values, bytes) < 0 ||
            ...
            ttir_cf_while_loop_b16_kernel_launch(program, grid, block,
                                                 x_active, out_active, n,
                                                 trip_count) < 0 ||
            vc4_m2_copy_dtoh(program, out_values, out_dev, bytes) < 0) {
            printk("ERROR: ttir_cf_while launch/copy failed trip=%d n=%d\n",
        ...
        printk("TTIR_CF_WHILE_CASE case=%d n=%d trip=%d waves=%d coverage=%d mismatches=%d sentinel_mismatches=%d hash=%x\n",
    ...
    printk("VC4_TEST_RESULT name=ttir_cf_while_loop_b16_vc4triton status=%s cases=%d elements_checked=%d total_mismatches=%d sentinel_mismatches=%d launch_failures=%d active_qpus=%d lanes=%d max_n=%d max_coverage_n=%d buffer_n=%d output_hash=%u saw_nonzero_output_hash=%d saw_real_ttir_snapshot=%d saw_cpp_ttir_importer=%d saw_value_scf_cf=%d saw_vc4value_to_vc4kernel=%d saw_ttir_scf_while=%d saw_tail_mask_load_store=%d saw_cpu_oracle_sentinel=%d saw_no_arith_sitofp=%d runtime_allocations=%d runtime_launches=%d elapsed_usec=%d\n",
