...
CLAIMS_PATH = RUN_ROOT / "phase10_mask_memory_isolation_claims.json"
...
COMMON_RESULT_FIELDS = {
    ...
    "saw_ttir_mask_memory",
    "saw_value_mask_classifier",
}
...
def fail(message: str) -> None:
    raise SystemExit(f"phase10 mask-memory isolation claim audit failed: {message}")
...
def audit_fixture(repo: Path, fixture: dict) -> None:
    ...
    for op in fixture.get("forbidden_ops", []):
        if op in ttir_text:
            fail(f"{name}: no-mask/full fixture contains forbidden marker {op}")
    ...
    for one_field in required_fields - {"total_mismatches", "sentinel_mismatches", "launch_failures", "active_qpus", "lanes"}:
...
def main(argv: list[str]) -> int:
    ...
    if not isinstance(fixtures, list) or len(fixtures) != 4:
        fail("claims file must list exactly four Phase 10 mask-memory isolation fixtures")
    ...
    if not isinstance(static_negative, list) or len(static_negative) != 2:
        fail("claims file must list sparse-mask and nonzero-other static negative coverage")
    for entry in static_negative:
        audit_static_negative(repo, entry)

    print("TTIR_MASK_MEMORY_CLAIM_AUDIT=PASS")
